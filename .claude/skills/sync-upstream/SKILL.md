...
`scripts/sync-chibirigor-docs.mjs` into **two editions** —
`src/content/docs/chibirigor/` (EN, root locale) and
`src/content/docs/ja/chibirigor/` (JA), plus their per-locale figures under
`public/` (all git-ignored, regenerated on every build). Upstream ships both
languages, so neither edition is a translation target: there is **no
translation step and no `check-translations` involvement**. Updating it is a
pure submodule-pointer bump — the generated pages are gitignored, so only the
pointer is ever staged. See [`../../../AGENTS.md`](../../../AGENTS.md)
§ "chibirigor book".
...
(`link: '/chibirigor/'`) and the glossary (`slug: 'chibirigor/glossary'`);
both are locale-agnostic and resolve to the reader's locale, so do **not**
write a `/ja/` prefix into them. Touch the config only if upstream moves or
renames the glossary, or restructures the top-level `little/` / `seasoned/` /
`appendix/` directory layout itself. Fold any such edit into the chibirigor
bump commit.
...
### Translation drift (`check-translations.mjs`)
...
`check-translations.mjs` sees one thing: a `sourceSha` that stopped matching.
Three sync hazards are invisible to it, and all three share a shape — a
**structural** change upstream (a page added, renamed, renumbered or removed; a
diagnostic reworded) breaks something hand-maintained, and nothing in the survey
above says so. Walk all three on every sync; fold any fix into the **second**
commit. A run is done here only when each blind spot is checked or ruled out as
N/A.

**1 — Handbook & User Manual sidebar.** Two groups in
[`astro.config.mjs`](../../../astro.config.mjs) list their pages **explicitly**
rather than via `autogenerate`: **Handbook** (chapters `01`–`NN` plus the
appendix sub-groups) and **User Manual** (the `manual` index plus its numbered
chapters — explicit so it stops before `manual/plugins/`, which is promoted to
its own autogenerated **Plugin Reference** group). Everything else
(`type-specification`, `internal-spec`, `adr`, `design`, `notes`, and the plugin
pages) discovers itself. So a new/renamed/removed handbook chapter or appendix,
or a top-level manual chapter, is silently absent from — or 404s in — the
sidebar until the `items` array is edited:

```sh
ls upstream/rigor/docs/handbook/*.md
ls upstream/rigor/docs/manual/*.md       # top-level chapters only
```

Diff each list against the config (ignoring `README.md`, which maps to the group
root slug `handbook` / `manual`) and insert `{ slug: 'handbook/<name>' }` or
`{ slug: 'manual/<name>' }` in chapter order; drop entries for removed pages.
Handbook appendices go under "Coming from another type checker" or "Connections
to type theory" per the upstream README's table of contents.

**2 — llms.txt curated index.** The rest of the llmstxt.org surface is derived
and needs no action (see [`../../../AGENTS.md`](../../../AGENTS.md) § "llms.txt,
llms-full.txt, and per-page Markdown twins"), but the two index bodies,
[`src/llms/llms.md`](../../../src/llms/llms.md) and
[`src/llms/llms-ja.md`](../../../src/llms/llms-ja.md), are hand-authored and
link a curated set of pages by `.md` path (`/manual/14-rails-quickstart.md`,
`/changelog-01x.md`, …). A build-time guard **fails `pnpm build`** when one of
those links stops resolving — that guard, not `check-translations.mjs`, is what
catches this, so run `pnpm build` once after a structural sync. Fix a broken
link in **both** files (they stay in lockstep) and `git add src/llms/`.
`llms-ja.md` is hand-owned JA content that the `ja/**` normalisers do not reach,
so apply the Step 4 term/typography rules to it by hand.

**3 — Splash showcase (needs Nix).** The two splash pages
(`src/content/docs/index.mdx` and its `ja/` mirror) print real `rigor annotate`
/ `rigor check` output inside their `<ShowcasePanel>` code blocks, which a bump
can silently invalidate (reworded diagnostics, changed display forms, new plugin
messages):
...
It runs every showcase snippet against the **pinned** submodule through the
upstream Nix Flake, diffs the page's `#=>` / diagnostic comments against actual
CLI output, and enforces EN↔JA code-block lockstep. It is deliberately outside
`pnpm build` (the site CI has no Nix), so nothing runs it for you. One-time
prerequisite:
...
A failure means the page must be re-aligned to the new real output.
...
they are idempotent and safe on already-normalised content. JA-native
upstream pages need nothing: `sync-rigor-docs.mjs` already pipes them
through the typography normaliser as it writes the JA tree. The **terms**
normaliser intentionally skips
...
# Plus any Step 3 blind-spot fix: astro.config.mjs (sidebar),
# src/llms/ (llms.txt index), src/content/docs{,/ja}/index.mdx (showcase).
...
- **A JA-native upstream file shows up in `src/content/docs/ja/`
  as auto-overwritten with typography changes**: expected, and already
  correct — the sync applies the typography normaliser on the way in.
  These pages are tracked (unlike the EN tree), so stage them with the
  second commit; do not hand-edit them.
