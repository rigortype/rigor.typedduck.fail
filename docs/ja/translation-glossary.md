...
| erasure | 消去／消去型 | 初出: 消去（erasure）／消去型（type erasure） | `type-specification/rbs-erasure` に準拠。音訳「イレイジャ」は廃止し「消去」に統一 |
| tightening | 引き締め | 初出: 引き締め（tightening） | `sig-gen` において宣言より狭い型を導出する動作（`tighter-return` 等）。「ナローイング」との混同を避ける |
