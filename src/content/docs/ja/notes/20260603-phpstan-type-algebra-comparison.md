...
sourceCommit: "75c5ae91620178cccb889d93997ecc9dfc0467f5"
...
| --- | --- | --- | --- |
| `TypeCombinator::union` | `Type::Combinator.union` | [`combinator.rb:363`](https://github.com/rigortype/rigor/blob/master/lib/rigor/type/combinator.rb) | ✅ 同等（決定論的正規化 + lattice恒等則） |
| `TypeCombinator::intersect` | `Type::Combinator.intersection` | [`combinator.rb:325`](https://github.com/rigortype/rigor/blob/master/lib/rigor/type/combinator.rb) | ✅ 同等 |
| `TypeCombinator::remove` | `Type::Combinator.difference`（`T - U`演算子） | [`combinator.rb:123`](https://github.com/rigortype/rigor/blob/master/lib/rigor/type/combinator.rb) | ✅ Rigorは明示的差分演算子を持ち、診断表示も`D - U`形を持つ（[type-operators.md](../../type-specification/type-operators/)）。むしろPHPStanより表現が厚い |
...
| `Type::isSuperTypeOf` | `Type#accepts(other, mode:)` → `AcceptsResult` | 各キャリア`accepts`（例[`constant.rb:114`](https://github.com/rigortype/rigor/blob/master/lib/rigor/type/constant.rb)） | ✅ gradual consistencyとして実装。厳密subtype（`subtype_of`）はslice 5+ で`SubtypeResult`予定 |
...
| `IntegerRangeType`範囲算術 | `try_fold_binary_range`ほか（additive / multiply / divide / comparison、corner計算 + `0×∞=0`等の代数的配慮） | [`constant_folding.rb:800`](https://github.com/rigortype/rigor/blob/master/lib/rigor/inference/method_dispatcher/constant_folding.rb) | ✅ **同等水準**。範囲 × 範囲の四隅積・除算ガード・無限端処理まで実装済み |
...
| offsetアクセス（`getOffsetValueType`等） | `indexed_access`型関数 + ShapeDispatch（Tuple/HashShapeの`[]`/`fetch`/`dig`） | [`shape_dispatch.rb`](https://github.com/rigortype/rigor/blob/master/lib/rigor/inference/method_dispatcher/shape_dispatch.rb), [type-operators.md](../../type-specification/type-operators/) | ⚠️ エンジン内部では精密。**プラグインから呼べるoffsetファサードは`indexed_access`程度に限定** |
...
| プラグインの型構築facade | `services.type`（= `Type::Combinator`） | [`services.rb:43`](https://github.com/rigortype/rigor/blob/master/lib/rigor/plugin/services.rb) | ✅ 正規化必須のfacadeを注入（PHPStanの「`new`より`TypeCombinator`」方針と一致） |

プラグイン拡張点（[`plugin/base.rb`](https://github.com/rigortype/rigor/blob/master/lib/rigor/plugin/base.rb)）は`node_rule`（86, 137行）/ `dynamic_return`（210行）/ `type_specifier`（239行）/ `producer`（86行）。PHPStanの`DynamicMethodReturnTypeExtension` ≈ Rigor `dynamic_return`、`TypeSpecifierExtension` ≈ `type_specifier`。**`OperatorTypeSpecifyingExtension`に対応するフックだけが欠けている**（`grep -i operator lib/rigor/plugin/`は空）。
...
当初「Rigorにはプラグインの二項演算フックがない」と仮説したが、**2026-06-03のコード・スパイク＋統合スペックで反証・確定した**（[`spec/integration/plugin_operator_dynamic_return_spec.rb`](https://github.com/rigortype/rigor/blob/master/spec/integration/plugin_operator_dynamic_return_spec.rb)、4例green）。Rubyの`a + b`はPrismでは`name: :+`の`Prism::CallNode`であり、通常の呼び出しと同じく`call_type_for` → `MethodDispatcher.dispatch`に`call_node: node` / `method_name: :+` / `scope:`付きで流れる（[`expression_typer.rb:1233`](https://github.com/rigortype/rigor/blob/master/lib/rigor/inference/expression_typer.rb)）。dispatchの優先順位は**`ConstantFolding`（precise tiers）→ `try_plugin_contribution`（`dynamic_return`）→ RBS**（[`method_dispatcher.rb:74-97`](https://github.com/rigortype/rigor/blob/master/lib/rigor/inference/method_dispatcher.rb)）。プラグイン所有のレシーバは`Nominal[CustomType]`であって`Constant` / `IntegerRange`ではないためConstantFoldingは`nil`を返し、**dispatchはplugin tierに落ちる**。`dynamic_return_type`はreceiverクラスのみでゲートしmethod名は一切問わない（[`base.rb:382`](https://github.com/rigortype/rigor/blob/master/lib/rigor/plugin/base.rb)）。スペックは`:+ :- :* :/`すべてでブロックが発火し、寄与した型が`(a <op> b)`の結果型として確定することを確認済み。
...
**評価2026-06-03 — 却下（需要なし）**。 Rubyのキャストは`x.to_i` / `Integer(x)`等のメソッド呼び出しで、既にdispatch経由で精密化される。真偽値の扱いは`narrow_truthy`/`narrow_falsey`（[`narrowing.rb:67`](https://github.com/rigortype/rigor/blob/master/lib/rigor/inference/narrowing.rb)、エンジン内蔵）＋ プラグイン拡張可能な`type_specifier`が等価機構。型→型のcoercion facadeを欲する消費者はexamples/pluginsに皆無。
...
**評価2026-06-03 — 保留（条件付き）**。 `ShapeDispatch`はTuple/HashShape限定のclosed tier（[`shape_dispatch.rb`](https://github.com/rigortype/rigor/blob/master/lib/rigor/inference/method_dispatcher/shape_dispatch.rb)）で、独自コンテナ型を持つプラグインは現状ゼロ。将来カスタムコレクション型を定義するプラグインが現れたときの最有力拡張候補ではあるが、消費者が出るまでは着手しない。
...
6. **演算子糖衣 → dynamic_return**（**LANDED** 2026-06-03 — [`spec/integration/plugin_operator_dynamic_return_spec.rb`](https://github.com/rigortype/rigor/blob/master/spec/integration/plugin_operator_dynamic_return_spec.rb)）：`Nominal[Custom] <op> Custom`が`dynamic_return receivers: ["Custom"]`のブロックに`call_node`として届き、結果型がplugin tierで確定すること（`:+ :- :* :/`全数 + 宣言外演算子の辞退 + coerce方向の左偏重型付けの4例）。観測にはcore型センチネルが必要（ユーザー定義クラスは偽陽性回避のため未定義メソッド診断が出ない）。
...
- Rigor: [`combinator.rb`](https://github.com/rigortype/rigor/blob/master/lib/rigor/type/combinator.rb), [`constant_folding.rb`](https://github.com/rigortype/rigor/blob/master/lib/rigor/inference/method_dispatcher/constant_folding.rb), [`shape_dispatch.rb`](https://github.com/rigortype/rigor/blob/master/lib/rigor/inference/method_dispatcher/shape_dispatch.rb), [`plugin/base.rb`](https://github.com/rigortype/rigor/blob/master/lib/rigor/plugin/base.rb), [`plugin/services.rb`](https://github.com/rigortype/rigor/blob/master/lib/rigor/plugin/services.rb), [internal-type-api.md](../../internal-spec/internal-type-api/), [type-operators.md](../../type-specification/type-operators/), [value-lattice.md](../../type-specification/value-lattice/), [relations-and-certainty.md](../../type-specification/relations-and-certainty/)。
