...
1. インストール済みのソースルートを解決する。**主要なリゾルバは対象プロジェクト自身のバンドル**
...
2. その慣習的なエントリファイル（`lib/<name>.rb`、およびダッシュ → ディレクトリの変種）をPrismで
   パースし、ルート名の下で**トップレベル**のクラス／モジュール／定数の宣言を記録する。エントリー
...
- **`vendor/bundle`／`BUNDLE_PATH`のプロジェクト**（Docker／CIの標準）── 主要なリゾルバが
  対象の実際のgemを読むので、RBSのないgemが**すべて**カバーされる。これはこの修正が想定して
  作られたケースである。
- **グローバルgemのプロジェクト**（`mise`／`rbenv`／システム、`vendor/bundle`なし）── バンドル
  リゾルバは何も見つけられず（rigorは対象の`GEM_PATH`を見られない。それは別のRubyの下にある）、
