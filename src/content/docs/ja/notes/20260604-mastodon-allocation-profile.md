---
title: "Mastodonでの`rigor check`のプロファイリング：アロケーションバウンドである"
description: "rigortype/rigor docs/notes/20260604-mastodon-allocation-profile.mdの翻訳です。"
...
二次的なクラスタは構造的なものだ: `Scope`はナローイング/バインディング変更のたびに全体を`rebuild`するイミュータブルな~20フィールドの値オブジェクトであり（アロケーション3.0 %）、`CallContext`はディスパッチごとに構築される`Data`だ（アロケーション4.2 %）。
