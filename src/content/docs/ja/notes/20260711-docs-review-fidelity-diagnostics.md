...
- **`documentation_url`のアンカースキーム**（`04:27–35`）は`RuleCatalog.doc_anchor`（`rule-<id with dots→dashes>`）および`DOCUMENTATION_BASE`と一致する。`<a id>`アンカーは存在する。`rbs_extended.unsatisfied-conformance`は`documentation_url`を持たず`rigor explain`がそれを解決できないという注記は正しい（`RULE_FAMILIES = call flow assert dump def`、`rbs_extended`エントリーなし → `resolve`は`[]`を返す）。
...
- **セッション変更2（ユニオンアーム述語の極性、`present?`/`blank?`、`narrowing.rb:876`）** —— 実装済みと確認（ActiveSupportメソッド）。`present?`/`blank?`がnil許容レシーバーをナローイングできないと主張するハンドブックの例はない。`03-narrowing.md`の述語表と「ナローイングされないもの」のリストは、それらを扱ってもいないし矛盾もしていない。
