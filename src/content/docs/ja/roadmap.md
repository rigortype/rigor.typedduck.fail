...
このファイルは**計画資料**であり、リリースログではありません。「何が出荷されたか」の記録については、[`CHANGELOG.md`](https://github.com/rigortype/rigor/blob/master/CHANGELOG.md)（アクティブな`0.2.x`サイクル）と、アーカイブの[`docs/CHANGELOG-0.1.x.md`](../changelog-0.1.x/)（`0.1.x`）および[`docs/CHANGELOG-0.0.x.md`](../changelog-0.0.x/)（`0.0.x`）を参照してください。
...
- **セルフサービスのオンボーディング** —— SKILLトリオ + `docs/install.md`経由（v0.1.9 / v0.1.13 / v0.1.14）、**`0.2.x`ラインでは[ADR-73](../adr/73-skill-driven-user-experience/)によりアップグレード**（2026-06-20着地、未リリース）: `rigor-next-steps`エントリポイント + ライブな`rigor skill describe`（存在チェックのみのプローブ → 推奨される次のスキル）を13スキルのカタログ上で、vercel-labs/skills + バンドルされたgem経由で配布し、現地試験で堅牢化（conference-app + 13モデルのOpenCodeクロスベンダー検証）。先送りされたフォローアップは後述の§「Future cycles」にある。
...
[2026-06-22の互換性を壊さない強化サーベイ](../notes/20260622-rigor-0.2.x-compatibility-safe-strengthening-survey/)は、`0.2.x`評価ラインの間に[`docs/compatibility.md`](../compatibility/) / [ADR-50](../adr/50-release-engineering-and-stability-strategy/)の規律を破らずにRigorがどこで強化できるかをカタログ化した。サーベイの11の機会 + P0〜P7バックログは、下記でその後方互換性コストによって分類される（このサマリーと食い違う場合はサーベイが拘束する;サーベイ自体はどの実装タスクも選ばず、バージョンバンプ／ベースライン移行／デフォルトseverityの昇格／コミットのいずれも許可しない）。いくつかの項目はバケット間で**分裂する** —— 精度加算的なコアは今すぐ安全だが、より声高なデフォルトやその上に構築された新しい規律はそうではない。

**（1）BCコストなし — 即座に実装可能**（ルートA内部、またはルートBの加算的デフォルトoff;唯一のゲートは実証的な偽陽性リスクであり、正式なAPI破壊ではない）:

- **セルフミューテーション / ティースバッチ**（サーベイ§1、P1） — dev専用の`tool/mutation/`、ADR-50の凍結サーフェスの外;アナライザー自身のオラクルをパブリックサーフェス変更ゼロで改善する。すでに[Future cycles §アナライザーのセルフテスト](#アナライザーのセルフテスト--ティース計測--型保護カバレッジadr-62--adr-63)トラックである。
- **決定論的なビルトイン / stdlib精度 —— FP安全なfold**（サーベイ§2、P3） — `MethodDispatcher.dispatch`が、新しい診断を発火させずに`Dynamic`を削減する具体的なレシーバー上でfoldする;ユーザーがオーバーライド可能な`coerce`を経由するfoldは見送る。（*実際の診断を広げる*foldはバケット3。）
- **`Dynamic[T]`来歴ラベル**（サーベイ§3、P2） — dynamic起源の原因ファクトを内部的に運び、`coverage --protection`ラベル / `--format json`メタデータ経由で加算的に表面化する;`untyped = Dynamic[top]`関係の意味論を保存する。最も価値の高い説明レバー（汎用的な穴を次のアクションに変える）。（strict-dynamicな*ポリシー*はバケット2。）
- **キャッシュ / インクリメンタル / パフォーマンス**（サーベイ§5） — 契約外の内部事項;シリアライズされた意味が変わるときは常に`Cache::Descriptor::SCHEMA_VERSION` / `Cache::Store::FORMAT_VERSION`（マーカー`4.2`）をbumpし、陳腐化したエントリーが*ミス*するようにし、決して誤って読まれないようにする;ウォールタイムのノイズではなく決定論的なアロケーションシグナルから検証する。
- **設定可能な推論バジェット、現状挙動をデフォルトに**（サーベイ§6） — spec[`budgets:`](../type-specification/inference-budgets/)テーブルを、今日のハードコードされたガードと等しいデフォルトで配線し、まず枯渇を説明として扱う（`RIGOR_BUDGET_TRACE`を拡張）;新しい`.rigor.yml`キーは意図的に命名する（一度文書化されればパブリックな語彙になる）、内部の燃料定数は決して漏らさない。
- **加算的なCLIヘルパー**（サーベイ§7、P5） — `rigor doctor` / `rigor upgrade` / `rigor skill describe --deep`は既存のcheck / baseline / プラグインのエビデンスを再利用し、デフォルトの`check`は変更しない;JSON出力はどれも初日から安定した契約として設計し、深いプローブはオプトインのままにする。（`describe --deep` + カバレッジ扱いやすさラベルはすでに[SKILL駆動のオンボーディングUX](#skill駆動のオンボーディングuxadr-732026-06-20着地--先送りされたフォローアップ)の下で追跡されている。）
- **加算的なプラグイン精度**（サーベイ§8、P6） — バンドルプラグインの認識器、`config_schema`のデフォルト、*オプションの*フック;新しい認識器は正の認識トレースをデフォルトのエラーに変えてはならない。（新しいパブリックプラグイン*サービス*はバケット2;必須フックの変更はバケット4。）
- **構造化された診断メタデータ**（サーベイ§9） — `rule_catalog.rb`ごとに`evidence_tier`形の加算的JSONフィールドを増やす（nilなら省略）ことで、エージェント/CIが`message`の散文ではなくデータで分岐できるようにする;新しいフィールドはそれぞれ凍結された文書化済みの意味論とともに出荷し、決して表示上のミラーにしない。
- **off / infoファーストの新規ルール**（サーベイ§10） — 安定したid（`CheckRules::ALL_RULES`、エイリアスは保持）で`:off` / `:info`として出荷される新しい診断はデフォルトの実行を何も変えない;外部コーパスのFPスイープの後にのみ昇格させる。（新しい*著作規律*を課すルールはバケット2。）

**（2）BCを伴うが`bleeding_edge:`カードの背後で出荷可能**（新しい著作上の期待、デフォルトoff、ユーザーの`severity_overrides:`が依然勝つ;オーバーレイ`BleedingEdge::FEATURES`は今日空）:

- **最初の`BleedingEdge::FEATURES`エントリー**（サーベイ§11、P7） — 配線済みオーバーレイの背後の単一データ行;安定したid + マイグレーションノートを持つ本物の次メジャーの規律のために予約する、通常の精度修正の投げ込み場には決してしない。あらゆる将来のfeature idのスタイルを定める。
- **Strict-dynamic規律**（サーベイ§3の末尾） — バケット1の来歴ファクトの上に構築される、説明のつかない`Dynamic`値に対して以前クリーンだったコードを*失敗させる*あらゆるポリシー。
- **新規律ルール**（サーベイ§10の末尾） — 異なる著作スタイルを要求することでイディオマティックなRubyを失敗させる診断は、サイレントなデフォルト昇格ではなく`bleeding_edge:`経由で出荷される。
- **新しいパブリックプラグインサービス / 読み取り側の追加**（サーベイ§8） — 加算的だが契約の拡張である: 一致するRBS + 意図的な`public_api_drift_spec.rb`更新とともに出荷し、v1.0フリーズに対して設計される。

**（3）BCを伴う、大規模な意味論 / エンジン作業 — 別ブランチでの準備**（信頼できるようになる前に外部コーパスの偽陽性スイープが必要;`make verify`だけでは不十分 —— P0）:

- **レシーバー型付け / nil許容性 / フロー / オーバーライド診断のデフォルト拡張**（サーベイP0） — 精度は内部的に存在するが、デフォルトプロファイルへの昇格には、Rigor自身のコードが供給できないRails / ActiveSupport / DSL / モンキーパッチ / RBSギャップのコーパススイープが必要。
- ***実際の*診断を広げる精度fold**（サーベイ§2の末尾） — `Dynamic`レシーバーを具体化する鋭いfoldは、動作しているコードに対して`always-truthy` / possible-nil / undefined-methodを発火させうる（liquidのStringScanner / `||`-`&&`のエッジナローイングの先例）;コーパス差分でゲートする。
- **`freeze` / `dup`のシェイプキャリア保存**（サーベイ§4の末尾） — 既知の反射的な`always-truthy`相互作用が解決されるまで先送り;保守的な無効化（§4の残り）はバケット1。
- **ベースラインフォーマットの移行**（サーベイ「artifact format」+ Non-goals） — キャッシュのbumpより読者に見える;`Analysis::Baseline::CURRENT_VERSION`は設計上ミスマッチでraiseするため、フォーマット変更はマイグレーションパスとガイダンスを必要とするパブリックサーフェスの作業であり、独自のブランチで行う。

**（4）compat規律の下で禁止される / マイナーで現実的に不可能**（メジャーバージョンのみ;サーベイの「避けるべき互換性の罠」）:

- **非互換な`rigor:v1:`アノテーション文法 / 意味論** — *新しい*バージョンプレフィックスを導入する;`v1:`の意味を決して再解釈しない。
- **`untyped = Dynamic[top]`の意味論を変える** — 関係契約の変更;§3の来歴作業はこれを保存するために明示的に要求される。
- **レガシーエイリアスなしで診断idをリネームする** — リネーム*かつ*`LEGACY_RULE_ALIASES`エントリーなら互換;エイリアスなしのリネームが罠。
- **プラグインフックをマイナーで必須にする** — オプションのフックのみが加算的;必須フックの変更は既存のマニフェストを無効化する。
- **変更されたアーティファクトスキーマをサイレントに誤読する** — スキーマ変更は（raise / missで）無効化しなければならず、陳腐化したバイトを現在のフォーマットとして決して再解釈してはならない。
...
- **`lib/rigor`自体のセルフミューテーションテスト（Product C）—— ハーネス着地済み（dev専用、凍結サーフェスの外）;コールドメソッドバックログは消化済み**。ADR-62/69/70の機構を*内向き*に向ける: Rigor自身の実装をミューテートし、self-check（型軸、`DiagnosticOracle`）かRigor自身のRSpecスイート（テスト軸、`TestSuiteOracle`）のいずれかでkillする;両方の生存者は実装の穴だ。`tool/mutation/self_mutate.rb`は`Protection::{MutationScanner,TestSuiteOracle}`の薄いドライバであり、Product C固有の部品のみを足す —— **in-bundle**のテストランナー（出荷されるオラクルはBundlerの環境を剥がす、*外部*のGemfileには正しいがRigor自身のスイートには誤り）、規約によるspec選択、ディスク復元の安全性、そして型生存者をワンショットのスイート行カバレッジインデックス（`spec_helper`のオプトインな`COVERAGE_JSON`ダンプ）に対して分類する`--coverage-gap`のツリー全体モードであり、安価で高信頼な層がrspec実行を**ゼロ**回しか要さないようにする。基礎 + 生きたトラッカー: [`docs/notes/20260618-self-mutation-testing-plan.md`](../notes/20260618-self-mutation-testing-plan/)。**方法論の教訓（ロードベアリング）:**カバレッジギャップのカウントは**メソッドコールドネス**によってノイズ除去されねばならない —— Rubyの行カバレッジは複数行の式をその最初の行に計上するので、素朴な「行 ∉ カバー集合」チェックはカバー済みの式の継続行を誤フラグする;穴は*完全に未カバーの`def`*の内側でのみカウントされ、クラスボディ／定数のサイトは除外される（ロジックではなくデータ）。このファネルは生のツリー全体カウントを1969 → 214 → 22にした。**結果:** `lib/rigor`の310ファイルすべてをスイープ —— コールドメソッドの穴は`cli/mcp_command`と`trinary`だけだった（両方ともクローズ済み）;**ユニットスイートはメソッドレベルで完全**。残る`needs-verification`層（*カバー済み*の行の上の型生存者 —— カバー済み≠アサート済み）は融合モードを介して**ファイルごとに**進められ（ほとんどの生存者は等価ミュータント —— メッセージ / inspectテキスト —— か、より広いspecでカバーされている）。**2026-06-21: 有効性スイープはメカニカルで反復可能なワークフローへと成熟した** —— 候補ファイルを〜8本バッチ（60〜300 LOC、specあり、未計測のもの） → 生存者リストを読む → 本物か等価ミュータントのフロアかを裁定する → 対象を絞ったテストを書く（privateヘルパーは`.send`経由） → ハーネスをファイルごとに再実行する → spec専用でコミットする（rspec + rubocopで十分;`make check`/`check-plugins`は`lib`/`plugins`を解析するが`spec`は解析しない）。7バッチにわたって**〜28ファイル**を等価ミュータントのフロアまでクローズした（スキャナ群、HKTクラスタ、LSPプロバイダ群、`mcp/server`、依存関係ソース推論とCLIコマンドファイル群、…）;**バッチごとの記録 + killingテクニックのカタログ**は生きたトラッカーに保存されている。**2つの運用ポリシー（ロードベアリング）:**（1）ハーネスのテスト軸は規約でマップされた*ユニット*specのみを実行し、インテグレーション/CLIディスパッチャーspecは決して実行しない —— そのため`cli/*_command`のオーケストレーションは大きなカウントを報告するが、大半は*integration-blindness*である —— ディスパッチャーspecに照らしてトリアージし、コマンドの未テストの*デフォルト*モードにのみユニットテストを追加すること（メッセージ/ヘルプの末尾ではない）;（2）融合ハーネスを`make verify`（または別のハーネス呼び出し）と並行して実行しないこと —— それぞれがコールドなenv+スキャンを実行し、リソースを食い合う。次の実装者のために**6つの再利用可能なkillingテクニック**が記録されている: `fetch(_, default)`のデフォルト値に対する不在キーテスト;構造化フィールドを共有する2つのパスを分離するパス固有のメッセージテキスト;ビルトした配列/argvの各要素のソースを固定する`include`ではなく厳密な`eq`;nil区切り文字ミュータントが依然連結できる`join("\n")`に対する行数（部分文字列ではなく）;`bytesize`対長さを噛む多バイトフィクスチャ;privateヘルパー用の`.send`。**需要ゲート付き / 次の課題:**未計測の60〜300 LOCファイルの〜50本のバッチ処理を続け、次に300 LOC超のエンジン層へ;`overload_selector`の全ブロックオーバーロードの残差2件（ファブリケートしたメソッド型が必要）;意味的オペレーター（現在のセットは実行可能だが診断の形をしている）;規約による選択を置き換える`{line → specs}`のspecごとインデックス;ブロードファズ / ロバストネスのバリアント向けの独立した（mise / クリーンHEAD）サブプロセス型オラクル;diffスコープのアドバイザリーCIジョブ（ADR-71のインクリメンタルなくさび、Rigor自身のリポジトリで正当化される）。
...
### SKILL駆動のオンボーディングUX（ADR-73、2026-06-20着地 — 先送りされたフォローアップ）

オンボーディングUXのアップグレードはmasterに出荷された（未リリース）: `rigor skill describe` + `rigor-next-steps`エントリポイント + 13スキルのカタログ + vercel-labs/skills配布、加えて現地試験駆動の6つのUX修正（`CHANGELOG.md` § `[Unreleased]`;再開用詳細と現地試験ノートは`docs/CURRENT_WORK.md`）。2つのフォローアップは先送りのまま残った（[ADR-73](../adr/73-skill-driven-user-experience/) §「Field-trial follow-ups」、未決定事項）:

- **`rigor skill describe --deep` — checkを意識したヘッドライン**。*ヘッドライン*の推奨を`rigor check`が見つけるものに反映させる（エラー → `baseline-reduce`、モンキーパッチのクラスタ → `monkeypatch-resolve`、`RBS 0` / 設定エラー → `doctor`）、WD2の純粋な存在チェックのみの既定挙動はオプトインフラグの後ろに保つ。`CheckCommand#build_check_runner`から抽出した共有の「checkを実行 → `Analysis::Result`」ヘルパーが必要。**限界価値は控えめ** —— 着地済みのエージェントプロンプトルーティングは、エージェントがどのみち実行する`check`の発見事項からすでに洗練させている —— ので需要ゲート付き。
- **カバレッジ扱いやすさラベル**。`coverage --protection`の「ここに型を追加せよ」の穴をジェネリック型パラメータ / 外部gem / フレームワークDSLで分類し、ユーザーが手書きRBSでは閉じられないものを追わずに済むようにする。**`Dynamic`の来歴トラッキング**が必要（`Inference::ProtectionScanner`はレシーバーが`Dynamic`であることは知っているが、*なぜ*かは知らない）;おそらく独自のADRになる（`Dynamic[T]`キャリアに触れる）。需要ゲート付き。
