...
- `FloatRange`は素の`Float`メンバーおよびそれを含む`FloatRange`によって吸収されます。このルールとそのInteger除外は[imported-built-in-types.md](../imported-built-in-types/)にあります（`Float`の比較絞り込みはtruthyエッジのみを広げるため、falseyエッジは`Float`を保持し、さもなくば合流が両方の解釈を運んでしまいます; `Integer`の両エッジは絞り込まれるため、`IntegerRange`は両メンバーを保持します）。
...
要素ごとの条項自体は独自の吸収を付与しません。このリストを1レベル下で再度問い合わせるため、任意のネストの深さにおいて、直接のユニオンメンバーが折りたたまれるのとまったく同じ要素に対して発火します。`[Float, String] | [Float[0.0..], String]`は`[Float, String]`に折りたたまれ、`[[Float, String], Integer] | [[Float[0.0..], String], Integer]`も同様です。`[1, String] | [Integer, String]`は両方のアームを保持します。値固定されたメンバーはその公称基底によって吸収されず、要素ごとの条項は直接のルールを追い越せないためです; `[Integer, String] | [Integer[0..5], String]`および`[String, Integer] | [lowercase-string, Integer]`も同じ理由で両アームを保持し、1つの骨格の2つの解釈ではなく異なるキャリアである`Tuple`と`Nominal[Array]`のメンバーも同様です。
