...
- メソッド名から示唆されない戻り値、または名前的クラスが語る以上のことを語る型 ── `Symbol`ではなく`:asc | :desc`、`Hash`ではなくレコードシェイプ ── は、チェッカーが検証するドキュメントとしてインラインで記述する価値があります。すべてのメソッドで同じ名前的クラスを言い直すこと（`#: String`、`#: Array`）は、読者が名前と本体からすでに得ている情報に何も付け加えません。
