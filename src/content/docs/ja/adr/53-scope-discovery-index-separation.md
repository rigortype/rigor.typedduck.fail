...
トラックB完了（B1 + B2は2026-06-11、B3 + B4は2026-06-13）。**アーキタイプ: 熟議型。ステークス: 高（推論エンジン
...
時間がフラット。**トラックB完了**（B1シャドウラン等価性ハーネス → B2フローコレクターのマージ → B3完全なルールウォーク統合 → B4でADR-52 WD4の`Plugin::NodeRuleWalk`と収束）。

トラックB B1+B2（`6858872c`）: シャドウラン等価性ハーネス + 2つのflowコレクターを1つの`CheckRules::RuleWalk`に載せた。トラックB B3（`b85c51c6` IvarWrite + `4f1745aa` DeadAssignment + `963a2947`メインパス）: `RuleWalk`を一般化し、統合コンテキスト（`in_loop_or_block` + 修飾されたクラス／モジュールのプレフィックス + `inside_def`）を1つのイミュータブルなノードごとの`Context`でスレッドするようにした；コレクターは`NODE_CLASSES`、任意の`RULE_WALK_GATES`（各レガシーウォークの走査プルーニングを再現するウォーク所有のサプレッション）、および`#visit(node, context)`を宣言し、その収集／フィルタロジックはそのまま移植される。5つの組み込みのファイルごとのウォーク（2つのflow + IvarWrite + DeadAssignment + メインの`NodeWalker.each`パス — 最後のものはウォーク上に`MainPassCollector`としてホストされる）は、いまや1つの走査に乗る。各スライスは、self-checkツリー、プラグイン／examples、Mastodon`app/models`、kramdown`lib`、haml`lib`の上で`RIGOR_SHADOW_RULE_WALK=1`をアクティブかつサイレントにしてバイト単位同一（診断）でゲートした（それはメインパスの最初の実行で、いかなるドリフトよりも前に、本物のidentity-`==`の不一致を捕捉した）；`rule_walk_equivalence_spec`が5つのコレクターすべてをホストする（174例）；bench-perfはニュートラル。

B4（2026-06-13）: 組み込みの`RuleWalk`とプラグインの`NodeRuleWalk`が、いまや1つのファイルごとの走査を共有する — `RuleWalk::CollectorDriver`（コンパイル済みフックテーブル + スタンドアロンウォークが適用したのと同じContext降下）が、`NodeRuleWalk`の単一の`each_with_ancestors`型のDFSの内部で駆動され、それがライブの`ancestors`スタック（プラグインの`NodeContext`）とイミュータブルな`RuleWalk::Context`（組み込みコレクター）を一緒にスレッドする。組み込みは厳密なノードクラスディスパッチを保ち、プラグインルールは`is_a?`メモマッチングを保つ。両者は別々のバケツ（コレクターごとの`results` / プラグインごとの`Result`）に蓄積し、それぞれ独自の診断ビルダーが組み立てるので、順序とプラグインごとのエラー分離が構造上保たれる。Runner / `WorkerSession`は完成した`scope_index`からコレクターを構築し、収束したウォークを一度実行し（`CheckRules.build_node_collectors` + `node_collector_driver`）、母集団を満たした集合を`diagnose`へ渡す（`node_collectors:`）；`diagnose`は直接の呼び出し元（`node_collectors`がnil）のためにスタンドアロンの`RuleWalk.run`パスを保つ。B4はB3cと合成されたので、収束したウォークは5つの組み込みコレクターすべて — メインパスを含む — をホストする。よってファイルはいまや、すべての組み込みおよびプラグインのノードルールについて一度だけウォークされ、ADRの「ファイルごとに1ウォーク」目標を満たす（以前のB3c前の「ファイルごとに2」という枠組みはもはや当てはまらない: 分離したメインの`NodeWalker.each`パスはもう残っていない）。レガシーのコレクターごとの`#collect`オラクル、メインパスのインラインの`NodeWalker.each`オラクル、およびスタンドアロンの`RuleWalk`ウォークは保持される — 等価性スペック（収束したウォーク、メインパスを含む、を駆動するよう拡張された）と`RIGOR_SHADOW_RULE_WALK=1`の両方がそれらに依存する。ゲート: `make verify` + bench-perfがグリーン、self-check / check-plugins / Mastodon`app/models`（6プラグイン）と`app/controllers`+`app/mailers`（プラグインヘビー）の診断がバイト単位同一、シャドウハーネスは両方でダイバージェンスなし。
...
> 上記のテーマBの延期は解除されました: #1135は1つのcref/selfモデル変更を21個の`ScopeIndexer`ウォーカーに適用し（+2,817行）、これこそがこの行が待っていた需要です。ADR-116は、発見テーブルへ拡張された本ADRのシャドウハーネスの背後で、ウォーカーを1つずつ共有された宣言コンテキストウォークへと移植します。一括の汎用ビジター書き直しは試みません。ルールコレクターをインデックス化へ畳み込むことを棄却する上記の行は引き続き有効です。
