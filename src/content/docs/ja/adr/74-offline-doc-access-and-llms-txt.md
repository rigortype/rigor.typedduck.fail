...
**改訂（2026-06-21）—— `rigor-ask`をフルの質問サーフェス（ドキュメント*および*ユーザーのコード）へ拡張**。スキルは上記3つのドキュメント参照の形（診断 / 型モデルの概念 / フラグ）から、ユーザーが実際に切り出す質問の全範囲へと成長しました: Rigorが他のチェッカーとどう比較されるか（→ ハンドブックの比較付録、`handbook/10-sorbet`）、特定のフレームワークやgemに対応しているか（→ `rigor plugins`＋プラグインごとの`rigor docs rigor-<gem>`ページ）、メソッドの型付けやアノテーションの書き方（→ RBSの各章＋`rigor sig-gen`）、そして「これは何か / rubocop＋テストではなくなぜ採用するのか / 自分たちに合っているか」という採用の質問（→ `handbook/01-getting-started`）。より深い変更は*このADR自身の主張*を拡張します: 「ソースから回答し、記憶からは決して回答しない」が、バンドルされたマニュアルだけでなくユーザーの**自身のコード**も対象にするようになりました——プログラムについての質問では、スキルは読み取り専用の解析コマンド（`rigor check` / `annotate` / `type-of` / `triage` / `coverage`）を実行し、Rigorが実際に推論した内容から回答します。そしてgemはそのフルソースを出荷するため（ADR-31）、どのドキュメントページも詳細を綴っていない箇所では、スキルは推測するのではなくバンドルされたファイルを直接読んでよいのです。質問が実はタスク（「CIをセットアップする」「ベースラインを削減する」）であるときは、スキルは手短に答え、ワークフローをインラインで再実装するのではなく`rigor-next-steps`または該当する実行スキルへ引き渡します——2スキルモデルを保ったままに。依然としてカタログ限定で新しいCLIサーフェスはなく、凍結される`description`はagentskills.ioの1024文字上限（`waza check`の仕様ゲート）の内に収められます。
