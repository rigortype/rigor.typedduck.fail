...
**修正2026-09-14** WD11とWD12の下で修正し、[#998](https://github.com/rigortype/rigor/issues/998)をクローズした。WD11の2つのリーダーが共有しない構文のリストに`%a{}`の行形状の分割が加わり、プラグインは組み込みパーサが受け付ける同一行の表記の双方を読み取るようになった;メソッド型がパースできない同一行は、暗黙のうちに中途半端に適用されるのではなくWD12の下で報告される。

**修正2026-09-14** WD12の下で修正し、[#1019](https://github.com/rigortype/rigor/issues/1019)（#998の「診断なしにドロップされる行はない」基準の残余）をクローズした。WD12にさらに2つの原因が加わった: `T`がパースできない`# @rbs name: T`パラメータアノテーション（`RBS::Inline::AST::Annotations::VarType#type`が`nil`であることから読み取られる）、およびgem自身の検出器がアノテーションとして読み取ろうとして失敗した`@rbs`接頭辞のコメント段落（`# @rbs-ext …`、自前の診断なしに通常のコメントへと畳み戻される）。

**修正2026-09-08** WD13を追加し、[#824](https://github.com/rigortype/rigor/issues/824)をクローズした。WD13は、同じメンバーの`sig/`宣言とインラインアノテーションの間の優先順位を決定する ── メンバーごとに`.rbs`が勝ち、ドロップはWD12のルールのもとで報告される。
...
- **メソッド上の`%a{}`が行の形状によって逆方向に分割される**（2026-09-14追加、[#998](https://github.com/rigortype/rigor/issues/998)）。別個のメソッド型の上にある単独行の`# @rbs %a{…}`はgem限定である: 組み込みパーサは`AnnotationSyntaxError: expected a token pARROW`を報告してアノテーションを失う。同一行の`# @rbs %a{…} () -> T`および`#: %a{…} () -> T`は組み込みパーサ（およびSteepのインラインモード）が受け付けるものであり、gemはそれらを静かに失う —— 前者ではメソッド型を、後者では行全体を失う。プラグインは文法ではなくgem側を修復する: ライターが走る前に、gemのパース済みアノテーションリスト内の同一行形式をアノテーションとメソッド型へ分割し、各々をもとの行上でgem自身の`AnnotationParser`によって再読み取りさせ、残りの部分が整形式のメソッド型でないものはgemがパースしたそのままにしてWD12に報告させる。したがって、Rigorでは同一行の表記の双方が束縛され、組み込みパーサへの移行は単独行のもののみを失うことになる。[`docs/notes/20260912-inline-refinement-carrier-probe.md`](../../notes/20260912-inline-refinement-carrier-probe/)で実測。
...
同一行の`%a{}`形式（WD11、#998）は、ライター側から到達した同じ失敗です: `# @rbs %a{pure} () -> String`はアノテーションを保持してメソッド型を破棄し、`#: %a{…} () -> String`は`SyntaxErrorAssertion`となり、他の2つのリーダーにとって合法であるにもかかわらず、#997の通知によってパース不能として報告されていました。修正は有効な表記を尊重することであり、通知を黙らせることではありません: 修復は通知が計算されるリスト上でも実行されるため、有効な行は束縛されて何も言わず、メソッド型が真に失敗している行は依然として報告されます —— `#:`表記では#997の通知によって、`@rbs`表記では破棄されたテキストを名指す通知によって報告されます。

下流での報告は緩和であって修正ではない。自分自身のライターが捨てるアノテーションを受理するパーサは上流`rbs-inline`の欠陥であり、そこで直せばRigorのユーザーだけでなくすべてのrbs-inlineユーザーに対して差が閉じる。

さらに2つの原因が[#1019](https://github.com/rigortype/rigor/issues/1019)（同一行の分割の外側にあるため#998が沈黙したまま残した2つの行）をクローズしました:

- `T`がパースできない`# @rbs name: T`パラメータ（またはローカル変数）アノテーション。gemの文法はコロンと型を任意としているため、不正な形式の`T`は、例外をraiseするのではなく`VarType#name`を設定したまま`VarType#type`を`nil`にします —— `# @rbs n: Integer[1..10]`（`Integer[1..10]`は有効なRigorリファインメントですが、RBSの型位置には置けません —— [マニュアル16](../../manual/16-rbs-extended-annotations/)）はパラメータを`untyped`として型付けし、下流では誰もアノテーションしなかったパラメータと区別がつきません。通知はコメントテキスト上のいかなるパターンからでもなく、`VarType#type.nil?`から直接読み取られ、代わりにリファインメントを運ぶ`%a{rigor:v1:param:}`の表記を名指します。
...
`sig/`とそれ自身の`.rb`内のインラインアノテーションの**両方**で宣言されたメソッドには、決定された結果がありませんでした。両方の宣言が単一のRBS環境に入り、定義ビルド時に`RBS::DuplicatedMethodDefinitionError`が発火し、単一の`rbs.coverage.definition-build-failed`行の背後でクラス全体が`Dynamic[top]`へ縮退しました ── 実在するメソッドもタイポも同様に解決を停止しました。[#779](https://github.com/rigortype/rigor/pull/779)が評価されていた間にRigor自身のツリーで測定したところ、アノテーション付きファイル234件中17件が`sig/`と重複しており、この衝突によって44のクラスがダウンしました。これはどちらの方向であれ移行を行っているプロジェクトの日常的な状態であり、オーサリングエラーではありません。

**決定: メンバー（MEMBER）ごとに、`.rbs`宣言が勝ちます**。衝突するインラインメンバーは、環境が構築される前に合成宣言から削除されます;ファイル内の他のすべてのアノテーションは依然として束縛されます;クラスはビルドされます。ドロップされた各メンバーは、そのメンバー、勝利した`.rbs`、およびアノテーションされた`.rb`を名指して、`source-rbs-annotation-not-honoured` `:info`として一度報告されます ── これは新しいケースに適用されたWD12のルールであり、#779が提案したサイレントな削除を採用できなかった理由でもあります。

#### 仕様が指すSteepへの言及は何も解決しない

`overview.md`は、RigorがインラインアノテーションについてSteep 2.0の優先順位に従うべき（SHOULD）と述べています。ソースコードと突き合わせて読んだところ、**Steepにはここでの優先順位が存在しません**:

- rbsは`RBS::Source::Ruby`（インライン）宣言と`RBS::Source::RBS`宣言を*同一の*`ClassEntry`にマージし、その`context_decls`はフラットなタグなしリストです ── `insert_ruby_decl`は`.rbs`ファイルが作成したエントリーを再利用します（`rbs/lib/rbs/environment.rb:379-390`、`environment/class_entry.rb:15-29`）。
...
メンバーごと、および`(class, method, kind)`ごとであり、定義ビルダーが実際に衝突するものと一致します: 属性はそのリーダーとライターの名前を寄与し、エイリアスはその新しい名前を寄与し、`def self?.x`は両側を寄与します。2つの隣接する重複は意図的にカバー**されません** ── **バンドル済み**RBSに対するプロジェクトの`.rbs`はファイルレベルの隔離であり（#777）、1つのメンバーを宣言する2つの`.rbs`ファイルは依然としてクラスの定義ビルドを失敗させます。どちらの側も他方よりレビューされているわけではないからです。

レポートは、環境構築中に記録されるのではなく、ローダーの入力（合成ソースとプロジェクトのシグネチャファイル）から導出されるため、ビルドを実行しないキャッシュHITでも同一のセットが報告されます。そのストリーム上の他のすべてのエントリーと同様に、アノテーションされた`.rb`の1行目に位置づけられます: メンバー自身の位置は合成RBSバッファ内に存在し、これは誰も持っていないドキュメントを記述しており、さらにADR-54環境キャッシュは位置をいずれにせよドロップするからです。

再現での測定: 修正前は`RBS::DuplicatedMethodDefinitionError ::Demo#shared has duplicated definitions`となり、`Demo`上のメソッドは解決されませんでした;修正後は`Demo#shared`が`(::String) -> ::Integer`（`.rbs`の型）となり、インラインのみの`Demo#only_inline`は依然として束縛され、`sig/`の`only_sig`は無傷であり、`definition_build_failures`は空で、1つの`:info`が両方のファイル名を名指します。
