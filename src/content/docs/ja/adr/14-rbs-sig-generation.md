...
2. 新しい型が`Inference::Acceptance.accepts(existing, new, mode: :strict)`の下で既存の宣言の部分型であり、AND
3. 新しい型がRBSの*リテラル*型へ消去されない。本体がリテラルとして証明する宣言は、その本体に対する作者の抽象化であり、絞り込みを待つ型ではない（#837; §「推論対RBSの矛盾ルール」の日付付き段落）。第3項はキャリアではなく消去についてである: RBSがリテラルとして表記できない`Type::Constant`はそのクラス名へ消去されるため、`3.14`の本体は依然として`Float`を提案する。
...
**2026-09-12 — 宣言された`untyped`戻り値は、逆方向において本ルールの対象外となります: 宣言が全くないものとして扱われ、推論された戻り値が提案されます（MUST）**。 `void`および上記のRBSリテラルのケースは、いずれも本体が証明するものよりも*多く*を述べる宣言 —— 本体が導出できない作者の抽象化 —— に関するものです。宣言された`untyped`（RBSの`Bases::Any`）はその逆であり、*何も*述べていません。`tighter?`の受理チェックは、「何もないこと」と「推論された形式が絞り込む実際のもの」とを区別できません —— `untyped`はすべてを受理し、すべてによって受理されるため、2番目のケースを捕捉するために存在するチェックの後方半分が1番目のケースも捕捉してしまい、`compare_against_declared`は推論が実際に証明した戻り値を出力することなく、そのメソッドを`equivalent`に分類してしまいました。辞退（decline）よりも悪いことに、`equivalent`は`Classification::EMITTABLE`に含まれず、`skipped`行でもないため、その候補は理由を説明するものが何もないまま`--print`とJSONペイロードの双方から消滅していました（[#995](https://github.com/rigortype/rigor/issues/995)）。ADR-93のインライン`# @rbs param: Type`アノテーション（戻り値節なし）は、自身がドキュメント化するすべてのメソッドに対してまさにこの宣言形状を合成するため、サイレントな脱落は、ADR-14が支援するために存在する、パラメータがアノテーションされ戻り値が裸のメソッドに着弾していました。`declared_untyped?`は`tighter?`が走る前に`compare_against_declared`をショートサーキットし、宣言を持たないメソッドが得るのと同じ`tighter-return`候補を構築します —— `declared_return_rbs`として`"untyped"`を運ぶため、`--diff`および`[tighter, was: untyped]`プリントタグは、宣言が存在していたことを依然として読者に伝えます。
