...
この反転は、まず6つの前提条件をクリアすることでゲートされ、この裁定の執筆時点ではそのどれもクローズしていなかった;**WD16が1〜4と6を解決する**（2026-08-22）:
...
### WD16 — 卒業の前提条件、その解決（オーナー裁定、2026-08-22）

WD15のリストはコードからではなく設計から書かれており、その前提のうち2つは成り立たない。この裁定は何がそれらを置き換えるかを記録する。前提条件1〜4と6はここで解決される;5はリリースそのものの時点で書かれる。

**1 — プールされたバックエンド: 逐次への劣化こそが答えだ**。スレッドバックエンドは存在せず、今後も存在しない——`pool_backend`は`:fork`・`:ractor`・`:sequential`を選択し、スレッドプールはGVLの下で並列性を何も買わず、その一方でRBSのC拡張の状態をワーカー間で共有してしまう。Ractorバックエンドはrbs 4.xの下では1ファイルすら解析できない: `RBS::Namespace.[]`はモジュールのivarに保持されたプロセス全体の可変フライウェイトを通じてすべての名前空間をインターンし、非メインのRactorはそれを読むことを許されず、事前ウォームはそこに届かない。キャッシュされた環境をパースし戻す間にトライが参照されるからだ（[#414](https://github.com/rigortype/rigor/issues/414)）。したがって、書かれたとおりの前提条件はRigorが所有するいかなる作業でもクリアできない。それはコードがすでに持つ振る舞いで置き換えられる: `fork`がなければ、収集する実行は逐次に劣化し、forkパスが使うのと同じ`pool_degraded_diagnostic`チャネルを通じて**そう言う**。この劣化は単に安全なのではなく健全だ——逐次パスも収集はするので、エフェクトグラフはどちらにせよ完全であり、Windowsが払うのは実時間であって、決して正しさではない。[#410](https://github.com/rigortype/rigor/issues/410)（非forkバックエンドを通じてサイドテーブルを運ぶ）はこれとともにクローズする: その表明された前提「新しい部品はメッセージチャネルだけ」は、対象とするバックエンドがファイルを解析できない間は真ではない。

**2 — 語彙: 語彙1はこのまま出荷する**。2026-08-22にSteinsと突き合わせて読むと、[#378](https://github.com/rigortype/rigor/issues/378)の3項目は、提起されたときとは違うかたちで解決する。`mutate.self` / `mutate.instance` / `mutate.static`の綴りはすでに一致している（WD14）。`io.db.read` / `io.db.write` / `io.db.transaction`の葉はRigorだけのものだ——Steinsの組み込み集合は`io.db`で止まる——これはレジストリ表がすでに述べているとおりで、葉の追加は認識される境界が何を認めるかを決して変えられない。本当の乖離はアプリケーション意味のルートであり、それは字句的ではなくアーキテクチャ的だ: Steinsはエコシステムラベル（`io.redis`・`email.send`）は**組み込みではなく**、プラグイン自身のマニフェストを通じてレジストリに到達するという立場を取る一方、Rigorは`telemetry`・`email.send`・`job.enqueue`・`cache.read`・`cache.write`を共有ファイルの行として出荷する。Rigorはそれらを保持する: それらは`tolerated:`が掴むものであり、ポリシーが実際に名指すものであり、プロジェクトがそれを書けるようになる前にプラグインを必要とすべきではない。変わるのは主張だ——仕様のレジストリ表はその層を「共有」と呼び、その綴りの一致をMUSTとしていたが、Steinsは今日それを支持しないので、その行はRigor所有となり上流に提案される。したがって整合はどちらの向きにもブロッカーではない: Steinsが後から主張する綴りは`retired:`と語彙のバンプを通じて着地する——まさにこのために存在するメカニズムだ——そして#378はゲートとしてではなく上流との対話として開いたままになる。

**3 — WD13の予算: CIの`effect-budget`ジョブが裁定者だ**。予算の失敗を報告した2026-08-19のローカル計測は、その2日前のより完全で静かな計測に照らして撤回された;両者を分けたのはホストの汚染であって、方法ではない。開発者ホストで決められるgo/no-goはその失敗を再現し続けるので、助言的な`effect-budget`ジョブのバンドがmastodonの半分を確定し、それ以外の何もしない。gitlabの半分——1秒の境界に対する1.34秒の`Propagator.propagate`——は撤回されておらず生きた数字だが、それはコーパス最大の規模で実行ごとに1回払われるクロージャコストであって、プロジェクトごとの税ではない: それはv0.3.5〜v0.3.9の窓にわたる[#424](https://github.com/rigortype/rigor/issues/424)の最適化ターゲットになり、リリースノートがそれを開示する限り、反転をゲート**しない**。

**4 — `effects.lsp`: エディタモードはエフェクトフリーのままで、設定がそう言う**。WD13の「エディタモードはv1ではエフェクトを走らせない」は、v1の但し書きではなく恒常的な答えになり、`false`を既定とする実在のキーとして綴られる。存在していてオフのキーこそが、反転の後にその席が黙って空になるのを防ぐものだ;メソッドのラベルを報告するホバーは、それ自身の消費者を持つv0.4.xのスライスであって、前提条件ではない。今日`lib/rigor/language_server/`の中でエフェクトを読むものは何もないので、作業はキーと、そのドキュメントと、既定オンのプロジェクトがキーストロークごとに収集コストを払わないよう保つガードだ。

**6 — 汚染のみの行**は[#411](https://github.com/rigortype/rigor/issues/411)で決定され、[#415](https://github.com/rigortype/rigor/pull/415)で出荷された: 汚染だけを運ぶ行は既定で省略され、`--full`がそれを保持する。

### WD17 — 宣言レーンは決して判定されない;それを強制するのはスナップショットゲートだ（オーナー裁定、2026-08-24）

出荷済みのフィーチャーを2つのRailsアプリケーションにわたって初期採用者として歩いた調査（[`20260823-effect-user-stories-corpus.md`](../../notes/20260823-effect-user-stories-corpus/)）は、チームがエフェクトシステムに持ち込む10の問いを立て、その旗艦の問いが答えられないことを見出した: *「このレイヤーはデータベースに触れてはならない」*は診断になりえない。見落としによってではない——このADRがすでに下した、それぞれ単独では正しい2つの決定の合成によってだ。

`PluginFacts`はすべてのプラグイン行を宣言レーンに通し、`Attribution`はプロジェクト自身の表について同じことをする。**`proven`は「アナライザーがコードを読んだ」を意味し**、どちらも読まれたコードではないからだ（WD6）。`EnvelopeCheck`は証明レーンだけを読む。主張が発見を決して製造してはならないからだ（弁別的基準）。合成すると: **いかなるエンベロープも、いかなる`%a{pure}`も、いかなる`effect.envelope-exceeded`も、データベースアクセス、キャッシュ書き込み、メール送信、ジョブのエンキューに対して発火することは決してありえない**——そしてRailsアプリケーションでは、それこそがポリシーが名指すであろうすべてのラベルだ。

計測は、`rigor-survey/redmine` @ `a12198ea0`と`rigor-survey/mastodon` @ `163f96cee`の11,702のレポート行にわたる: `io.db.read`と`io.db.write`が証明レーンに現れる回数は**ゼロ**だ（宣言: 709 / 644と1,975 / 1,283）。mastodonの`app/serializers/**`にかぶせた`effect: []`エンベロープは56件の発見を落とす——45件の`mutate.self`、7件の`global.read`、3件の`nondet.time`、1件の`mutate.static`、そして`io.db.*`はなし——その一方で`REST::V1::InstanceSerializer#invites_enabled`のメッセージは、`UserRole.create!`を*通り抜けて*歩き、その先のivar代入を報告するチェーンを印字する。redmineの`app/helpers/**`にかぶせた同じスタンザは、同じ不在を伴う343件の発見を落とす。

**裁定: この二分は立ち続け、プラグイン由来のラベルの強制サーフェスは`rigor effects check`だ**。スナップショットは宣言レーンを記録し、それ自身のマーカーの下で差分を取る——redmineの呼び出しパスに`Journal.create!`を追加すると`IssuesController#index ≤+ io.db.write`が印字される——ので、「このレイヤーはデータベースへの書き込みを*始めて*はならない」は今日すでに強制可能であり、そしてそこでだけ強制可能だ。間違っていたのはモデルでは決してなかった;間違っていたのは[`19-effect-labels.md`](../../manual/19-effect-labels/)であり、それは宣言された境界を判定するサーフェスとして、スナップショットを観測するサーフェスとして提示している。それは自分自身のコードについての境界には正しい向きで、Railsチームが最も気にかけるラベルには間違った向きだ。

2つの代替案が比較検討され、退けられた:

- **オプトインの`:info`の`effect.declared-exceeded`**。既定でオフでビルドを失敗させられないルールは、2026-08-22/23のバッチの4つの欠陥と同じ形——誰も読まない宣言——だ。それはまたFP予算をプラグイン行の正確さに費やすことになり、#456は、行が3つの独立したメカニズムを通じてマッチに失敗しうる一方、過剰マッチはまだどのメカニズムを通じても起きていないことを示したばかりだ。
- **ファーストパーティプラグインのフレームワーク由来の行を証明レーンに昇格させる**。これは`proven`の拡張ではなく再定義であり、`rigor check`の赤を、Rigorが読んだコードではなくプラグイン著述の正しさに賭けることになる。再考するのは、プラグインの行がいつか*検証済み*——アナライザーが実際に読んだシグネチャまたはソースから導出されたもの——になったときだけだ。それは別の寄与種別であって、この寄与種別への信頼フラグではない。

**裁定が禁じないもの**。2つの隣接するナローイングがその横に着地したが、例外ではない: カタログの姿勢は、**構文**が定数として名指すレシーバーについて答えてよい（[#463](https://github.com/rigortype/rigor/issues/463)——カタログは読まれ監査された知識であり、唯一の問いは誰がレシーバーを特定したかだった）。そしてバンドル済みプラグインは、自身のgemが導入する祖先関係を宣言してよい（[#465](https://github.com/rigortype/rigor/issues/465)——祖先関係の主張はラベルを加えず、既存の行を*到達可能*にするだけで、その行のレーンは変わらない）。

**WD15の前提条件5について**、リリースノートの移行注記は1文を得る: 既定オンは収集とエンベロープチェックをオンにし、エンベロープチェックはプラグイン由来のラベルでは失敗しえず、それらが強制されるのは`rigor effects check`だ。それには依然としてレーンの但し書きも「まずキャッシュをクリアせよ」も要らない。
