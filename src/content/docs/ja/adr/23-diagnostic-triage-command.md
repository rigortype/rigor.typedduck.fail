...
sourceSha: "fe76d64486c16158bfdfb14dd3be179ba4a38e27986ca7e14a050b0ce2c4a67d"
sourceCommit: "840db09d878cd50bf66f76b9b66fe7a16eeb15b4"
sourceDate: "2026-06-21T19:31:02+09:00"
sourceLanguage: "en"
...
WD6（ボリュームビューからinfoをデフォルトで除外）はv0.2.3で実装済み**。
...
### WD6 — トリアージのボリュームビューはアクション可能な診断のみをルーティングする

分布、セレクタ、ホットスポットの各セクション — **ボリュームビュー** — はデフォルトで`:error` + `:warning`診断のみをカウントする。`:info`は除外され、`--include-info`で復帰できる。サマリー行は引き続き完全な`:info`カウントを報告し、テキストレポートは隠された数を明記する。

この決定は2026-06-21のオンボーディング実地調査（実際のRailsアプリ、`conference-app`）に基づく。その`rigor triage`の結果は**267件の合計 — 9件error / 1件warning / 257件info**で、infoのほぼすべてが*プラグイン認識トレース*だった:
...
これらは「Rigorがこの呼び出しを解決した」というポジティブな記録であり、問題ではない。意図的に`:info`でemitされており、`rigor explain` / ホバー / クロスプラグインファクトで価値がある（プラグインはこれらを自身の付加価値として文書化している）。しかしトリアージに集約されると:

- **シグナルを埋もれさせる**。10件のアクション可能な診断（本当の作業）が257件のトレース行の下に沈む;「主な内訳」がプラグインノイズとして読める。
- **ホットスポットランキングを反転させる**。コントローラーが*最も多くのモデル / ルート / i18n呼び出しを解決した*ために — つまり最も*正常に動作している*Railsコードだから — トップホットスポットとしてランクインする。ランキングが修正すべき箇所とは逆を指す。
- **エージェントの注意を浪費する**。実地調査ではエージェントが「`model-call: 68`はなぜか？」という問いに3回の探索ラウンドを費やし、「何もない — RigorがモデルをResolveしているだけ」という結論に達した。

`:info`は`evidence_tier`を持たない（[ADR-65 WD2](../65-diagnostic-evidence-tier-and-doc-url/)）:信頼度を持つ真陽性の主張ではないため、注意をルーティングするビューには属さない。これは再評価トリガー#1（「ヒューリスティックヒントが有用より誤解招くと判明 → 削減」）をヒントからボリュームサーフェス全体に一般化したもの。

スコープ上の決定:

- **Hintsは完全なストリームを参照する**（アクション可能なサブセットではなく）。`gem-without-rbs`認識器（H3）は意図的にinfo深刻度の`rbs.coverage.missing-gem`*通知*（カテゴリー的にプロジェクトレベルの通知であり、呼び出しごとのトレースではない）を読む — これはデフォルトで生き残らなければならない本当に有用なオンボーディングヒントである。他の認識器はerror/warningルールをキーにする。
- **H5（システミッククラスタ）とH6（本物のバグ）**はカウントベースの深刻度非依存の認識器であり、これらのみが認識トレースを「システミッククラスタ、ひとつの修正で多くをクリア」や「本物のバグの可能性」として読む可能性がある — 正常に動作するコードを脅かす、根本的な偽陽性の罪。`--include-info`が設定されない限り`:info`から保護される。
- **サマリーは完全なカウントを維持する**。*ビュー*からinfoを隠すことは、その*存在*を隠してはならない;オンボーディングユーザーは引き続き「257 info」と`--include-info`でそれらが表示されることを知ることができる。

これは技術的にデフォルトの`rigor triage`テキストおよび`--format json`出力への挙動変更（新しい`include_info` JSONフィールド;ボリュームビューはデフォルトでは`summary.total`に合計されなくなる）であり、v0.2.3のBC（後方互換性）を伴う変更として出荷される。
...
`include_info`が`false`（デフォルト、WD6）の場合、`distribution` / `selectors` / `hotspots`のカウントは`:info`を除外するため`summary.total`には合計されない;`summary.info`は引き続き完全なカウントを報告する。
...
- プラグイン提供の認識器拡張ポイント（WD2）と構造化された`Diagnostic`フィールドのロバストネス修正（WD3 / スライス4）は見送りであり、却下ではない。
