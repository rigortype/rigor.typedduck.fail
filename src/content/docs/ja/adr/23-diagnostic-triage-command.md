...
- **Hintsは完全なストリームを参照する**（アクション可能なサブセットではなく）。`gem-without-rbs`認識器（H3）は意図的にinfo深刻度の`rbs.coverage.missing-gem`*通知*（カテゴリ的にプロジェクトレベルの通知であり、呼び出しごとのトレースではない）を読む — これはデフォルトで生き残らなければならない本当に有用なオンボーディングヒントである。他の認識器はerror/warningルールをキーにする。
