...
sourceSha: "8365ff95d4598b486b6a3ba072bfe716b7573f1b03854be22c55d01f05d4d50c"
sourceCommit: "db7b23d42e9b47560438b67dfe16d53e03f70575"
sourceDate: "2026-09-10T10:21:55+09:00"
translationStatus: "translated"
sidebar:
  order: 4048
---

ステータス: **Accepted — `Data.define`スライス1〜4実装済み（v0.1.17）に加えベアローカルのブロック形式パリティ;`Struct`フォローアップのスライス1〜5実装済み（フレッシュチェーン + 畳み込み安全なバインドローカル + セッター再型付け）**。2つの新しい型キャリア（carrier）— **メンバークラスキャリア**（`Type::DataClass`）と**メンバーインスタンスキャリア**（`Type::DataInstance`）— を導入し、`Data.define`で定義した値オブジェクト（value object）のメンバー読み取りが精密な型に畳み込まれる（`Point = Data.define(:x, :y); Point.new(1, 2).x` → `Constant[1]`）。3種の定義形式すべて（定数代入形式・`class X < Data.define(...)`サブクラス形式・ベアローカル形式）、位置引数とキーワード引数の両構築、および`[]` / `to_h` / `deconstruct` / `deconstruct_keys` / `members` / `with`の各プロジェクションに対応して出荷済み。`DataFolding`ディスパッチ層は、名前付き形式に対してスコープインデクサが書き込むクロスファイルの`Scope#data_member_layouts`サイドテーブルを参照する。**スライス4（ブロックボディ堅牢化）が着地:**ボディがメンバーの合成済みリーダー（`def x`）を再定義している名前付きクラスは、そのメンバーの*読み取り*畳み込みを行わなくなった（再定義済みメソッドが呼ばれるため）— 一方、値アクセサ（`[:x]` / `to_h`）はリーダーをバイパスするので引き続き畳み込み対象 — プロジェクトdef-nodeテーブルの実`def`ノードを条件とする。**ベアローカルのブロック形式パリティが着地:**解決可能なクラス名を持たないブロック形式（`c = Data.define(:x) do … end`）も畳み込まれるようになった — ブロックのASTを直接スキャンしてメンバーリーダーの`def`を探すので、ヘルパーのみのブロックは畳み込まれ、リーダーを再定義する（または`&proc`の）ブロックはbailする。畳み込みは**精度追加のみ** — 新しい診断ファミリーなし、偽陽性のサーフェスなし（プロジェクトの偽陽性の規律に沿う）。フェーズ5カバレッジ監査[`docs/notes/20260523-struct-encoding-coverage.md`](../../notes/20260523-struct-encoding-coverage/)に基づき、同監査がこれをADR相当として先送りし`Data.define`をより良い最初のターゲットと指名した。

**`Struct`フォローアップ — スライス1 + 2着地（健全な*トランジェント*形式）**。可変な姉妹キャリア`Type::StructClass` / `Type::StructInstance`が出荷され、`Struct.new(...)`は`StructClass`のメンバーレイアウトに畳み込まれ、`.new` / `[]`が`StructInstance`を実体化し、**フレッシュに実体化された**インスタンスからのメンバー読み取りが畳み込まれる（`Struct.new(:x, :y).new(1, 2).x` → `Constant[1]`。匿名・定数・サブクラス・ローカルクラスの各形式、位置引数 + `keyword_init:`に対応）。変異健全性のストーリーは、書き込みサイトの無効化ではなく**フレッシュレシーバゲート**によって解決される: *格納された*バインディングからのメンバー読み取りは`Dynamic[top]`に劣化する（この式がたった今実体化したレシーバーは、それ以降に変異され得ないため無効化は不要;格納されたバインディングは変異され得るため畳み込まない）。「フレッシュに実体化された」が意味するのは`.new` / `[]` / `.with`、または本体がそれらのいずれかを返す解決可能なファクトリーメソッド（[#599](https://github.com/rigortype/rigor/issues/599)）であり、チェーンされた呼び出し一般では**ない**——[#595](https://github.com/rigortype/rigor/issues/595)が2026-09-02にそれを訂正した。selfを返す流暢なビルダーが、変異の後に構築時の値を提供しているのが見つかったためだ。これは書き込みサイトに一切触れず — すべてのエスケープ経路を列挙するよりはるかに偽陽性リスクが低く — まさに先送りされたスライス3が緩めるゲートそのものだ。完全な記録は後述の§「Structのフォローアップ」を参照。
...
**需要ゲート付きの2つの残余は一緒に着地した**。*ベアローカルのブロック形式パリティ*（`c = Data.define(:x) do … end`）: ブロックは読み取り時のリーダー再定義ガードのための解決可能なクラス名を持たないが、ブロックのASTは`Data.define`の時点で手元にあるので、それを直接スキャンする — ブロックがどのメンバーリーダーも再定義しないときキャリアは畳み込まれ、解決不能なケースが以前そうしたように保守的にbailする（`&proc`のブロック、リーダーを再定義する`def <member>`）。そして`Struct`の**スライス4**（セッターを通じた変異済みメンバーの精密な再型付け — `s.x = 5; s.x` → 代入された型、姉妹メンバーは精度を保つ）: 畳み込み安全なローカルを直線的なメンバーセッターを許すよう拡張したもので、そのバインディングをセッターのライトバック（`StructFolding.apply_setter_writeback`、`eval_call`のポスト呼び出しスコープにて）が最新に保つので、後の読み取りが畳み込まれる。ループ / ブロック / ラムダ内のセッター、エイリアス、あるいはエスケープはローカルを畳み込みなしのまま保つ（FPセーフ）。[`docs/notes/20260615-struct-folding-slice3-design.md`](../../notes/20260615-struct-folding-slice3-design/)で設計されている。
...
1. **ブロックボディが存在する**（`Data.define(:x) do … end` / `class Point < Data.define(:x); def m; end; end`）— ブロックは追加メソッドを定義したり、リーダーを再定義したり、定数を追加したりする可能性がある。**出荷済み（スライス4）:** **名前付き形式**（`class Point < Data.define(...)`と`Const = Data.define(...) do … end`）はメンバーレイアウトを引き続き畳み込む — ブロックは通常ヘルパーメソッドを追加するのみで、`.x` / `to_h` / `[]`は精度を保つ。拒否される読み取りは、ボディが実`def x`でリーダーを*再定義*しているメンバーのみ: その読み取りはメンバーを返さず再定義済みメソッドを実行するため、畳み込みは不健全になる。両名前付き形式はオーバーライドをクラス名のもとに`def`ノードとして登録するため、プロジェクトdef-nodeテーブル（`Scope#user_def_for`）のエントリーが識別子となる — 合成済みリーダーにはdefノードがない。値アクセサ`[]` / `to_h` / `deconstruct`はリーダーをバイパスするためfoldable（畳み込み可能）のままで、ゲートはベアメンバー読み取りのみにかかる。**ベアローカル**ブロック形式（`c = Data.define(:x) do … end`）は解決可能なクラス名を持たないため、そのブロックのdefをdef-nodeテーブルで参照できない — しかしブロックのASTは`Data.define`の時点で手元にあるので、ガードはそれに対して直接適用される（`fold_define_block` / `MemberShapeProjection.block_redefines_member_reader?`）: ボディがどのメンバーリーダーも再定義しないときキャリア全体が畳み込まれ、リーダーを再定義する`def <member>`または`&proc`のブロック（スキャン可能なボディがない）に対しては保守的にbailする。Rigor自身の`lib`（ブロック+サブクラス形式が密集）に対して検証済み: 自己チェックのリグレッションなし。
...
2つのファクトが重なっている: `infer_user_method_return`は本体を固定された`self_type`のキャリアの下で型付けするので、本体内のセッターが返される`self`を汚染することは決してなく、そしてゲートは呼び出し先が何を返すかを尋ねたことがなかった。ホワイトリストはその両方に一度に答え——その外側の呼び出し先は、内部で何をしていようと拒否される——そしてこれはADR-48のスライス5の`:self`の付与が使うのと**同じ**述語である（[`StructMaterialization.materialization_call?`](https://github.com/rigortype/rigor/blob/master/lib/rigor/inference/method_dispatcher/struct_materialization.rb)。自身のファイルを持ち、`StructFolding`がそれを`extend`する。`module_function`のコピーは`extend`を通じてprivateで届くので、この述語は自身のモジュール名の下でしか呼べない）。1つのインデックス化の注意点が、両方の消費者について`.with`のガードを等しく境界づける: `discovered_includes`は**ブロック形式**のファクトリー本体についてはpopulateされないので、`Struct.new(...) do include M end`の*内側*でincludeされ、自前の`with`を供給するモジュールは走査から不可視である——発見側の修正と実際の形は[issue #599](https://github.com/rigortype/rigor/issues/599)に記録されている。述語を共有することが要点だ: 2つのゲートの答えが分岐していた間は、正しくホワイトリストされた`.with`の畳み込みが、もう一方のゲートが誤ってフレッシュと呼んだレシーバーから発火しえた（`x.dup_self.with(indent: 9).shout`）ので、合成された形は片方だけの修正を生き延びてしまった。

代償は、本当にフレッシュなヘルパーの戻り値だ: `def make = Line.new("x")`としたときの`make.text`はもう畳み込まれない。ホワイトリストは`make`を透かして見られないからだ。計測したところ、それは名前を挙げたコーパスの対象のうち構造体を最も多用する実ファイル——hamlとhamlitのパーサ、faradayの`Options`/`Request`、mailの`received_parser`——では観測可能なコストをまったく生まない。それらでは前後の実行が同一の診断の集合と同一の`type-scan`のカバレッジを示す（未認識570、`CallNode` 461/1966）。この損失が効くには、ヘルパーの戻り値がすでに`StructInstance`へ推論されている必要があり、これらのサイトではそうなっていない。より豊かな修正——呼び出し先の推論された戻り値を参照し、それが`self`のエイリアスでないと証明できるときに受け入れる——は[#599](https://github.com/rigortype/rigor/issues/599)として追跡されている。このリポジトリ自身のリグレッションのガードのうち3つはファクトリー経路の上に書かれており、識別力を保つために直接の実体化へ移す必要があった。それが、これを優先する具体的な根拠である。

**スライス計画（Struct）:**スライス1 = `StructClass`キャリア + `Struct.new`認識;スライス2 = `StructInstance`キャリア + フレッシュチェーンのメンバー畳み込み + 定数/サブクラス形式のためのサイドテーブル（`Scope#struct_member_layout`）。両者は一緒に着地した（サイドテーブルは一般的な定数形式に必要で、マテリアライゼーションの基盤を完成させる）。**スライス3着地（2026-06-15）:**フレッシュレシーバゲートは、いまや*変異のないバインドローカル*からのメンバー読み取りも畳み込む。これは保守的な畳み込み安全スキャン（[`Inference::StructFoldSafety`](https://github.com/rigortype/rigor/blob/master/lib/rigor/inference/struct_fold_safety.rb) — ローカルは、そのすべての使用がメンバー読み取り / 既知の純粋プロジェクションであるときに限り畳み込まれる;セッター・インデックス書き込み・エイリアス・エスケープ・未知のメソッド呼び出しのいずれもがそれを失格にする）によって証明される。健全性はカウント恒等式に依拠する: ローカルが畳み込み安全であるのは、すべての`LocalVariableReadNode(n)`が純粋読み取り呼び出しのレシーバーである（`total_reads == pure_receiver_reads`）とき、かつそのときに限る。これはエスケープ経路を列挙せずにすべての変異 / エスケープ / エイリアスを捕捉する（許可リストにより、取りこぼしは過度に保守的になるだけで決して不健全にはならない）。この集合はローカル変数スコープごとに一度計算され（`def` / `class` / `module`の境界を尊重する;ブロックはローカルを共有する）、トップレベル（`ScopeIndexer`）とメソッド本体（`build_method_entry_scope` / `build_user_method_body_scope`）のエントリーポイントでスコープ（`Scope#struct_fold_safe?`）にインストールされる — セルフチェックで性能中立と計測済み。**スライス4着地:**畳み込み安全スキャンは、変異が**直線的なメンバーセッター**（`s.x = v`）のみであるローカルも許容するよう緩められる: セッターの代入された型が`eval_call`のポスト呼び出しスコープでそのローカルの`StructInstance`バインディングへ書き戻され（`StructFolding.apply_setter_writeback`、`MutationWidening.widen_after_call`を鏡映）、後の`s.x`が代入値へ畳み込まれ、姉妹の`s.y`は精度を保つ。この書き戻しが健全なのは、スキャンが依然としてエイリアス、エスケープ、`[]=`、あるいはループ / ブロック / ラムダ内のセッター（単一の静的パスでは反復ごとの効果をモデル化できない）を失格にするからだ — 直線的な条件分岐は問題なく、その分岐スコープはjoinされる。[`docs/notes/20260615-struct-folding-slice3-design.md`](../../notes/20260615-struct-folding-slice3-design/)で設計されている。

**スライス5着地（[#525](https://github.com/rigortype/rigor/issues/525)）——本体内のメンバー読み取り**。スライス1〜4はいずれも畳み込み可能性をレシーバーの**ノード**から判断するので、レシーバーがまったく書かれていないメンバー読み取り——`Struct.new(:text) do def shout; text.upcase; end end`のごく普通のメソッド本体——には判断材料が何もなく、`Dynamic[top]`へ劣化し、そのメソッドの戻り値まるごとを道連れにしていた。呼び出し元は答えを知っているので、それを下へ渡す: `try_user_method_inference`がレシーバーの**式**を判定し——認識された**実体化**（`Point.new(…)`・`Point[…]`、自前の`with`を定義していない構造体の`.with(…)`のコピー）、畳み込み安全なローカル、あるいは現在の本体の付与を継承する`self`——そして`build_user_method_body_scope`がその結果を、本体スコープの畳み込み安全な集合の中に`:self`のセンチネルとして記録する。`self`はキーワードでありRubyのどのローカルもその名前を持てないので衝突がなく、`Scope`に新しいフィールドは要らない。`foldable_receiver?`には、`nil` / `self`のレシーバーに対する対応する腕が加わり、その付与が発行された当のキャリアに結び付けられる。

この付与が使う実体化の判定は、スライス2の`fresh_receiver?`が使うものと**同じ**である——実装は1つ、`StructMaterialization.materialization_call?`だ。どちらも素のチェーンされた呼び出しをフレッシュとは読まない: メソッドが自分自身のレシーバーを返しうる以上、「チェーンされている」は「フレッシュである」を意味しなくなり、そしてselfを返す流暢なビルダーはごく普通のRubyである。`def with_text(v) = (self.text = v; self)`に対する`Line.new("a").with_text("z").shout`は、さもなければ`shout`に2文ぶん古いメンバーのマップを付与し、実行時の値が`"Z"`であるところで`"A"`を畳み込んでしまう。スライス2は緩いほうの読みで出荷され、これと並べて訂正された——上のフレッシュレシーバゲートの段落と[#595](https://github.com/rigortype/rigor/issues/595)を参照。

呼び出し元の証拠に加えて2つの条件がある: キャリアが`StructInstance`であること、そして本体における`self`のすべての使用が純粋な読み取りであること（`StructFoldSafety.self_fold_safe_body?`）。selfに対するメンバーのセッター、エスケープする素の`self`、そしてメンバーのリーダーでも固定された純粋な読み取りでもないself呼び出しは、いずれも拒否する。最後のものが、この付与を本体の行う呼び出しについて閉じたものにしている: 兄弟の`def reset!; self.text = ""; end`に対する`def shout; reset!; text.upcase; end`は自身のセッターを持たないので、直接のセッターだけを探すガードは、実行時の読み取りが`""`であるのに`text`を構築時の値へ畳み込んでしまう。認識されないself呼び出しは代わりにレシーバー自身のクラスに対して解決され、同じ問いを問われる。したがって単に**委譲する**だけの本体（`def outer; shout; end`）は付与を保てるが、どの深さであれライターに到達する本体はそれを失う。

その解決は境界づけられており、すべての境界は**閉じる方向**に失敗する——付与を拒否するのであって、発行することは決してない。相互呼び出しの循環は再帰せずに拒否する;何にも解決しない名前（`puts`・`raise`・RBSにしかない祖先のメソッド）は拒否する。答えは実際に検査された本体に裏打ちされていなければならないからだ;`super`は拒否する。リゾルバが自クラスのdefだけを歩くからだ;そして走査は**4ホップ**で上限が設けられているので、それより長い委譲の連鎖は、その中のすべての本体が純粋であっても拒否する。したがって委譲が付与を保つのは*上限の内側*であって無条件ではない——上限の代償は深い連鎖における精度であって、誤った畳み込みでは決してない。

**この付与はADR-84の戻り値メモのキーの一部である**。それは3つ目の、呼び出しサイトごとに変わる次元だ: 同じ`(def_node, receiver, arg_types)`が、畳み込み可能な呼び出しサイトからは畳み込まれたメンバーの型を返し、そうでないサイトからは`Dynamic[top]`を返す。キーの要素を取り除いて検証した——最初に走った呼び出しサイトが両方について答えてしまい、畳み込み可能なほうが先に来る順序では、何も現在性を証明していないローカルからの読み取りに畳み込まれた値を提供する。単なる不正確さではなく誤った型である。これは別途スレッディングされるのではなく構築された本体スコープから読まれるので、キーが結果を生んだスコープから乖離することはありえない。
