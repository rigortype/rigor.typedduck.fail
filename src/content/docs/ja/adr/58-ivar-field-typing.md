...
sourceSha: "1c2fe4f6fee44eeb2abe3f5ae93a86ff0aafe56473eeb3fe4698d63a37f4d03b"
sourceCommit: "32fcfb01032273679a99853a37f53a6e842b3330"
sourceDate: "2026-09-24T22:24:33+09:00"
...
**ステータス、2026-09-24 — 拡大（#1338）**。 `||=`のスキップは、偽値リテラルだけでなく、真値部分を持たないすべての右辺をカバーするようになりました。`@x ||= raise "unset"`は何も保存しませんが、その`union(bot, nil)`寄与は`nil`であり、クラスのすべてのインスタンスメソッドでivarを束縛していました;その結果ガードは証明可能にraiseすると読まれ、`sig-gen`は別のクラスや`attr_writer`がivarを保存するリーダーに対して`-> bot`を宣言してしまっていました。

**ステータス、2026-09-24 — `&&=`の境界を閉鎖（#1340）**。上記の`&&=`アームの境界は無害ではありませんでした: 右辺としてシードされると、`&&=`は自身のivarを束縛したため、`if (@x &&= 1)`は常に真値と畳み込まれる可能性があり、`@x &&= raise "b"`は`bot`と読まれる可能性がありました（クラス内の他の場所での直接の`@x`読み取りは書き込み前の`nil`を追加したため、両者を隠蔽していました）。`&&=`の寄与は現在、ソース順を問わずivarの別の書き込みと並んでいる場合にのみカウントされ、後続の`op=`は依然としてその上でディスパッチします; `&&=`のみが書き込むivarは未シードのまま留まり、書き込みは未束縛ターゲットである`Dynamic[top] | v`（ステートメント評価器の読み取り）として読まれます。
