...
sourceSha: "a4888d2268d6b9b27370db837642e0988b9044ec0195757a61af7d00c58e5e4a"
...
> - **セルフミューテーションのファイルごとの有効性（Product C）—— 成熟・繰り返し可能;残りのファイルのバッチ処理を続けること**。`tool/mutation/self_mutate.rb <file…>`（融合したself-check ∪ rspec）で`lib/rigor`のテストスイートの穴を塞ぐ。ツリー全体のコールドメソッド層は枯渇した（スイートはメソッドレベルで完全）;オープンな作業は`needs-verification`層（カバー済みだがアサートされていない）のファイルごとの裁定。**2026-06-21（session 1）: 7つの融合バッチを通じて約28ファイルを等価ミュータントのフロアまでクローズした**（config_audit、lockfile_resolver、class_registry、reflection、method_catalog、conflict、diagnostic、options、mutation_protection_report、prism_colorizer、return_type_heuristic、builder、walker、coverage_command、hkt_reducer、hkt_registry、kernel_dispatch、overload_selector、precision_scanner、protection_scanner、project_patched_scanner、debouncer、document_symbol_provider、signature_help_provider、mcp/server、…）。**2026-06-21（session 2）: バッチ8〜19で約88ファイルを追加で計測し、さらに4ファイルをフロアまでクローズした**（member_shape_projection 15 → 0、diagnostic_publisherは4つの穴をクローズ＋2つの等価なclampフロア、incremental_snapshot 17 → 0、node_locator 3 → 0）。コードの改善が1つ: `Diagnostic`コンストラクタがline ≥ 1、column ≥ 1を検証するようになった —— これにより`clamp(0, …)`／`clamp(nil, …)`の等価性が形式的に正しくなる（Ruby 4.0はnilのclamp境界を下限なしとして扱う）。持ち越し: `rbs_extended/conformance_checker.rb`（293 LOC、約110サバイバー、RBSフィクスチャ依存）。**60〜300 LOC層は今や実質的に計測済み**（ほとんどのファイルが100%の融合保護または等価ミュータントのフロアにある）;300 LOC超のエンジン層とconformance_checkerが残る。**ワークフローは今や機械的だ**: 候補ファイルを約8つバッチにまとめる → サバイバーリストを読む → 本物か等価ミュータントのフロアかを裁定する → ターゲットを絞ったテストを書く → ハーネスを再実行する → spec専用でコミット。**2つのポリシー、load-bearing:**（1）ハーネスのテスト軸はコンベンションでマップされた*ユニット*specのみを実行し、統合/CLIディスパッチャーspecは決して実行しない;（2）**融合ハーネスを`make verify`と同時に実行しないこと**。トラッカーは6つの再利用可能なkill技法を記録している（不在キーの`fetch`デフォルト;共有された構造化フィールドに対するパス固有のメッセージテキスト;構築された配列への`include`でなく厳密な`eq`;nil区切り文字のある`join("\n")`に対する部分文字列でなく行数;`bytesize`のためのマルチバイトフィクスチャ;プライベートヘルパーへの`.send`）。ROADMAP §「Analyzer self-testing」;トラッカーは[`docs/notes/20260618-self-mutation-testing-plan.md`](../notes/20260618-self-mutation-testing-plan/)。
>
