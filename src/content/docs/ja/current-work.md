...
sourceSha: "0b80ed951006d0339788d8d28f9a5ef7d96f662e28a6bdaf4d25a962752b398d"
...
**最新リリース: v0.2.6（2026-06-27）** —— 6番目の`0.2.x`評価ラインのカットであり、[ADR-50](../adr/50-release-engineering-and-stability-strategy/)が統制する。カットごとの完全な記録は`CHANGELOG.md`（§ `[0.2.6]` … `[0.2.0]`）にある;ここで再要約しないこと。`make verify`はクリーン。

**master上の未リリース分（→ 次の`0.2.x`カット）: Railsの型カバレッジの弧**であり、`rigor-survey`のredmine / mastodonターゲットのオンボーディングから生まれた（2026-07-04;ノート[`20260704-rails-coverage-onboarding-carrier-trap.md`](../notes/20260704-rails-coverage-onboarding-carrier-trap/)、完全な詳細は下記のエントリーポイントブロックにある）。2つのスレッド:

- **`rigor coverage --protection`が保護を約13ポイント過小カウントしていた** —— そのスキャンスコープが（a）プラグインレジストリと（b）クロスファイルのクラス発見を欠いていたため、プラグインが型付けするレシーバー（`params`、`Model.where → Relation[Model]`）と兄弟ファイルのクラス定数（`Account`、`User`）が`Dynamic`と読まれていた。両方修正済み;メトリクスは今や`check`が実際に型付けするものと一致する（redmine `app`+`lib` 0.195 → **0.339**、mastodon `app/models` 0.177 → **0.311**）。要となる教訓: **`check`経由の`dump_type`がグラウンドトゥルースであり、`coverage`の比率が外れ値だった**（かつ、単一ファイルの`dump_type`プローブはクロスファイルのシンボルには誤りだ —— ディレクトリ全体を解析すること）。
- **rigor-actionpackが、リクエストコンテキストのリーダー**`params` / `session` / `request` / `flash` / `cookies`をそれぞれのAction Packクラスとして型付けするようになった —— **意図的にバンドルRBSを持たない**寛容な名前的型だ（具体的なレシーバー → 保護済み + ディスパッチ解決可能、メソッドサーフェスはエンジン寛容のまま → 偽陽性に安全;部分的なRBSはキャリア加算性の罠を再び引き起こすだろう）。加えてsig-gen修正（生成されたサブクラスRBSが今やそのスーパークラスを担う）と、それが表面化させたエンジン修正（不正なプロジェクトの`.rbs`がRBS環境全体をnullにしなくなった）。
...
> **▶ v0.2.6が最新リリース;masterは未リリースのRailsの型カバレッジの弧を運んでいる（上記）。進行中のリリースはない** —— 次のカットはユーザーがタグを打つ`0.2.x`評価ラインのリリースだ（バージョンバンプ + `rake release`はゲートされたまま;自律的にバンプしないこと）。次の実装者はRailsカバレッジの継続（0）、エンジン精度のキュー（A/B）、`0.2.x`評価ラインのタスク（C）、またはADR著作バックログ（D）から選ぶ。`make verify`はクリーン。
>
> **（0）Railsの型カバレッジの弧 —— 2026-07-04セッションの引き継ぎ（前提 + 次のレバー）:**
> redmine / mastodonのオンボーディングは1つのメタ発見に収束した: **`coverage --protection`のスキャンスコープは`check`のスコープを鏡写しにしなければならない**、さもないと比率が実際の保護を過小に述べる。2つの忠実度ギャップが修正された（プラグインレジストリ + クロスファイルの`discovered_classes`シード、どちらも`coverage_command.rb#scope_with_inferred_params` / `#plugin_aware_environment`内）;3つ目のファミリーが残っているかもしれない —— ランナーは`discovered_def_nodes` / `discovered_superclasses` / メンバーレイアウトテーブル（`Runner#project_scope_seed_tables`）もシードするが、カバレッジは依然としてそれらを省く。もっともこれらはレシーバーの型付き度よりメソッド解決に影響するので、保護のデルタはおそらく小さい（投資する前に検証すること）。
> - **リーダーファミリーはDONE**（rigor-actionpackの`params`/`session`/`request`/`flash`/`cookies`を`dynamic_return`経由で、寛容な名前的型、偽陽性ゼロ）。このパターンは、RigorがそのクラスのRBSを持たない任意のフレームワークのハッシュ的 / コンテキストオブジェクトへ一般化する。
> - **ADR-67（パラメータ推論）はすでに実装済みだ**（WD1+WD3+WD5）で、`ParameterInferenceCollector.collect`（3ラウンド上限付き不動点）経由で`coverage --protection`内を走る —— それが「提案済み / 未配線」だったという記憶は陳腐化している。WD2（構造的インターフェースの下限）が真に未構築のスライスであり、**欠落した構造的インターフェースキャリアにブロックされている**（新しい型キャリア + 受理 / 保護の処理 —— ADRの「最も安価」という枠付けより大きい;ADR-67 WD2のステータスを参照）。
> - **残りの未保護（これらのアプリで約67%）は素のDynamicなivar / アソシエーション / メソッドチェーン**であり、根は型なしパラメータ → Dynamicなivarだ。**ADR-58はDONEで、保護ではなくpossible-nilの発火ポリシーを統制する**（ADR: ivarを`Node | nil`へ精緻化すると偽陽性クラスは*悪化する*）。助けになるはずのフレームワークヘルパー型付け（`current_user` → `User`）は**安全なクイックスライスではない** —— それはnilableなので、`Model | nil`は使うたびにpossible-nilを発火させる;`t(...)` → `String`は完全に既知のクラスでundefined-method偽陽性のリスクがある。残る本当のレバーは**機能規模**だ: WD2キャリア、またはADR-58のフローライブなnilポリシーと正しく相互作用するnilability対応のヘルパー型付け。
> - オンボーディングの成果物はサーベイのチェックアウト内にUNTRACKEDで存在する（`~/repo/ruby/rigor-survey/{redmine,mastodon}/.rigor.dist.yml` + `.rigor-baseline.yml`、acknowledgeモード、プラグイン配線済み、`sig/`なし）。
>
