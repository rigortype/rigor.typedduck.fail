...
sourceSha: "46d2c80450d8342c4ac6b12ee65c152cb734547f807aa90bb58b7f4db7377f2c"
...
> - **セルフミューテーションのファイルごとの有効性（Product C）—— 成熟・繰り返し可能;残りのファイルのバッチ処理を続けること**。`tool/mutation/self_mutate.rb <file…>`（融合したself-check ∪ rspec）で`lib/rigor`のテストスイートの穴を塞ぐ。ツリー全体のコールドメソッド層は枯渇した（スイートはメソッドレベルで完全）;オープンな作業は`needs-verification`層（カバー済みだがアサートされていない）のファイルごとの裁定。**2026-06-21: 7つの融合バッチを通じて約28ファイルを等価ミュータントのフロアまでクローズした**（config_audit、lockfile_resolver、class_registry、reflection、method_catalog、conflict、diagnostic、options、mutation_protection_report、prism_colorizer、return_type_heuristic、builder、walker、coverage_command、hkt_reducer、hkt_registry、kernel_dispatch、overload_selector、precision_scanner、protection_scanner、project_patched_scanner、debouncer、document_symbol_provider、signature_help_provider、mcp/server、…）。**ワークフローは今や機械的だ**: 候補ファイルを約8つバッチにまとめる（60〜300 LOC、specあり、未計測）→ サバイバーリストを読む → 本物か等価ミュータントのフロアかを裁定する → ターゲットを絞ったテストを書く（プライベートヘルパーは`.send`経由）→ そのファイルでハーネスを再実行する → spec専用でコミット（rspec + rubocopで十分;`make check`/`check-plugins`は`lib`/`plugins`を解析し`spec`は解析しない）。**次セッションのエントリー: 60〜300 LOCで未計測のファイルが約50個残っている**（トラッカーのバッチが示すように除外リストを構築する）—— その後が300 LOC超のエンジン層。**2つのポリシー、load-bearing:**（1）ハーネスのテスト軸はコンベンションでマップされた*ユニット*specのみを実行し、統合/CLIディスパッチャーspecは決して実行しない。そのため`cli/*_command`オーケストレーションはほとんどが*統合ブラインドネス*である大きなカウントを報告する —— ディスパッチャーspecに対してトリアージし、コマンドのテストされていない*デフォルト*モードに対してのみユニットテストを追加し、メッセージ/ヘルプのテールは追加しない;（2）**融合ハーネスを`make verify`（または別のハーネス呼び出し）と同時に実行しないこと** —— それぞれがコールドenv+スキャンを実行し、互いにリソースを奪い合う。トラッカーは6つの再利用可能なkill技法を記録している（不在キーの`fetch`デフォルト;共有された構造化フィールドに対するパス固有のメッセージテキスト;構築された配列への`include`でなく厳密な`eq`;nil区切り文字のある`join("\n")`に対する部分文字列でなく行数;`bytesize`のためのマルチバイトフィクスチャ;プライベートヘルパーへの`.send`）。ROADMAP §「Analyzer self-testing」;トラッカーは[`docs/notes/20260618-self-mutation-testing-plan.md`](../notes/20260618-self-mutation-testing-plan/)。下記のメソッドコールドネスによるノイズ除去のGotchaに留意すること。
>
