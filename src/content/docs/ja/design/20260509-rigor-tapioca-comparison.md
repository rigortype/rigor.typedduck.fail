...
このドキュメントは参考情報。プラグイン契約の拘束力のあるソースは[ADR-2](../../adr/2-extension-api/)。Sorbetインプットプラグインの拘束力のあるソースは[ADR-11](../../adr/11-sorbet-input-adapter/)。
...
Rigorのプラグイン契約は**明示的にPHPStanをモデルにしている**
...
Rigorはソースのみから証明できるものについてRBIを生成する — クラス定義、メソッドシグネチャ（RBS/`RBS::Inline`/推論された戻り値型から）、定数型。重いメタプログラミングではTapiocaのカバレッジに及ばないが、「コミット前のスモークテスト」ワークフローには十分。
