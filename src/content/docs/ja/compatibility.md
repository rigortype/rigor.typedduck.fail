...
sourceSha: "84e7246eef7b7785ac31c2008ef89231c09101be7f56efcc8207a455a063dbdb"
...
ステータス: **ドラフト（v0.1.xプレビューライン。v0.2.0トライアルとv1.0.0凍結に向けて列挙中）**。
...
| ライン | ここでの安定性の意味 |
| --- | --- |
| **`0.1.x` — プレビュー（現行）** | サーフェスはまだ*拡張中*であり、ライン内での破壊的変更が許容されます。この文書はサーフェスを**列挙**して凍結に標的を与えますが、まだ拘束力は持ちません。（例えば[ADR-60](../adr/60-pre-freeze-plugin-contract-consolidation/)は、このラインで最後の機会となるプラグイン契約の後方互換性破壊を意図的に取り込みました。） |
| **`v0.2.0` — 評価トライアル** | **列挙されたサーフェスでのマイナー非破壊を約束する**最初のバージョンであり、運用上の<ruby>規律<rp>（</rp><rt>discipline</rt><rp>）</rp></ruby>として位置づけられます。推論エンジンがまだ進化を続ける中で実施される、v1.0.0契約のリハーサルです。サーフェスは列挙され、トライアルとしてコミットされますが、凍結はされません。 |
| **`v1.0.0` — ハード凍結** | 以下に列挙されたサーフェスが互換性ポリシーのもとで**拘束力を持つ**ようになります。適合ユーザーの設定・プラグイン・抑制を無効化する変更は破壊的変更となり、これ以降はメジャーバージョンでのみ許されます。 |

つまり今日（`0.1.x`のカット）これは**ドラフトの列挙**です。v0.2.0がトライアルとしてコミットし、v1.0.0が凍結することになるサーフェスです。ラインがプレビュー中の間は、特定のRigorバージョンを固定してください。
