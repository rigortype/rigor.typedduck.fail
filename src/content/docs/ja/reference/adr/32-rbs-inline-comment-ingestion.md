...
`# @rbs name: :asc | :desc`で宣言されたパラメータ型は`def f: (:asc | :desc) -> ...`と全く同様にパラメータをバインドする。パラメータバインダーは区別しない;診断コードは同じ;ADR-5の堅牢性非対称の著作ルールが同じ境界で適用される。
...
- [ADR-5](../5-robustness-principle/) — 堅牢性非対称の著作;インラインRBSユーザー宣言パラメータは等価な実`.rbs`宣言と全く同様に扱われる（WD7）。
