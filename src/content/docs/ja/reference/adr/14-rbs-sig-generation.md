...
- ADR-5に観察可能に準拠したまま: デフォルトで戻り値は厳格、デフォルトでパラメータは寛容、ユーザーがパラメータの絞り込みにオプトイン。
...
- **`observed-strict`** — `observed`と同じだが、その上に生成器は既知のケイパビリティ（capability）ロール（例えば、すべての呼び出し元が`.to_s`しか消費しない観察された`String`パラメータは`_ToStr`に広がる）にもさらに広げる。これは節2の最大限の設定;ケイパビリティロールカタログ（v0.1.xはまだ出荷していない — §「未解決の問題」を参照）を必要とする。フラグは予約されているがそのカタログが存在するまで不活性。
