...
- **スライス2a** — `Rigor::Inference::HktBody`（5つの`Data.define`ボディ木ノード型: `TypeLeaf`、`Param`、`AppRef`、`Union`、`NominalApp`）;遅延自己再帰処理のための呼び出しごとの進行中スタック（再帰的な総和が終了するための「結び目固め」トリック）と燃料予算（WD3に従いデフォルト64）を実装する`Rigor::Inference::HktReducer`によるD4評価ルール。`Definition#body_tree`スロットを追加;ボディ文字列はスライス2bパーサー用にそのまま並んで残る。`Type::App#reduce` + `HktRegistry#reduce`便宜ラッパー。
- **スライス2b** — `Rigor::Inference::HktBodyParser`（最小文法: union + アトム + nominal_app + app_ref + param、`JSON.parse`の再帰的総和と類似の再帰データ形シグネチャに十分;条件 / インデックスアクセス形はフォローアップに残る）。`HktDirectives.parse_define`は今、自動的にパーサーを呼び出しボディ文字列から`body_tree`を埋める;パース失敗はfail-softで`body_tree: nil` + `:info`レポーターエントリになる。エンドツーエンドのレデューサ等価性がプログラマティックなJSON_VALUEボディに対して検証された。
...
- **スライス5** — 再帰的`type`エイリアス経由のシュガー。明示的`%a{...}`形が冗長すぎるというユーザーフィードバックにゲート。
...
`JSON.parse`に対して実際に健全な最小精度フロアは**再帰的総和**:
...
3. **`rigor-lisp-eval`デモ**。
   [デモシグネチャ](https://github.com/rigortype/rigor/blob/main/examples/rigor-lisp-eval/demo/sig/lisp.rbs)
...
コンパクトなシュガー形での同じ登録（提案;最終構文はオープン質問OQ4でTBD）:
...
— アナライザがRHSで自身を名指す再帰的`type`エイリアスをタグの暗黙的登録として認識することで。これは*シュガー*パス;明示的な`%a{rigor:v1:hkt_register}` / `%a{rigor:v1:hkt_define}`ペイロードが正準形のまま。
...
- **WD5.**再帰的`type`エイリアス経由のシュガー構文（D2の2番目のブロック）は*aspirational*;v1は明示的な`%a{…}`形のみを出荷する。シュガーはユーザーフィードバックにゲートされたフォローアップスライス。
...
- バンドルされたJSON RBSディスパッチパスを更新、`JSON.parse(str)`が`App[json::value, String]`に解決され、ナローイング時に再帰的総和に簡約されるように。
...
- **OQ4.**シュガー構文: どの形を出荷するか？ 3つの候補: （a）再帰的`type`エイリアス（D2 2番目のブロック）;(b) Sorbetの`T.type_alias`風;（c）明示的な`%a{…}`ペイロードのみを残す。暫定: スライス1〜3は（c）、フィードバックが要求すればスライス5で（a）。
