...
SorbetやSteep、RBSといったツールの登場によってRubyエコシステムが成熟するにつれ、**型推論**と**実用的な漸進的型付け**（gradual typing）を最大限に活かす静的解析器への需要が高まっています。既存のツールはアプリケーションコードに多くの型アノテーションを要求することが多く、それが人間の開発者にとっても、AIコーディングアシスタント（LLM）にとっても、ノイズになっています。さらに、Rubyの動的な性質とメタプログラミングは、従来の型チェッカーにとって依然として大きな障壁です。
