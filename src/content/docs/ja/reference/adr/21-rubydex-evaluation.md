...
| 3 | ツール（LSPクロスファイル機能） | **条件付き承認、キュー**。`plugins/rigor-lsp-rubydex/`下で`textDocument/definition` / `textDocument/references` / `workspace/symbol`の*オプトイン*LSPケイパビリティ（capability）プロバイダーとして採用。アナライザの主要パスを変更しない。 | LSPロードマップがこれらの能力の1つにコミットしたとき。 |
