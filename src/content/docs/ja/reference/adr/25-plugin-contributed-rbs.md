...
`Plugin::Base`プラグインは、診断（`diagnostics_for_file`）、呼び出しごとの戻り値型（`flow_contribution_for`）、マクロサブストレート宣言、型ノードリゾルバ、プラグインをまたぐファクトを提供できる — しかし**RBSは提供できない**。「このプラグインがロードされたときにシグネチャ環境を拡張する」というマニフェストフックが存在しない。
...
### WD6 — 1.0以前のプラグインコントラクトに加法的

プラグインコントラクト（ADR-2）は1.0以前であり、v0.2.0で安定化する。この変更は**新しいオプショナルなマニフェストフィールド**である — `signature_paths:`を宣言しないプラグインは影響を受けず、既存のプラグインは何も壊れない。v0.1.x系統内で投入しても安全であり、v0.2.0が凍結するコントラクトサーフェスの一部であるべきである。
...
- **マニフェストフィールドがひとつ増える** — 1.0以前のコントラクトに。加法的かつ低リスク（WD6）。
...
- [ADR-2](../2-extension-api/) — このADRが拡張するプラグインコントラクト。
