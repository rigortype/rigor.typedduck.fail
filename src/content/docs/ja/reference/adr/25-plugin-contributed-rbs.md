...
プラグイン契約（contract）（ADR-2）は1.0以前であり、v0.2.0で安定化する。この変更は**新しいオプショナルなマニフェストフィールド**である — `signature_paths:`を宣言しないプラグインは影響を受けず、既存のプラグインは何も壊れない。v0.1.x系統内で投入しても安全であり、v0.2.0が凍結する契約サーフェスの一部であるべきである。
