...
- すべてのRigorで推論された型は有効なRBSに保守的に消去できますが、Rigor→RBSの方向は一般的に*無損失ではありません*。すなわち消去はリファインメント（refinement、篩型とも）・リテラルunion・シェイプ・動的起源のproductnanceを縮退させるかもしれません。消去はRigorが証明したより狭い型を決して生成してはなりませんが、より広い型を生成してかまいません。
