...
Rigorが自身のシグネチャ（組み込みカタログエントリー・推論されたユーザーメソッド型・RBS::Extendedペイロード）を書くときに候補型から*選ぶ*方法は、[ADR-5](../5-robustness-principle/)の非対称頑健性原則によって統治されます。すなわち下流の精度伝播を最大化するための厳格な戻り値、呼び出し元に防御的な強制を貼り付けるよう強いるのを避けるための寛容な引数です。手書きのRBSオーサリングが束縛します。原則はRigor自身がシグネチャを書くときのデフォルト選択を導きます。
