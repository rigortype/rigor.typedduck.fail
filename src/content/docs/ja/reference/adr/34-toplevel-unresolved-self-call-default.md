...
sourceSha: "ebfd715d0a07942fd735502e95ce6c9ba9986e47df8073f6c461a4a6fc505aec"
...
この挙動はADR-24 WD3——「unresolvedなself呼び出しは`Dynamic[top]`のまま」——をすべてのコンテキストに均一に適用した直接の結果である。WD3がその判断を下した理由は、フォルスポジティブの規律としてRailsモデルクラスボディに対するものだった: すべての`attr_accessor` / `has_many` / `scope` / `validates`は、静的ウォーカーが解決できない`self`コンテキスト呼び出しであり、unresolvedなケースをエラーにすると、そのドメインがノイズで溢れる。
...
**Why:**フォルスポジティブの規律はプロジェクトの最優先価値（`feedback_false_positive_discipline`の下でメモリに記録済み）。エスケープハッチなしに診断を出荷すると、動作するコードが怯えてしまい、診断が価値を加えるより速く信頼を損なう。
...
ADR-17スライス3（キャッシュディスクリプター）は独立しており、ADR-34スライス1の前後どちらでもランドできる。
