...
この境界は健全性（soundness）、パフォーマンス、サードパーティコントラクトサーフェスの安定性のための正しいデフォルトです。しかし、ユーザーが表面化させた2つの具体的な問題点があります:
...
- 解析器は`paths:`を走査するのと同じエンジンを使用してGemの`.rb`ファイルを走査します。メソッドディスパッチ、ナロイング、ファクト（fact）ストア、バジェット適用が変更なく実行されます。
