...
次のスライス（slice）は**`rigor-dry-types`**（Tier A基盤）です。
...
上位tierプラグインは[ADR-9](../9-cross-plugin-api/)の`Plugin::FactStore`チャネル経由でTier-Aプラグインのファクト（fact）を消費します。dry-*プラグインの正準なチャネル名は次のとおり。
...
- **dry-monadsの`Result[T, E]`キャリア**。`Rigor::Type::*`内の忠実なResult / Maybeキャリア（carrier）があれば、`rigor-dry-monads`が`.success?` / `.failure?`述語に正確なナローイング（narrowing）を貢献できる。今日のプラグインは`flow_contribution_for`が`Dynamic[T]`タグ付き和を返すことで貢献している。キャリア導入は別個のADR（ADR-3の修正）であり、ここではスコープ外。
