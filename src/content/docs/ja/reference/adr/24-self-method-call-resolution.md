...
さらに広く言えば、プロジェクト内の*すべての*`self`メソッド呼び出しがその実際の戻り型とパラメータ契約を失う — `self`メソッドに対するチェインされた推論が`Dynamic`に縮退し、`self`呼び出しに対する本物のアリティ / 引数型バグが検出されない。ガードヘルパークラスタは単に最も可視性の高い症状にすぎない。
