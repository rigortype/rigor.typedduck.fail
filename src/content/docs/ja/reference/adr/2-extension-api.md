...
| メタデータ | クラスリフレクション拡張、カスタム非推奨、許可された部分型、追加コンストラクター、例外分類、条件付きスタブファイル。 | Rigorはプラグインが動的Rubyメンバー、部分型ファクト、初期化メソッド、非推奨/内部メタデータ、例外ポリシー、生成されたまたは条件付きRBSを提供できるようにすべきです。 |
