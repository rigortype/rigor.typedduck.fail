...
1. **フェーズ1 — 値オブジェクトの共有可能性（着地済み）**。エンジンがディスパッチを通じて送るすべてのリーフキャリアは構築時に`Ractor.shareable?`。カバレッジ: `Type::*`（16クラス）、`TypeNode::*`（7クラス）、`Cache::Descriptor`、`Analysis::FactStore`、`FlowContribution`。リグレッションガードは[`spec/rigor/ractor_readiness_spec.rb`](https://github.com/rigortype/rigor/blob/master/spec/rigor/ractor_readiness_spec.rb)。
