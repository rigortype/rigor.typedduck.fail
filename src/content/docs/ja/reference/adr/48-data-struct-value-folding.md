...
sourceSha: "bf499ddfb2703f6301eafc8cbfc662ec334eef729fd2b495c36c0853d072eb35"
...
## Structのフォローアップ（先送り、独自の健全性ストーリーを持つ別スライス）

`Struct`はこのADRのコミット済みスコープに**含まれない**。クラスキャリアはほぼ同じ（`keyword_init: bool`フィールドと`Struct.new("Name", :x, keyword_init: true)`の引数シェイプ解析を追加するだけで、既存の`struct_new_positionals`が処理する）。しかし**インスタンスキャリアは変異のもとで不健全**だ: `Struct`には`x=`セッターと`[]=`があり、`StructInstance`のメンバーマップは後続の書き込み・エイリアス経由の外部変異で無効化される可能性がある。これを健全にモデル化するには、（a）フローセンシティブな無効化（観測されたセッター/`[]=`/インスタンスのエスケープ時にメンバー値精度を落とす — `ScopeIndexer#widen_member_for_observed_mutators`の変異拡大機構が先行技術）か、（b）解析が読み取りまでに変異が到達しないことを証明した場合のみメンバー*読み取り*を畳み込む、のいずれかが必要だ。どちらも本物の設計サーフェスであり、`Data`がキャリアシェイプを実証した後、独自のADRスライスに属する。`DataClass` / `DataInstance`キャリアは意図的に`Data*`という名前にしている（中立的な`Record*`でなく）。この境界を明示的に保つため。Structフォローアップは、`kind:`識別子を追加してリネームするか、`StructClass` / `StructInstance`キャリアを姉妹として追加するかを決める。
