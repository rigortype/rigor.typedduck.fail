...
- **2つの関係を分離して保つ**。サブタイピング（`A <: B`、値集合の包含）と漸進的一貫性（`consistent(A, B)`、動的境界の互換性）は統一しません。`untyped`は動的型であり、`top`とは別物です。
...
| 機能 | 詳細はこちら |
| --- | --- |
| `Dynamic[T]`代数と漸進的型付けのprovenance | [value-lattice.md](../type-specification/value-lattice/), [special-types.md](../type-specification/special-types/) |
