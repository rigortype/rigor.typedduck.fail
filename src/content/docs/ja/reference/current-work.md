...
sourceSha: "cc8997ffc72c596d86081cccb8641682d27bc3aec29c7e9d64b116c2d539e7d2"
...
1. **スライス1を完了** —— `ScopeIndexer`プリパスにclass / superclass / includeのソースマップを追加し（プリパスは既に`path`を保持している —— `merge_discovered_defs`のメソッド帰属をミラーする）、残りの`Scope`アクセサ（`superclass_of`、`includes_of`、発見済みクラスのルックアップ）を計装する;`deps` / `dependents` + ファイル単位の診断エントリーを永続化する（ADR-45の`Cache::Store#fetch_or_validate`を再利用）;**必須**の`--verify-incremental`クロスチェックモードを追加する（インクリメンタル対 フル`--no-cache`がバイト単位で同一でなければならない —— 受け入れゲート）。
