...
1. **外部ユーザーSKILLトリオ**（[ADR-22 § WD8](../adr/22-baseline-and-project-onboarding/)）— **完了**。トップレベルの`skills/`ツリー下の、自分自身のプロジェクトに対して`gem install rigortype`を実行するRigor新参者を狙った3つのSKILL（agentskills.ioポータブル規約、`waza check`仕様準拠、パブリックCLIサーフェスのみ）:
   - `skills/rigor-project-init/` — 初回オンボーディング: Gemfile / Gemfile.lockウォーク → プラグインセットを提案 → **採用モード選択**（アクノレッジ / ベースライン対ストリクト） → `.rigor.dist.yml`を書く → `rigor triage --format json`を実行 → （アクノレッジモード）`.rigor-baseline.yml` + `baseline:`設定行を書く → 本物のバグの可能性を表面化 → 2つのエスカレーションパス（プロジェクトプラグイン / Rigor issue）を提示。SKILL.md + 3つの`references/`モジュール。
