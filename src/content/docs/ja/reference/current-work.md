...
sourceSha: "00fe778fefb0e80e95ddc05c9de7555d26eb7334e748949296abd6e94d841948"
...
`make verify`はクリーン。**進行中のアクティブな作業はADR-46のインクリメンタル解析トラック**（v0.1.17パフォーマンスサイクルの目玉）。そこから再開する:

**ADR-46 —— ファイル横断の依存グラフによるインクリメンタル解析**（[`docs/adr/46-incremental-dependency-graph.md`](../adr/46-incremental-dependency-graph/) §「Staging」）。ADR-45の粗い全体実行キャッシュのファイル単位の後継: リーフコントローラーを編集 → 1ファイルだけ再チェック;モデルを編集 → そのモデル + その呼び出し元を再チェック。**スライス1a着地済み**（コミット`4eccc390`）: `Analysis::DependencyRecorder` + 計装された`Scope#user_def_for`チョークポイント、`Runner.new(record_dependencies: true)` → `runner.file_dependencies`でオプトイン、デフォルトはオフ。**次は順番に:**

1. **スライス1を完了**。**スライス1bランディング済み**（祖先エッジ）: `class_sources`マップ（`class_name → Set<宣言ファイル>`）が`ScopeIndexer.accumulate_project_index`で構築され、記録スコープにシードされ、`Scope#superclass_of` / `#includes_of`が解決時に宣言ファイルの完全な集合を記録する（複数ファイルにまたがって再オープンされたクラス → 消費者がすべての再オープンに依存;設計上の過剰記録）。同じ変更で潜在的なスライス1aのギャップを修正した —— ADR-44の単一アロケーションボディスコープ（`build_user_method_body_scope` / `build_fresh_body_scope`）が`discovered_def_sources`を落としていたため、メソッドボディエッジが暗黙的selfの祖先解決呼び出しに対してサイレントに記録されていなかった;両者は現在`def_sources` + `class_sources`を持つ。デフォルトはオフ、`make verify`グリーン、診断結果はバイト同一。**スライス1で残るTODO:**（a）発見済み*クラス*ルックアップエッジ —— クラス名解決は`discovered_classes`ハッシュを直接読む（チョークポイントのアクセサがない）ため、ファイルが`class Foo`の*存在*（その祖先ではなく）に依存することはまだ記録されていない;チョークポイントのアクセサか、クラス解決時のネガティブエッジのパスのいずれかが必要。（b）`deps` / `dependents` + ファイル単位の診断エントリーを永続化する（ADR-45の`Cache::Store#fetch_or_validate`を再利用）。（c）**必須**の`--verify-incremental`クロスチェックモード（インクリメンタル対 フル`--no-cache`がバイト単位で同一でなければならない —— 受け入れゲート）。
2. **スライス2 —— ボディ層**（宣言構造のフィンガープリントが未変更のとき`ΔF ∪ dependents`を再解析する）。これがMVCの勝利をもたらすスライス。
3. スライス3（構造層のネガティブ依存追跡）+ 4（シンボル粒度）はADR-46 §「Staging」に従う。
...
- **ADR-46 —— インクリメンタル依存グラフ（設計 + スライス1a）**。上記の次セッションのエントリーポイント。ファイル単位の依存を`Scope`アクセサのチョークポイントで記録 → `dependents`インデックス → ファイル単位のキャッシュ;健全性は必須の`--verify-incremental`クロスチェックでゲートされる。[ADR-46](../adr/46-incremental-dependency-graph/)とそのメモリノートを参照。
