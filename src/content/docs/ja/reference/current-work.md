...
sourceSha: "0ac1047a9e6cdfd9f0e4b2cad66a80fc53d8c445454c1d4952f90106b3b7ca64"
...
### 推論バジェット — 仕様表は未配線;2層のフォローアップ

調査と新しい`RIGOR_BUDGET_TRACE`計装が2026-06-03に着地（コミット`42df2481`;ノート[`docs/notes/20260603-inference-budget-reality-survey.md`](../notes/20260603-inference-budget-reality-survey/)）。
判明したこと: 仕様の設定可能な`budgets:`表（[`docs/type-specification/inference-budgets.md`](../type-specification/inference-budgets/)）はv1向けに規範的でありながら**配線されていない** — 実際に効いているカットオフは、ハードコードされた3つのサイレントガード（再帰の再入≈深さ1、祖先ウォーク100、HKT fuel 64）とADR-10の`budget_per_gem`だけです。これらのガードは小さな再帰下降パーサで発火し、本当のコストの崖とは*直交*しています（Redmine: 331ファイル / 172秒 / 1.5 GB、再帰ガードヒットはわずか71件）。フォローアップは2層です:

- **Layer 1 — ドキュメント/仕様の衛生（安価・高確度）**。 `docs/manual/03-configuration.md`の`budget_per_gem`のバグを修正（「時間バジェット、ミリ秒、デフォルト1000」とあるが、実際にはメソッド定義の**個数**でデフォルトは**5000**）。`recursion_depth`を整合させる（仕様5対 配線済みの深さ1終了ガード — 「終了フロア」と「精度アンロール深さ」を分離）。`ancestor_walk`（100）と`hkt_fuel`（64）を文書化された表に追加。推論バジェットの欠けているユーザー向け説明を執筆（配置は未定 — ハンドブックの付録か、マニュアルのセクションか）。
- **Layer 2 — 解決済み。そしてそれはバジェットではなかった**。スライス2aは`union_size`を反証（メモリはユニオン型の幅と無相関）。スライス2bはRedmineをヒーププロファイルし、1.5 GBが**`rigor-activerecord`の1件のメモ化漏れに起因する4.2 M個の保持String**だったことを発見（`schema_table_or_nil`が`db/schema.rb`欠如時にAR呼び出し箇所ごとにエラー文字列を再追加していた）。修正済み（コミット`b1fbcba3`）: Redmine 1518 MB / 173秒 → **217 MB / 84秒**（−86 % / −51 %）、診断は不変;Mastodonは元々影響なし（`schema.rb`を同梱しているため）。大規模アプリのコストの崖とバジェット表は**切り離されています**。バジェット配線（`union_size` / `structural_growth`）は現在**デマンド先送り** — バジェット型のコストを示すコーパスプロジェクトは存在しません;もし将来現れたら、まず2a形式の分布プローブを再実行します（ADR-41 WD3）。

順序付け（すべて**完了**）: （1）調査ノート＋記録;（2）先行研究ノート（[`docs/notes/20260603-inference-cutoff-prior-art.md`](../notes/20260603-inference-cutoff-prior-art/)）;（3）理想設計ADR（[ADR-41](../adr/41-inference-budget-design/)、Proposed、Evaluation判定＋スライス2a/2bの結果付き）;（4）Layer 1の安全なドキュメント修正（マニュアルの`budget_per_gem`バグ＋仕様の実装状況ノート）;（5）スライス2aのユニオン型アリティ分布（Survey 4 — `union_size`を反証）;（6）スライス2bのヒーププロファイル＋`rigor-activerecord`の修正（Survey 5）。**残り、すべてデマンドゲート付き:**規範的な仕様値の変更（`recursion_depth` 5→1＋`ancestor_walk`/`hkt_fuel`行）＋ユーザー向けバジェットドキュメントはADR-41の受け入れ待ち;バジェット配線はコストを示すプロジェクト待ち。`RIGOR_BUDGET_TRACE` / `RIGOR_HEAP_PROFILE` / `RIGOR_HEAP_TRACE`プローブは着地済みで再利用可能です。
