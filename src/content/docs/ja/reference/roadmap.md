...
sourceSha: "3e0daf16f05681e534e239f333887537b74c1d392b7e2816aba6349a11ab42f2"
...
以下のv0.2.0ゲートはその後**3つから2つに削減された**: SKILLトリオ（ゲート3）が出荷され、サブツリー分割／プラグインごとの公開ゲートは単一バンドル`rigortype` gem配布モデル（[ADR-31](../adr/31-contribution-and-supply-chain-policy/) + コミット`9769f5fa`）によって**置き換えられた**。残る唯一の実質的なゲートは、*外部*のサードパーティ`rigor-*` gem（`gem "rigortype"`に依存する作者自身のリポジトリ）のためのプラグイン契約安定化である。
...
5. **作成者ヘルパー採用ギャップ（ボイラープレート計画フェーズ0a/0b——ヘルパーは着地済み、コンシューマーは移行中）**。2026-06-02調査: 0a/0bヘルパーは存在するが、ほとんどのプラグインは退役させるはずだった重複を依然手書きしている。すべてFP安全な純粋リファクタリング（振る舞いを保存;各々プラグインのゴールデンマスタースペック + `make verify`で保護）。**（b）着地済み**——`Diagnostic.from_node` / `.from_location`が、実際の`Rigor::Analysis::Diagnostic`を構築する4つのプロダクション解析器（activerecord / activestorage / hanami / sorbet）で採用された;`rigor-rspec`（実際のクラスではなくローカルの`Diagnostic` Struct）と`rigor-actionmailer`（その`start_column+1`は診断ではなく発見インデックスの`def_column`）は異なるパターンとして正しく除外された。**（c）着地済み**——例プラグイン自身の`def diagnostic`（`rigor-pattern` / `rigor-routes` → 継承された`Base#diagnostic`;`rigor-web`の`ProtocolChecker`ヘルパー → `Diagnostic.from_location`）。**（a）着地済み**——ボイラープレート計画のQ1前提条件（**シンボル専用 + String返却の`Source::Literals`バリアント**）が出荷された: 抽出グリッドは両軸（受け入れる種別 × 返却型）にわたって完成し——`symbol` / `symbol_name` / `symbol_or_string_name`が`symbol_or_string`に加わり、公開APIドリフトスペック + RBSシグで固定された。**10個のバンドルプラグイン + 1個の例が移行された**（statesman / rspec / activestorage / factorybot / actionpack / rails-routes / graphql / actionmailer / dry-schema / activerecord + patternの例）、各々ゴールデンマスタースペックに対して振る舞いを保存。**残り（a-tail）:** sorbet / activerecord / graphqlなどのassocキーの*名前一致*イディオム（`el.key.is_a?(SymbolNode) && el.key.unescaped == "x"`）は値抽出ではなくキー比較である——4ヘルパーグリッドの外;専用の`symbol_named?(node, name)`ヘルパーがそれを吸収できるが、独自の（需要駆動の）スライスである。
...
### ドキュメント — ユーザー向けドキュメントのオーバーホール（完了、ドキュメント限定）

エンジン／プラグイン契約作業とは独立した、ユーザー向けドキュメントに対する[`doc-coauthoring`](../.claude/skills/)ユーザーフレンドリー化パス。**完了:**ハンドブック（全19ファイル — 12章 + 7付録）、マニュアル（全14章）、[`docs/types.md`](../types/) — 章のオリエンテーション + 「この章の内容」ミニ目次、すべてのアンカーをコールドリード検証、加えて*コード検証済み*のドキュメント正確性修正の一括。最大の修正: ハンドブックの`assert_type(…)`の例は値先頭だったが、エンジンは`assert_type("type-string", expr)`を要求する（引数0は文字列リテラルでなければならない）ため、41個すべてが黙ってnoopしていた — 反転した。その他: `disable`設定キー（ドキュメントは無視される`disabled_rules:`を書いていた）、forkであって「Ractor」ではないワーカーの表現（ADR-15）、診断カタログの網羅性、HKT章のバンドル登録ファクト（fact）（`json::value` + `csv::parsed`、9メソッド）。**プラグインごとのドキュメント — 完了:**バンドルされた30個のチェッカープラグインすべてが`docs/manual/plugins/<id>.md`ユーザーページを持つようになった（「（ii）」分割 — 公開マニュアルにコンシューマービュー;開発者／契約資料は`plugins/<id>/README.md`に留まり、スリム化＋陳腐化解消され、自身のページへリンクアップする）。`rigor-playground`は意図的にページなし（インフラ、チェッカーではない）。突き合わせパス中に表面化した2件のプラグイン*コード*ギャップは、メンテナーのレビュー判断のためにフラグされている（ドキュメント修正ではない）: `rigor-dry-validation`のRBSオーバーレイ配線は（activerecordがそうしているように）陳腐な`vendor/bundle/…`パスの代わりにADR-25のマニフェスト`signature_paths:`を採用すべき;また`rigor-shoulda-matchers`はルールIDを二重プレフィックスする（`plugin.shoulda-matchers.shoulda-matchers.…`）。移行の*手法*は将来のプラグイン追加のために[`docs/notes/20260603-plugin-doc-migration-playbook.md`](../notes/20260603-plugin-doc-migration-playbook/)に保存されている。完全な詳細 + 移行チェックリストは[`docs/CURRENT_WORK.md`](../current-work/) §「ドキュメントのオーバーホール」にある。ドキュメント限定（コード変更なし）;コミットは`master`上にローカルで、プッシュされていない。
