...
sourceSha: "7da0afb3964efe1c4c7dd8a9c697cd916d42eb40bf1f47b4cb829870fd0e8686"
...
- **ADR-28パススコープのプロトコルコントラクト — エコシステム機会**。[ADR-28](../adr/28-path-scoped-protocol-contracts/)（2026-05-23）はprovide-and-check拡張ポイントを導入する。このメカニズムはフレームワークが規約ディレクトリ内のすべてのファイルに**均一なメソッドシグネチャ**を要求する場合に最も有益。既存プラグインのエコシステム適合性監査（適合度順）:
  - **`rigor-hanami`**（新プラグイン — **v0.1.9サイクルでランド済み**）: `app/actions/**/*.rb`、`#handle(Hanami::Action::Request, Hanami::Action::Response) → void`。ADR-28の正規プロダクションプラグイン消費者。アクションボディへの型付きパラメーター提供により、`request` / `response`の誤用がコアの`call.undefined-method`診断として表面化する。`#handle`はvoid（レスポンスはインプレースで変更される）なので戻り型チェックは意図的にスキップ（`return_type_name: nil`）。設定オーバーライド: `action_path:`。
  - **`rigor-actioncable`強化**（需要駆動）: `app/channels/**/*.rb`の`#receive(data)`メソッドは現在`data`を`Dynamic[Top]`として受け取る。`method_name: :receive, param_types: [{index: 0, type_name: "Hash"}]`のコントラクトにより、すべてのチャネルのreceiveボディ内で`data`が`Hash`として型付けされる。`#subscribed` / `#unsubscribed`コールバックはパラメーターもvoid戻り型もなく、ADR-28の実用的なユースケースではない（提供するパラメーターも、チェックする戻り値もない）。
  - **`rigor-activejob` / `rigor-sidekiq`**（適合しない）: `#perform`シグネチャはジョブ/ワーカークラスごとに異なる — 位置インデックスモデルは均一なシグネチャを要求する。両プラグインはすでにインデックスベースのアナライザーで呼び出しサイトのarityをカバーしており、ADR-28コントラクトの追加価値はvoid戻り型チェックのみで、実際には優先度が低い。
  - **`rigor-graphql` / `rigor-pundit`**（適用不可）: graphql-ruby 2.xリゾルバメソッドはキーワード引数を使用（位置指定ではない）;ADR-28モデルは位置インデックスのみをマップする。Punditポリシーメソッドの型（`user`、`record`）はプロジェクト固有であり、プラグインマニフェストで汎用的に宣言できない。
  - `rigor-hanami`のランディングの一環として`ProtocolContract#return_type_name`はオプション（nil = void、戻り型チェックをスキップ）になった。`ProtocolChecker`（rigor-web）は名前的ルックアップの試み前にnilをガードする。
