...
sourceSha: "eabfea8f35863fd4f59b5abbb0b286ebbd1dfe25f5261a56226a24c489179e61"
...
### エージェントのワークフロー / SKILL（コミット済み: v0.2.0）

- **外部著作者向けプラグインSKILL**。今日の[`.claude/skills/rigor-plugin-author/`](https://github.com/rigortype/rigor/blob/main/.claude/skills/rigor-plugin-author/)は**コントリビューター専用**である — rigorモノレポの`Makefile`、`spec/integration/{plugins,examples}/`、`.rubocop.yml`、そして`plugins/<id>/` / `examples/<id>/`配置を前提とし、`make verify` / `bundle exec exe/rigor check lib`のようなrigor内部コマンドを参照する**。外部ライブラリ開発者およびプロジェクト固有プラグイン著作者**向けに別のSKILL — 独自リポジトリで`gem "rigortype"`に依存するスタンドアロンの`rigor-foo` gemを書く人、または独自の`lib/`下でプロジェクト非公開プラグインを保守する人 — がv0.2.0サイクル向けにコミットされている。今日（v0.1.x）はスコープ外: このモノレポの外に外部の`rigor-*` gemは存在しない;ADR-2プラグイン契約サーフェスは依然pre-1.0;subtree-split / RubyGems公開フローは内部のまま。外部SKILLは次をカバーすべき: （a）公開された`rigortype` APIサーフェスに対するgemspec著作、（b）rigorの`plugin_helpers.rb`なしでのテストフレームワーク選択（RSpec / Minitest）、（c）プロジェクト非公開プラグインのパターン（gemspecなし、`Dir["lib/**/*.rb"]`などで読み込む）、（d）バージョンピンニング + 破壊的変更戦略、（e）プロジェクト側型のための`rbs_collection`統合。このSKILLが`.claude/skills/`に戻されたときに空いた、新しいトップレベル`skills/`ツリーの下に住むことになる可能性が高い。
...
- **Tier 1**: [`rigor-rails-routes`](../plugins/rigor-rails-routes/)（`:helper_table`を公開）、[`rigor-rails-i18n`](../plugins/rigor-rails-i18n/)、[`rigor-actionmailer`](../plugins/rigor-actionmailer/)、[`rigor-activejob`](../plugins/rigor-activejob/)。
- **Tier 2**: [`rigor-activerecord`](../plugins/rigor-activerecord/)（`:model_index`を公開;アソシエーション / enum / スコープ / バリデーション / コールバックすべてv0.1.5で着地）;[`rigor-actionpack`](../plugins/rigor-actionpack/)（4フェーズ: ルート / フィルター / レンダー / ストロングパラメーター）;[`rigor-factorybot`](../plugins/rigor-factorybot/)（フェーズ1 (a) + （c））。
- **Tier 3**: [`rigor-pundit`](../plugins/rigor-pundit/)、[`rigor-sidekiq`](../plugins/rigor-sidekiq/)、[`rigor-rspec`](../plugins/rigor-rspec/)、[`rigor-actioncable`](../plugins/rigor-actioncable/)、[`rigor-activestorage`](../plugins/rigor-activestorage/)（v0.1.5で着地）;[`rigor-graphql`](../plugins/rigor-graphql/)（v0.1.6 — Tier 3D、スライス1+2a+2b+2c+2dがクローズ）。
- **オプトインの非プラグインバンドル**: [`rigor-activesupport-core-ext`](../plugins/rigor-activesupport-core-ext/)（v0.1.5;トップ約50 AS core_extセレクタ向けのオプトインRBSバンドル）。[`rigor-typescript-utility-types`](../plugins/rigor-typescript-utility-types/)（ADR-13スライス6）。
- **ADR-16基板消費者プラグイン（v0.1.5）**: [`rigor-sinatra`](../plugins/rigor-sinatra/)（Tier A — ブロック-as-メソッド）、[`rigor-dry-struct`](../plugins/rigor-dry-struct/)（Tier C — heredocテンプレート）、[`rigor-devise`](../plugins/rigor-devise/)（Tier B — トレイトインライニングレジストリ）。マクロ展開基板をエンドツーエンドで行使する3つの純粋に宣言的なプラグイン。
- **メタgem（ADR-12 WD1に基づくGemfile-conveniencアンブレラ）**: [`rigor-rails`](../plugins/rigor-rails/)（v0.1.6スキャフォールド — Tier 1+2依存関係を宣言;ユーザーは`.rigor.yml`でアクティブにしたいサブセットを依然として列挙する）。
- **dry-rb基礎ペア（v0.1.6）**: [`rigor-dry-types`](../plugins/rigor-dry-types/) — `:dry_type_aliases`ファクト（15の正準 + 60のネストカテゴリ + ユーザー著作のコンポジション + 推移的参照）;[`rigor-dry-schema`](../plugins/rigor-dry-schema/) — `:dry_schema_table`ファクト（認識 + `each`リストスロット）;[`rigor-dry-validation`](../plugins/rigor-dry-validation/) — `:dry_validation_contracts`ファクト + `Contract#call → Result`のRBSオーバーレイ。`rigor-dry-struct` v0.2.0はADR-18の`returns_from_arg:`経由で`:dry_type_aliases`を消費し呼び出しサイトごとの精度向上を実現。
