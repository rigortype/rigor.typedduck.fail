...
sourceSha: "0a1f7f95d1ddb354540ab8d8de9a381a9438958e87bc5e9ec5a8d4a747612c2b"
...
リアルタイム診断と`annotate`スタイルの型コメントを備えたCodeMirror 6エディターを持つブラウザベースのプレイグラウンド。Fly.io上の薄いRack/Puma APIとCloudflare Pagesの静的フロントエンドでバックアップされる。5つの実装スライス: バックエンド`/check`エンドポイント → フロントエンドエディター + lintマーカー → `/annotate`トグルビュー → `/type-of`ホバー → （需要駆動）3つのゲート条件が成立したときのruby.wasm移行（公式Ruby 4.0 WASMビルド + `prism`/`rbs`のWASMパッケージ + WASM下でのRigorテストスイートのパス）。スライスはスケジュールされていない;プレイグラウンドは0.2.x評価ラインをブロックしない並行トラックである。[ADR-29](../adr/29-browser-playground/)を参照。
