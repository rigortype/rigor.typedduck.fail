...
- `Rigor::Inference::Acceptance`は`Singleton[T]`を`Module`、`Class`、`Object`、`BasicObject`の部分型として扱うようになりました。このルールなしに、パラメーターが`Class | Module`として型付けされているメソッド（例: `Object#is_a?`、`Module#define_method`）はすべてのsingletonレシーバーを拒否し、`lib/`と`spec/`の両方に全体的な偽陽性を生成していました。
