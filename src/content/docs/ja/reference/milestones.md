...
sourceSha: "274454f142eacc8813dcb1e377a9541cf9396b54fb539c1c5efa4e0c271c9600"
...
テーマ: **ADR-10をエンドツーエンドで提供、ADR-11 + Rails Tier 2作業を吸収、ADR-13（プラグインTypeNodeリゾルバ + TSユーティリティ型アダプター）をランディング、ADR-14のsig-genセルフドッグフードギャップをクローズ、ADR-11のコールサイトごとのアサーションゲーティングを完了、ADR-13スライス6統合specがワークアラウンドした上のリテラルトークンフォローアップをクローズする**。v0.1.2がエンジン深化フォローアップスイープを閉じ、4つのプラグイン例をv0.1.1の`flow_contribution_for`基板に移行した;v0.1.3はキューに入れられたADR作業、クロスプラグインAPIに依存するRailsプラグインファミリー、型言語の語彙拡張サーフェス、残存するセルフドッグフードギャップ、パーサー側のリテラルトークンギャップに転換する。
...
8. ✅ **5c（バウンダリクロス診断）** — `Index#mode_for(class_name)` / `#full_mode?(class_name)`がクラスごとのモード認識を公開;`MethodDispatcher.dispatch`は、RBSが非`Dynamic[Top]`キャリアに解決AND `mode: :full`オプトインgemのソースカタログが同じ`(class_name, method_name)`を含むときに、新しい`Environment#boundary_cross_reporter`を介して`dynamic.dependency-source.boundary-cross` `:info`イベントを記録する。RBSがディスパッチで引き続き勝つ。レポーターは`(class_name, method_name, gem_name)`ごとに重複除去。Runnerは実行終了時にドレイン。
...
### Track 5 — ADR-13（プラグインTypeNodeリゾルバ + TSユーティリティ型アダプター）

ADR-13は`Plugin::TypeNodeResolver`拡張ポイント、`Type::Combinator`上の5つのRigor正準シェイプ射影型関数、オプトインの`rigor-typescript-utility-types`例プラグインをランディングする。全8スライスが着地。

1. ✅ **スライス1** — `Rigor::TypeNode::Identifier(name:)` / `Generic(head:, args:)`値オブジェクトAST。ドリフトスナップショットがピン留め。
2. ✅ **スライス2** — `Rigor::Plugin::TypeNodeResolver`基底クラス、`Plugin::Manifest#type_node_resolvers`オプションキーワード、`Plugin::Registry#type_node_resolvers`集約者（登録順;最初の非nilが勝つ、ADR-13 WD3 / WD5に従い）。
3. ✅ **スライス3** — インラインスキャン+解決から「ASTへスキャン」+兄弟の`Resolver`パスへのパーサーリファクター。新しいASTノード`IntegerLiteral` / `IndexedAccess`、スコープコンパニオン`NameScope(resolver:, class_context:, type_alias_table:)` + `ResolverChain`。`ImportedRefinements.parse(payload, name_scope: nil)`がオプションスコープを受け入れる;`nil`（デフォルト）はスライス2の動作をビット単位で保持。
4. ✅ **スライス4** — フェーズA: `HashShape`上の`Type::Combinator.pick_of` / `omit_of` / `partial_of` / `required_of` / `readonly_of`。`PARAMETERISED_TYPE_BUILDERS`が5つの新しいヘッドに成長。`partial_of`は値型をnilに広げない — RigorのHashShapeは「キー不在」を「キー存在で値がnil」と区別する。
5. ✅ **スライス5** — フェーズB: `pick_of` / `omit_of`のTupleサポート（整数インデックス`K`）;`shape_projection_lossy?(type)`述語（非HashShape / 非Tuple入力に対して`true`）;`partial_of` / `required_of` / `readonly_of`はTuple上ではHashShape専用のまま（ネイティブの必須/任意/読み取り専用キャリアなし）。
6. ✅ **スライス6** — `examples/rigor-typescript-utility-types/`プラグインが`Pick` / `Omit` / `Partial` / `Required` / `Readonly`をコア関数にマップ。リゾルバスコープハンドル修正も並んで着地: `NameScope#resolver`は今、完全な`Resolver`（ビルトインレジストリ → チェイン → RBS Nominalフォールバック）を指すため、プラグイン作者は名前解決を再実装することなく`scope.resolver.resolve(arg, scope)`を介してサブ引数を再帰的に解決する。
7. ✅ **スライス3b** — `RbsExtended`（`resolve_directive_rhs` / `parse_*_annotation` / すべての`read_*`リーダー）からの呼び出し側スレッディング + 新しい`Rigor::RbsExtended::Reporter`実行ごとアキュムレータ。`Environment`は`name_scope`（`plugin_registry.type_node_resolvers`から`TypeNode::ResolverChain` + `NameScope`経由で構築）と`rbs_extended_reporter`を公開する。7つの解析器表面の呼び出しサイト（`Analysis::CheckRules` × 2、`Inference::StatementEvaluator` / `Narrowing` / `MethodParameterBinder` / `MethodDispatcher::OverloadSelector` / `MethodDispatcher::RbsDispatch`）が`environment:`を渡す。Runnerは実行終了時にレポーターを`dynamic.rbs-extended.unresolved`（ペイロード全体の解決失敗）と`dynamic.shape.lossy-projection`（パーサーのResolverで射影著作サイトで`shape_projection_lossy?`を参照）`:info`診断にドレインする;深刻度プロファイルは両方を再スタンプする。
8. ✅ **スライス7** — ハンドブックのTypeScript付録更新。[`docs/handbook/appendix-typescript.md`](../handbook/appendix-typescript/)内の`Readonly` / `Partial` / `Required` / `Pick` / `Omit`の5つのマップ型行が、オプトインの`rigor-typescript-utility-types`プラグインとマッチするコア型関数（`readonly_of` / `partial_of` / `required_of` / `pick_of` / `omit_of`）を並べて命名する;移行ビネットがプラグイン提供の`%a{rigor:v1:return: Pick[T, K]}`アノテーションを示す兄弟RBSスニペットを得る。

### Track 7 — rigor-sorbetのコールサイトごとのアサーションゲーティング

最後のADR-11延期項目をクローズ。新しい`Rigor::Scope#source_path`スロットは`Analysis::Runner#analyze_file`内でファイルごとに一度設定され、すべての`with_*`再ビルド + `build_joined_scope`を通じて伝播する。`rigor-sorbet`はカタログハーベスト中にファイルごとのsigilをキャッシュする（`@sigil_by_path`）;`flow_contribution_for`は`assertion_enforced_here?(scope)`を介してアサーション認識器パスをゲートし、`T.let` / `T.cast` / `T.must` / `T.bind` / `T.assert_type!` / `T.reveal_type`はSorbet自身が強制するファイルでのみ発火する。`enforce_sigil: false`は後方互換性のためにゲートをどこでも開く。

### Track 8 — `RBS::Extended` Symbol / Stringリテラルトークン

ADR-13スライス6統合specが合成ASTでワークアラウンドした`docs/CURRENT_WORK.md`エンジニアリング項目をクローズ。3つの新しい`TypeNode` ASTノード（`SymbolLiteral` / `StringLiteral` / `Union`）+ `ImportedRefinements::Parser`が型引数位置で`:name` / `"name"`を認識 + `|`によるリテラルユニオン折り畳み。`pick_of[T, :a | :b]` / `Pick[T, "a" | "b"]`が今、`ImportedRefinements.parse`を通じてエンドツーエンドで往復する;リゾルバはリテラルを`Type::Constant<value>`に変換し、ユニオンを`Type::Combinator.union`を介して変換する。

### Track 6 — ADR-14 sig-genセルフドッグフードギャップクロージャー

3ラウンド目の`lib/rigor/analysis/`セルフドッグフード（`343a475`）が4つの相互作用するギャップ（a） – （e）を表面化した。ギャップ（a） / （b）は`b6d3286`で着地;ギャップ（c） / (d) / （e）はこのトラックでクローズ。

1. ✅ **ギャップ（c） — ネスト型クラス-内-クラス発行**。`Writer#render_new_file`は`group_by(&:class_name)`の代わりに、修飾名セグメントでキーされたネームスペースツリーを構築する（中間祖先が自動的に挿入される）。厳密接頭辞の子クラスは親のブロック内にネストする: `Rigor::Analysis::DependencySourceInference::GemResolver::Resolved`と`GemResolver`自身のメソッドが今、1つの`module GemResolver`ブロックを共有する。
2. ✅ **ギャップ（d） — `&:symbol`ブロックパスディスパッチャー認識**。`Inference::ExpressionTyper#block_return_for`は、呼び出しが`Prism::BlockArgumentNode(SymbolNode)`を運ぶときに、期待されるブロックパラメーター型上でシンボルをディスパッチする。内側のディスパッチが辞退するときは`Dynamic[Top]`にフォールバックするので、外側のオーバーロードセレクターは依然としてブロックを見る。`Hash#transform_values(&:freeze)`は今、ブロック持ちのオーバーロード（`Hash[K, V]`を返す）を経由してルーティングされ、ブロックなしのもの（`Enumerator[V, Hash[K, untyped]]`を返した）ではない。
3. ✅ **ギャップ（e） — `Const = Data.define(...)` / `Struct.new(...)`認識**。`Generator#walk_defs`はRHSが`Data.define` / `Struct.new`である`Prism::ConstantWriteNode`を認識し、完全修飾名を`@class_shells`（新しい`MethodCandidate#class_shells`スロット経由で運ばれる）と`@namespace_kinds`に記録する。ライターは`create_new`の正しいネスト位置に空の`class Foo\nend`ブロックをレンダリングする;`Writer#merge_class_shells`は`update_existing`の最も近い既存の祖先に欠落シェルを注入する。`sig/rigor/analysis/dependency_source_inference/gem_resolver.rbs`内の手作業スタブ`class Unresolvable; end`はもはや必要ない。

### v0.1.3のスコープ外（v0.1.4以降に延期）
- **gemソースからの呼び出しごとの戻り値型精度** — ウォーカーは依然として`(class_name, method_name) → kind`の3つ組のみをカタログ化する。gemソースからメソッドごとの戻り値型を推論する（`mode: :full`が`Dynamic[Top]`より豊富に貢献できるように）のは、具体的なユーザー需要が表面化するまで延期される、より大きなウォーカー拡張。
