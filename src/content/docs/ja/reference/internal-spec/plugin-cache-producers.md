...
Rigorの永続キャッシュ（ADR-6、v0.0.8/v0.0.9）は`(producer_id, params, descriptor)`をキーとするバイナリエントリの単一シャードディレクトリです。スライス6はその契約（contract）のプロデューサー側をプラグイン作成者に拡張することで、計算コストが高いプラグイン貢献（スキーマの解析、動的メンバーテーブルの構築、生成メタデータのインデックス作成）が`rigor check`の実行をまたいでキャッシュされ、入力が変更されたときに正しく無効化されるようにします。
