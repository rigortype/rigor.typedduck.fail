...
2. **`rigor-sorbet`（スライス（slice）1〜8、機能完備）はすでにMangroveの
   ...
   `sorbet/rbi/{gems,dsl}/`にマテリアライズされる——これを`rigor-sorbet`のスライス4
...
- **プロセス** — `rigor-plugin-author`スキルで作成する（Phase 0/0.5の
  メンテナルーティング → 要件 → テンプレート → スキャフォールド → ウォーカー → 統合
  仕様）。非メンテナが主導する場合は[ADR-31](../../adr/31-contribution-and-supply-chain-policy/)の
  サードパーティパスが適用される。
...
| # | スライス | 契約適合 | 判定 |
| --- | --- | --- | --- |
| ① | `unwrap_in(ctx)` / `collecting`の早期リターン | **今すぐ構築可能**。手書きのウォーカー: `flow_contribution_for`が`:unwrap_in`呼び出しを認識し、ハッピーパスではレシーバの`Ok`型を、`Err`では例外的エッジを返す——`rigor-sorbet`の`T.absurd`（スライス6）/ `T.must`の認識器と構造的に同一。 | **v0スライスとして出荷**。 |
...
（①単体で出荷するか、②/③のエンジン／ADR作業を始めるか）はメンテナに差し戻す。
...
プローブ（現実的な形状——戻り値がユーザー定義のジェネリックであるプロデューサ）:
...
`unwrap!`のレシーバはキャリアNominalを持たず、rigor-mangroveは何もせず、タイポは
黙って出荷されてしまう。
...
`T::`名前空間ではないので、`translate_call`は`degraded` → `untyped`へ落ちる。レシーバの型は
rigor-mangroveが見る前に失われている。

**帰結——動くチェーンと動かないチェーン:**

| キャリア + プロデューサ戻り値の型ソース | unwrap時のレシーバ | rigor-mangrove |
| --- | --- | --- |
...
`signature_paths:`で出荷する——は、消費者のプロデューサメソッドも*また*RBSで型付け
...
②/③は依然として未解決の項目であり、その順序であるが、今やレシーバはunwrap箇所で
実際にその型を運ぶようになった。
...
RBS/sigの戻り値が`Ok[String, Integer] | Err[String, Integer]`であるプロデューサから得られる
...
（RBSの`class Child[..] < Parent[..]`宣言に従い、親の型引数を子のパラメーターにマップする）——
...
（すでにユニオンで型付けされたレシーバの上だけでなく）発火させられるものでもある——だが
