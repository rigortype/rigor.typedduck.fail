...
description: "rigortype/rigor docs/notes/20260518-matsumoto-2010-cfa-rigor-review.mdの翻訳です。"
...
| **実装: OCaml + BDDBDDB (Datalog)** | Rigor: 手書きRuby推論エンジン + ファイルベースキャッシュ（[ADR-4](../../adr/4-type-inference-engine/), [ADR-6](../../adr/6-cache-persistence-backend/)） | 論文のDatalog化はメンテ性で美しいが、（a） RBSとの結合、（b） pluginの任意Rubyロジック、（c） Ractor並列化（[ADR-15](../../adr/15-ractor-concurrency/)）といったRigorの制約とは噛み合わない。設計トレードオフの差。 |
...
- [ADR-5: ロバストネス原則](../../adr/5-robustness-principle/)
- [ADR-8: Steepにインスパイアされた改善](../../adr/8-steep-inspired-improvements/)
- [ADR-15: Ractor並行性](../../adr/15-ractor-concurrency/)
- [ADR-16: マクロ / DSL展開基板](../../adr/16-macro-expansion/)
- [ADR-17: monkey-patch事前評価](../../adr/17-monkey-patch-pre-evaluation/)
- [Control Flow Analysis仕様](../../type-specification/control-flow-analysis/)

## 姉妹ノート

- [Matsumoto & Minamide 2008（多相レコード型）Rigor観点考察](../20260518-matsumoto-2008-poly-records-rigor-review/)
