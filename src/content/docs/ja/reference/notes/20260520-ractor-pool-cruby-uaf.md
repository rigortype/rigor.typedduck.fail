...
4. **TSANが盲目だった.** Ruby 4.0.5のThreadSanitizerビルドはプロセスがSIGSEGVする前に**ゼロ**のレースを報告した。予想通り:TSANはRuby 4.0のM:Nスレッドスケジューラーを追跡できないため、happens-beforeトラッキングがRactorレースを見落とす。
...
- TSANはここでは使えるツールではない — Rubyのm:nスケジューラーがスレッドトラッキングを破壊する。ASANを使う;arm64コルーチンの誤検知を回避するためにuse-after-scope計装を無効化する。
