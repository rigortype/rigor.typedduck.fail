...
リファインメント（refinement、篩型とも）はRBSとの相互運用のためにベースの名前的クラスに**消去**されます。シグネチャが`-> String`と言っているメソッドはそのコントラクトを保ちます — Rigorは自身の解析の中でのみ、より厳密なビューを加えます。
...
`Dynamic[T]`（Top以外の内部）はより具体的な漸進的（gradual）形式です: 「この値に対する静的コントラクトはないが、静的ファセットは`T`のように振る舞う」。RBSで宣言された`untyped`境界が、Rigorがすでに何かを知っているクラスと出会うときに現れます。
