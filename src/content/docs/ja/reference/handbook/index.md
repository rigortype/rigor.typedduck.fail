...
- [**mypy / Pyrightから来た場合**](appendix-mypy/) —
  漸進的型付け（gradual typing）の類似点、`Literal` ↔ `Constant`、`TypeGuard`/`TypeIs` ↔ `predicate-if-true`/`predicate-if-false`、`Protocol` ↔ RBSの`interface`、`LiteralString` ↔ `literal-string`。
