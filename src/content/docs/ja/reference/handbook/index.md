...
sourceSha: "57eee1ba398b2f05082e3a244d2021141726e13ef17774b5d362ef8684e55938"
...
10. [**Sorbetとの共存**](10-sorbet/) — Sorbetを使用しているプロジェクトから来たユーザー向け: [`rigor-sorbet`](https://github.com/rigortype/rigor/blob/main/plugins/rigor-sorbet/)アダプターが`sig { ... }`ブロック、RBIファイル、`T.let` / `T.cast` / `T.must` / `T.unsafe`アサーションをRBSで書き直さずに型ソースとして読み取ります。
...
日常使用のためのRigorのインストール方法と、継続的インテグレーションへの組み込み方法。Rigorはスタンドアロンのツールでありプロジェクトの依存関係ではない;これらのページは各チャネルを完全にカバーする（[第1章](01-getting-started/)では見出しのみ）。

- [**Rigorのインストール**](appendix-installation/) — `mise`（推奨）、`asdf`、`gem install`、Nix、dev-containerのガイダンス。
- [**CIでのRigor実行**](appendix-ci/) — 専用ジョブでのRigor実行、最小限のGitHub Actionsワークフロー、CI専用`Gemfile`とDependabotによるRigorバージョンのピン留め。
