...
sourceSha: "0de43c466b73640c1110528fb20b6e4f092f18ee8c566ea86a4f9f08adf57cd4"
...
| `Readonly<T>` | （対応なし） | Rigorは現時点でミュータビリティを追跡しない。 |
| `Partial<T>` / `Required<T>` | （なし） | Rigorの`HashShape`はキーごとにrequired/optionalを持つが、`Partial`演算子はない。 |
| `Pick<T, K>` / `Omit<T, K>` | （なし） | マップ型にRigorの対応物はない。プラグインがそのニッチを埋める。 |
...
- **マップ型。**`Pick`、`Omit`、`Partial`、`Readonly`、テンプレートリテラル操作。これらのどれもコアのRigorにはない。
...
`Pick<T, K>`の正確なキーセット追跡は失われる — RBSにはそれがない。「返されるすべてのキーが`keys`にあり`obj`に存在していた」というランタイム安全性は保たれる。正確なキーセットが重要な呼び出し — 典型的にはcall-siteのHashリテラル — では、Rigorの`HashShape`がキーごとの型を保持するため、シグネチャがジェネリックでもcall-siteは精密なまま。
