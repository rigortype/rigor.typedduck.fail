...
| `Readonly<T>` | `readonly_of[T]`（オプトインの[`rigor-typescript-utility-types`](https://github.com/rigortype/rigor/blob/main/plugins/rigor-typescript-utility-types/)プラグイン経由） | `HashShape`の各エントリーに対するビュー層の読み取り専用マーカー。基底オブジェクトが凍結されていることを証明する**ものではない** — ADR-13 §「Readonly」。 |
...
| `Pick<T, K>` / `Omit<T, K>` / `Partial<T>` / `Required<T>` / `Readonly<T>` | オプトインの[`rigor-typescript-utility-types`](https://github.com/rigortype/rigor/blob/main/plugins/rigor-typescript-utility-types/)プラグインが、`HashShape`上の`pick_of` / `omit_of` / `partial_of` / `required_of` / `readonly_of`（および`Tuple`上の`pick_of` / `omit_of`）に各々をマップする。 |
| 条件型 | （対応なし — プラグインが必要） |
...
RBSシグはジェネリックなままになる。`Pick<T, K>`の正確なキーセット追跡を取り戻したい場合は、[`rigor-typescript-utility-types`](https://github.com/rigortype/rigor/blob/main/plugins/rigor-typescript-utility-types/)プラグインをオプトインし、戻り値型を`Pick`の綴りで注釈する:
