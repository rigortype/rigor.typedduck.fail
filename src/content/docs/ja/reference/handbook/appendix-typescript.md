...
sourceSha: "e198d61aed4c8f8949d85f3c4759d1164d89f70a9129bf72476ae0ba265791a8"
...
ふたつのシステムは目標を共有する — プログラムを実行する前にバグを見つける — が、そこへの道筋で意見が分かれる。TypeScriptはサウンドネス優先のオーサリングを好む（すべての値が検査された型を持ち、そうなるまでチェッカーが文句を言う）。Rigorはfalse-positiveなし推論を好む（証明できないものには沈黙し、推論が見通せない箇所でだけ`.rbs`を求める）。
...
| `Readonly<T>` | `readonly_of[T]`（オプトインの[`rigor-typescript-utility-types`](https://github.com/rigortype/rigor/blob/main/examples/rigor-typescript-utility-types/)プラグイン経由） | `HashShape`の各エントリーに対するビュー層の読み取り専用マーカー。基底オブジェクトが凍結されていることを証明する**ものではない** — ADR-13 §「Readonly」。 |
...
TypeScriptのフロー感応型ナローイングはRigorに直接対応するものがある。語彙は異なるが、動作は同じ。
...
両チェッカーとも同じ正確なunionを推論する。TypeScriptバージョンはパラメーター型と戻り型のオーサードアノテーションが必要だが、Rigorバージョンはどちらも不要。

sigを書く必要があるとき — モジュール境界で、ボディが動的すぎるとき、パラメーターシェイプを強制したいとき — それは`.rb`ソースではなく`sig/<file>.rbs`に書く。この分離は意図的なもの（ADR-1とADR-5を参照）。

## ジェネリクス

TypeScriptのジェネリクスは標準ライブラリの中心。Rigorのジェネリクスはより保守的なRBSのもの。RBSはクラスレベルの型パラメーターとバウンド付き制約のメソッドレベル型パラメーターをサポートするが、TypeScriptほど定型的なcall-site推論インスタンス化はまだサポートしていない。
...
| `Pick<T, K>` / `Omit<T, K>` / `Partial<T>` / `Required<T>` / `Readonly<T>` | オプトインの[`rigor-typescript-utility-types`](https://github.com/rigortype/rigor/blob/main/examples/rigor-typescript-utility-types/)プラグインが、`HashShape`上の`pick_of` / `omit_of` / `partial_of` / `required_of` / `readonly_of`（および`Tuple`上の`pick_of` / `omit_of`）に各々をマップする。 |
| 条件型 | （対応なし — プラグインが必要） |

RigorはRBSジェネリクスをディスパッチャー経由で読み込み、レシーバーが十分な情報を持つcall-siteでパラメーターをインスタンス化する。表示はRBSと同じ — `Array[Integer]`は`Array[Integer]`と表示される。
...
RBSシグはジェネリックなままになる。`Pick<T, K>`の正確なキーセット追跡を取り戻したい場合は、[`rigor-typescript-utility-types`](https://github.com/rigortype/rigor/blob/main/examples/rigor-typescript-utility-types/)プラグインをオプトインし、戻り値型を`Pick`の綴りで注釈する:
...
他のツールと比較したい場合は、兄弟付録ページが[PHPStan](../appendix-phpstan/)、[mypy](../appendix-mypy/)、[Steep](../appendix-steep/)をカバーしている。
