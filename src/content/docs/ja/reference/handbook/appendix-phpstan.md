...
- **Rubyの述語メソッドに対するファーストクラスのフローセンシティブなナローイング**。`s.empty?`/`n.zero?`/`n.positive?`等は名前で認識され、それに応じてナローイングされる。PHPStanはType-Specifying Extensionsで同じアイデアを持つが、Rigorはカタログを標準で提供する。
