...
[`docs/internal-spec/plugin.md`](../../internal-spec/plugin/)に固定されたプラグインコントラクトは、PHPStanの拡張APIがカバーするすべてのシェイプを与える。同様のライフサイクル（マニフェスト宣言、呼び出しごとのディスパッチ、ファクト（fact）発行）を持つ。第9章には高レベルの方向性がある。内部仕様が拘束力のあるコントラクト。
