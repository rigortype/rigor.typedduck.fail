...
この章では、Rigorがメソッド呼び出しについて知っていること — レシーバーの型、引数の型、推論された戻り値型、ブロックが付属している場合のブロックパラメータ — を扱います。
...
`call.argument-type-mismatch`は、Rigorが引数がパラメータ契約を満たせないことを**証明**できる場合にのみ発火します。「空の可能性がある」は沈黙します — 偽陽性なしルール。
...
`;`を接頭辞にした名前は、同名の外側ローカル変数をシャドウする新しいブロックローカル変数を導入します。Rigorはブロックエントリー時にこれらのローカル変数を`Constant<nil>`にバインドします — Rubyのランタイムセマンティクス — そしてブロック内の書き込みをそのブロックにローカルなものとして扱います:
