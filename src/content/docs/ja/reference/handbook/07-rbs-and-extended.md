...
Rigorの推論が型を証明できないとき、次の逃げ道はRBS — Rubyのシグネチャ言語 — です。RBSが求める精密な契約を表現できないとき、`RBS::Extended`がその上に小さなアノテーション表面を追加します。
...
- 引数シェイプエラーに対して`call.argument-type-mismatch`を発火させたい（インソース`def`はパラメーター契約を強制しません; RBS宣言メソッドのみが強制します）。
