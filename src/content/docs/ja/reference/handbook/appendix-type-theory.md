...
sourceSha: "8bb4baa11721198e8ffdf98667032c7726a38bbffdd33f9dbffb36616e2e35a1"
...
> [型の束](#型の束) ·
> [束の集合論的基礎](#束の集合論的基礎) ·
> [サブタイピングと漸進的一貫性](#サブタイピングと漸進的一貫性) ·
> [名前的vs構造的型付け](#名前的vs構造的型付け) ·
> [多相性](#多相性) ·
> [F有界多相とself型](#f有界多相とselftype) ·
> [オブジェクト形状 / 行多相](#オブジェクト形状--行多相hackそしてhashshapeの系譜) ·
> [分散](#分散) ·
> [リファインメント型と述語サブタイピング](#リファインメント型と述語サブタイピング) ·
> [Occurrence typing](#occurrence-typingフローセンシティブなナローイング) ·
> [パターンマッチと網羅性](#パターンマッチと網羅性) ·
> [漸進的型付け](#漸進的型付け) ·
> [ブレームと信頼境界](#ブレーム漸進的保証そして信頼境界) ·
> [エフェクトシステム](#エフェクトシステム) ·
> [健全性vs完全性](#健全性完全性そして偽陽性なしの姿勢) ·
> [推論の決定可能性](#推論の決定可能性) ·
> [Hindley–Milnerと主型](#hindleymilner主型そしてrigorの推論アーキテクチャ) ·
> [双方向型チェック](#双方向型チェック) ·
> [純粋な推論を超えて](#純粋な推論を超えて-リーチと精度) ·
> [式問題](#式問題とrigorのプラグイン契約) ·
> [より小さな接続](#より小さな接続概略) ·
> [Rigorがモデル化しないこと](#rigorがモデル化しないこと) ·
> [読書リスト](#短い読書リスト)
