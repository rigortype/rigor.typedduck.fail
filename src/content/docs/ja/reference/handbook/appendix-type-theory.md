...
sourceSha: "82f110f0ee826493adeb82b63bf6d7c0b2bd40b18cba735f7d8e0768c7e73856"
...
**リファインメント型**は基底型を述語で制限する: Liquid Types / SMT駆動システムではこれは`{x: Int | x > 0}`と書かれる。Rigorは予約名を持つリファインメントの厳選されたカタログを露出する:
...
| キャスト | ソース内キャスト演算子はない。オプトインの[`rigor-sorbet`](https://github.com/rigortype/rigor/blob/main/examples/rigor-sorbet/)プラグインは`T.let` / `T.cast` / `T.must`をキャスト形式として読む;`RBS::Extended`の`assert_type`ディレクティブは`.rbs`から同じ役割を果たす。 |
