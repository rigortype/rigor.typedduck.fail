...
漸進的型付け（gradual typing）は第2の関係を加える: **一貫性（`~`）**。`Dynamic[T] ~ U`は「実行時の値が`U`を満たすかを静的には知らないが、満たすことは許容される」を意味する。一貫性は反射的かつ対称的だが**推移的ではない**——これが漸進的型付けを「単に束に`Any`型を加える」から区別する鍵となる技術的な動きである。
