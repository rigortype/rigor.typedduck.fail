...
- **ライブラリ: `language_server-protocol` gem（薄い）**。JSON-RPC
  ...
  るのではなく、自身のディスパッチャ、ライフサイクル、メッセージ
  ルーティングを所有する。
...
| `initialized` | C→S | no-op | オプションの`workspace/didChangeWatchedFiles`登録をトリガする。 |
| `shutdown` | C→S | ランナー解放、ワーカードレイン | サーバーは`exit`まで生存する。 |
| `exit` | C→S | `exit 0` | プロセス終了。 |
| `textDocument/didOpen` | C→S | 仮想ファイルテーブル`{uri => bytes}` | 診断発行をトリガする。 |
| `textDocument/didChange` | C→S | 仮想テーブルを変異 | デバウンスされた診断発行をトリガする。 |
| `textDocument/didSave` | C→S | v1ではno-op | 診断は`didChange`によりすでに新鮮。 |
| `textDocument/didClose` | C→S | 仮想テーブルからエントリ削除 | URIに対し空の診断を発行してインラインマーカーをクリアする。 |
...
- `didChange`は`bytes`を変異させ`version`を増やす。診断発行が完了する
  まで`dirty: true`。
- `didClose`はエントリを削除する。URIの診断は空の発行でクリアされる。

診断実行が発火すると、サーバーはダーティなエントリごとに`BufferBinding`
を1つ実体化する。
...
なぜインメモリの`{path => bytes}`パーサーオーバーライドではなくテンポ
...
位で再利用できる。新たなパーサーエントリポイントなし、維持すべき第二
のコードパスなし。テンポラリファイルは`Dir.tmpdir`配下に置かれ、バッ
ファエントリが削除されたときにunlinkされる。
...
プラグインソースファミリ（`:builtin`／`"plugin.activerecord"`／…）を
...
- リクエストディスパッチャ。我々がメソッドシンボル → ハンドラのハッ
  シュを所有する。
...
2. **`Rigor::LanguageServer::Server`ライフサイクル**。状態機械、stdio
   上のJSON-RPCディスパッチャ、ケイパビリティネゴシエーション。フレー
