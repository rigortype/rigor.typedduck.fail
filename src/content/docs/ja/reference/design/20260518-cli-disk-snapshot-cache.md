...
2. **Marshalバージョン安定性**。`Cache::Store`はすでに`SCHEMA_VERSION`でキーするので、Rubyバージョンバンプはエントリーを無効化する。`MarshalableProjectScan`スナップショットはこの不変条件を継承する。
...
- `CHANGELOG.md`に`[Unreleased]` § Performance下のエントリーを追加。
- コミットでこの設計ノートを参照。
