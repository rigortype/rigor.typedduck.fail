...
ジェネリック位置は動的由来のスロットを保持します。例えば`Array[untyped]`は内部的に`Array[Dynamic[top]]`であり、**`Array[top]`ではありません**。要素を読み取ると`Dynamic[top]`が返ります。要素の書き込みは漸進的一貫性に従い、ストリクトモードはコレクションがチェックされていない値を保持することを報告できます（MAY）。同じルールがハッシュ、タプル、レコード、procのパラメーターと戻り値、シェイプ（shape）メンバーに適用されます。
