...
- **名前的クラスよりもケイパビリティロール**。 メソッドが引数に対して`:write`のみを使う場合、パラメーター型は`IO`ではなく`_Writable`（構造的ロール）であるべきです（SHOULD）。これにより`StringIO`、`Tempfile`、カスタムモックオブジェクト、および将来のすべてのクラスがコントラクトを満たせるようになります。
