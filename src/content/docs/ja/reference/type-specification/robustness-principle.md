...
| `Nominal[Class]` | 生の名前的型 | `Object#dup` → `self`（依然として名前的型） |
| `Dynamic[T]` | 最終手段の漸進的フォールバック | より具体的なものを証明できないとき |
...
- **サブタイピング対漸進的一貫性（[relations-and-certainty.md](../relations-and-certainty/)）**: 第2句の広げ方はサブタイピングの動き（より寛容なパラメーターはより厳格なものの上位型）です; `Dynamic[T]`を`top`に折り畳みません。
