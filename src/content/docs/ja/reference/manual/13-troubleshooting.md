...
Rigorはインストールされているが`PATH`上にありません。バージョンマネージャを使用している場合、通常はシェルが有効化されていないことを意味します——[Rigorのインストール](../01-installation/)を参照してください。応急処置として`mise exec gem:rigortype -- rigor …`で明示的に実行できます。
...
3. `rigor baseline generate` — 残りをスナップショットしてCIが新しい診断のみを追跡するようにする。[ベースライン](../06-baseline/)を参照。
