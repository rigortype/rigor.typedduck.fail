...
インラインの診断とNeovim、VS Code、Helix、Emacsでのホバーを得るには`rigor lsp`を使います。AIエージェントがリファクター提案前に型チェックし、カーソル位置の型を調べ、コードレビューセッションのコンテキストとしてプロジェクトの診断セットをトリアージできるようにするには`rigor mcp`を使います。
...
| `rigor_explain` | `rigor explain --format json` | JSONルールカタログエントリ |
...
`~/Library/Application Support/Claude/claude_desktop_config.json`（macOS）または`%APPDATA%\Claude\claude_desktop_config.json`（Windows）にエントリを追加します:
...
CursorのComposerはRigorツールを組み込み機能と並べて提供します。`rigor_check`はComposerリファクターの前に特に有用です——まず実行して現在の診断ベースラインを把握し、その後で比較します。
...
プロジェクトの診断ストリームを要約します: ルール分布、ファイルごとのホットスポット、最も一般的なエラークラスターに対するヒューリスティックヒント。
...
**典型的なエージェントの使い方:** `rigor_sig_gen`を呼び出してどのメソッドが`sig/`に書く価値のある精確な戻り値を持つかを確認し、候補を生成して挿入する。エージェントはシグネチャを自動的に書く**べきではありません** — 候補を人間のレビューのために提示し、確認済みエントリのみを`rigor sig-gen --write`で適用する。
...
- **HTTPトランスポート**（スライス2） — CI / リモートエージェント向けの`--transport=http`;最小限のRackエンドポイント。
