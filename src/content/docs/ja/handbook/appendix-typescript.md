...
| `Readonly<T>` | `readonly_of[T]`（オプトインの[`rigor-typescript-utility-types`](../../manual/plugins/rigor-typescript-utility-types/)プラグイン経由） | `HashShape`の各エントリーに対するビュー層の読み取り専用マーカー。基底オブジェクトが凍結されていることを証明する**ものではない**。ADR-13 §「Readonly」。 |
...
| 条件型`T extends U ? A : B` | （コアにはない。プラグインの提供で） | プラグインは引数のシェイプによって戻り値型を変えられる。 |
...
| タグ付きユニオン（判別共用体）`switch (x.kind)` | `case x; in {kind: :foo}`または`case x.kind; when :foo` |
...
TypeScriptは「長さ≥1の文字列」をテンプレートリテラル型かブランド型でしか表現できず、どちらも合成しにくい。Rigorにはファーストクラスのリファインメントキャリアがある。証明可能な非空文字列、証明可能な正の整数、証明可能な非空配列だ。
...
両チェッカーとも同じ正確なunionを推論する。TypeScriptバージョンはパラメータ型と戻り値型のオーサードアノテーションが必要だが、Rigorバージョンはどちらも不要。
...
| `Pick<T, K>` / `Omit<T, K>` / `Partial<T>` / `Required<T>` / `Readonly<T>` | オプトインの[`rigor-typescript-utility-types`](../../manual/plugins/rigor-typescript-utility-types/)プラグインが、`HashShape`上の`pick_of` / `omit_of` / `partial_of` / `required_of` / `readonly_of`（および`Tuple`上の`pick_of` / `omit_of`）に各々をマップする。 |
| 条件型 | （対応なし。プラグインが必要） |
...
- **ソース内メソッドボディからの推論された戻り値型**。`tsc`は関数ボディから戻り値型を推論し、呼び出し元に公開する。Rigorはソース内`def`に対して同じことをするが、RBS宣言されたメソッドは宣言された戻りに呼び出し元をバインドする。これは意図的な境界<ruby>規律<rp>（</rp><rt>discipline</rt><rp>）</rp></ruby>の選択だ（ADR-5、ロバストネス原則を参照）。
- **エディタIntelliSenseの同等性**。TypeScriptのツールには20年の投資がある。Rigorのエディタ統合はより若い。`rigor lsp`はインプロセスの言語サーバー（language server）を提供する（診断、ホバーによる型表示、アウトライン、型を考慮した補完。マニュアルのエディタ統合の章を参照）が、そのツールの充実度はいまだにTypeScriptに及ばない。
...
RBSシグはジェネリックなままになる。`Pick<T, K>`の正確なキーセット追跡を取り戻したい場合は、[`rigor-typescript-utility-types`](../../manual/plugins/rigor-typescript-utility-types/)プラグインをオプトインし、戻り値型を`Pick`の綴りで注釈する:
...
プラグインの`TypeNodeResolver`が`Pick[T, K]`を正準の`pick_of[T, K]`射影に変換する。どちらの場合でも、本当に重要なところでは呼び出しサイトは精密なままだ。呼び出しサイトのHashリテラルはシグネチャによらず`HashShape`であり、キーごとの型は`obj.key?(k)`のナローイングを通じて保たれる。
