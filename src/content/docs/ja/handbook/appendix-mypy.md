...
- **引数シェイプによる戻り値型変化のプラグインサイド**。Pyrightの「型エイリアスナローイング」とmypyのオーバーロードスタックがいくつかのケースをカバーする。Rigorのプラグイン契約（contract）はディスパッチポイントで完全なRubyコードを提供する。[`rigor-lisp-eval`](https://github.com/rigortype/rigor/tree/master/examples/rigor-lisp-eval/)の例が標準デモ — `Lisp.eval([:+, 1, 2])`は`Integer`を返し、`Lisp.eval([:<, 1, 2])`は`bool`を返す。
