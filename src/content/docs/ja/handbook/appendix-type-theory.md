...
- **静的 / 動的境界でのランタイム契約挿入**。オプトインの[`rigor-sorbet`](../../plugins/rigor-sorbet/)プラグインはSorbetの`T.let` / `T.cast` / `T.must`をキャスト形式として読むが、契約の*強制*は`sorbet-runtime`の仕事であり、Rigorのではない。Rigorの静的解析はキャストをヒントとして使うのであり、チェックとしてではない。
