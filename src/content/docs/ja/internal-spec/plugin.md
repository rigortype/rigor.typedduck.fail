...
- `dynamic_return(receivers:, methods:, file_methods:) { |call_node, scope| Type | nil }` — レシーバーのクラス、メソッド名、またはその両方でゲートされた、呼び出しサイトごとの**戻り値型**（少なくとも1つのゲートがREQUIRED ── どちらでもゲートしない規則はすべてのディスパッチで発火してしまうため、`dynamic_return`はロード時にそれを拒否します）。`receivers:`（クラス名の空でない`Array`、または`#prepare`の後に実行ごとに一度解決される`-> { … }`のcallable、ADR-52スライス3）を指定すると、エンジンは呼び出しのレシーバー型のクラスが宣言されたエントリーと等しいか、それを継承する場合にのみブロックを呼びます（`Environment#class_ordering`経由でマッチ）。`methods:`（Symbol／String名の`Array`、または実行時callable、ADR-52スライス4）は`call_node.name`でゲートします。`file_methods:`（パスを受け取り`(rule, path)`ごとにメモ化されるcallable、ADR-52スライス5a）は、解析対象ファイルによって変わる名前集合（rigor-rspecの`let`名）向けのファイルごとの特殊化であり、`methods:`を置き換えます。最初の非`nil`が勝ちます。エンジンはそれを`#dynamic_return_type(call_node:, scope:, receiver_type:)`を通して呼び出します。`rigor-mangrove`（アンラップ → 担われた`type_args[0]`）が実装済みのコンシューマーです。
  ...
  - **`dynamic_return`の回答はその呼び出しサイトでの`call.undefined-method`を抑制する**（issue #653）。このティアは`MethodDispatcher#resolve`において`RbsDispatch`の上に位置するため、プラグインが回答したとき、レシーバーのRBSはその呼び出しを決してディスパッチせず、サイトでの型はプラグインのものになります;存在チェックはその呼び出しが未定義であることを証明するために同じRBSを読んではなりません（MUST NOT）。ディスパッチャーは型付けパスの間に回答された各呼び出しノードを`Scope#plugin_typed_calls`に記録し、`Analysis::CheckRules`はその記録を参照します ── 診断を決定するためにプラグインブロックを再実行することは決してありません。この抑制は**レシーバークラスごとではなく、呼び出しサイトごと**です: 下記の`open_receivers:`免除よりも狭く、RBSもプラグインも回答しない名前は同じレシーバー上で引き続き報告されます。
    ...
    - **抑制の健全性はプラグイン自身のレシーバーゲートと同程度でしかない**。`receivers:`はクラス名にマッチし、`Singleton[C]`と`Nominal[C]`を区別しないため、通常のインスタンスメソッドルール（`receivers: ["Widget"], methods: [:price]`）も`Widget.price`に答え、この記録はプラグインの言に従って本物の`undefined method 'price' for singleton(Widget)`を沈黙させてしまいます。このレシーバー種別のギャップは以前から存在し個別に追跡されています;現在バンドルされたプラグインでこれに晒されているものはありません（`Nominal`ガードを持たない3つの`receivers:`ルールはactionpackのものであり、そのクラスはRBSを出荷していません）。
