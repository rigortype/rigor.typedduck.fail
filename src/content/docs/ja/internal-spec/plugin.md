...
- `dynamic_return(receivers:, methods:, file_methods:) { |call_node, scope| Type | nil }` — レシーバーのクラス、メソッド名、またはその両方でゲートされた、呼び出しサイトごとの**戻り値型**（少なくとも1つのゲートがREQUIRED ── どちらでもゲートしない規則はすべてのディスパッチで発火してしまうため、`dynamic_return`はロード時にそれを拒否します）。`receivers:`（クラス名の空でない`Array`、または`#prepare`の後に実行ごとに一度解決される`-> { … }`のcallable、ADR-52スライス3）を指定すると、エンジンは呼び出しのレシーバー型のクラスが宣言されたエントリーと等しいか、それを継承する場合にのみ（`Environment#class_ordering`経由でマッチ）**かつ**レシーバーがそのエントリーが名指す種別（kind）である場合にのみブロックを呼びます。`methods:`（Symbol／String名の`Array`、または実行時callable、ADR-52スライス4）は`call_node.name`でゲートします。`file_methods:`（パスを受け取り`(rule, path)`ごとにメモ化されるcallable、ADR-52スライス5a）は、解析対象ファイルによって変わる名前集合（rigor-rspecの`let`名）向けのファイルごとの特殊化であり、`methods:`を置き換えます。最初の非`nil`が勝ちます。エンジンはそれを`#dynamic_return_type(call_node:, scope:, receiver_type:)`を通して呼び出します。`rigor-mangrove`（アンラップ → 担われた`type_args[0]`）が実装済みのコンシューマーです。
  ...
  - **`dynamic_return`の回答はその呼び出しサイトでの`call.undefined-method`を抑制する**（issue #653）。このティアは`MethodDispatcher#resolve`において`RbsDispatch`の上に位置するため、プラグインが回答したとき、レシーバーのRBSはその呼び出しを決してディスパッチせず、サイトでの型はプラグインのものになります;存在チェックはその呼び出しが未定義であることを証明するために同じRBSを読んではなりません（MUST NOT）。ディスパッチャーは型付けパスの間に回答された各呼び出しノードを`Scope#plugin_typed_calls`に記録し、`Analysis::CheckRules`はその記録を参照します ── 診断を決定するためにプラグインブロックを再実行することは決してありません。この抑制は**レシーバークラスごとではなく、呼び出しサイトごと**です: 下記の`open_receivers:`免除よりも狭く、RBSもプラグインも回答しない名前は同じレシーバー上で引き続き報告されます。
    - **このルールはどちらのサブシステムが他方より優位であるかに依拠しない**。レシーバーのRBSが宣言するメソッドはルール自身の`lookup_method`を通じて解決され診断に決して到達しないため、`call.undefined-method`は記録が「プラグインの回答が完全に勝つ」と読まれようと「RBSが沈黙している場合にプラグインの回答が勝つ」と読まれようとまったく同一に発火します。すべてのプラグイン回答について記録が参照されるのは、それがより安価で誠実な形であるからにすぎず、本節が優先順位を確定させているからではありません。
    - **未解決の不一致 ── エンジンと[ADR-2](../../adr/2-extension-api/) §「プラグイン貢献のマージ」は食い違っており、本節はそれを解決しない**。ADRはプラグインを受け入れられたRBSよりも低い権限ティアに置きます:「下位の階層は上位の階層を弱めたり矛盾させたりしてはならない。上位の階層と矛盾する下位の階層の寄与は、サイレントなオーバーライドではなく診断である」、具体的には「動的戻り値拡張からの戻り値型は、選択されたシグネチャに対してチェックされる。プラグインは契約の範囲内で狭めることができる;互換性のない戻り値はシグネチャ違反である」。エンジンはこれをチェックしません: `MethodDispatcher#resolve`はプラグインティアを`RbsDispatch`の上に置き、単にプラグインの型を採用します。実際にはバンドルされた2つのプラグインがその不一致に依存しています: `rigor-activesupport-core-ext`はstdlibのRBSが`Float`を宣言している場所で`Time#-`をオーバーライドして`Nominal["ActiveSupport::Duration"]`を返します（演算子は`Numeric`上で型付けされており、オペランドを知ることができません（`Time.now - 30.minutes`は`Float`を射影する））。一方で`rigor-dry-validation`は自身のバンドルされた`sig/`が宣言する`to_h`をリファインしており、これはADR-2が許可するナローイングです。どちらの文書が譲るかは判定のために提起された決定事項であり、ここにあるものは何もADR-2を置き換えるものではありません。2つの文書が沈黙のうちに矛盾し合うのを防ぐために記録されています。
    - **抑制の健全性はプラグイン自身のレシーバーゲートと同程度でしかない**。そのため、そのゲートはレシーバーの種別（KIND）を保持します（上記issue #701）。`receivers:`エントリーがクラス名単独にマッチしていた間は、通常のインスタンスメソッドルール（`receivers: ["Widget"], methods: [:price]`）も`Widget.price`に答え、この記録はプラグインがすでに型付けを誤っていた呼び出しに対して、プラグインの言に従って本物の`undefined method 'price' for singleton(Widget)`を沈黙させてしまっていました。インスタンスエントリーはそこではもはや回答しないため、クラスレベルのミスは`RbsDispatch`に到達して報告されます。`"singleton(Widget)"`を宣言するルールはそれを抑制し、それこそが肝要な点です: プラグインはそのとき、クラスレベルの呼び出しをモデル化していると明示的に言明したことになります。
    - 解決されたシグネチャを読む2つのルール（`call.wrong-arity`、`call.argument-type-mismatch`）は、今日この記録によって**カバーされていません**: プラグインが回答したサイトにおいて、それらは依然としてその名前に対してRBSが宣言するどのようなシグネチャに対しても呼び出しを検証します。バンドルされたプラグインでその形状に達するものはありません ── それらのすべてが、共存するRBSが互換性のあるアリティで宣言するメソッドに答えます ── したがってこのギャップは現在出荷されているセットでは到達不能であり、それを塞ぐと本物のエラーを捕捉する2つのチェックを抑制してしまいます。実際にそれに遭遇するプラグインからの証拠に基づいて決定してください。
...
- `Rigor::Plugin::Inflector` — 実装済みのコンシューマー + Railsファミリープラグインのための共有の語形変化ヘルパー。`underscore` / `camelize` / `singularize` / `pluralize` / `classify`は実際の`ActiveSupport::Inflector`（固定された`ALLOWED_METHODS`許可リスト）に委譲します; `tableize`は意図的に委譲され**ません** ── AS自身の`tableize("Admin::User")`はスラッシュ区切りの`"admin/users"`を返し、決して有効なSQL識別子ではないため、`.tableize`は代わりにASに支えられた`underscore` / `pluralize`を`::` → `_`の平坦化と合成します。ただし、その平坦化は名前空間付きモデルに対するActiveRecordの実際のテーブル名計算では**ありません** ── ARは脱モジュール化（囲みモジュールを完全に除去）し、囲みモジュールが宣言している場合にのみ`table_name_prefix` / `table_name_suffix`を適用するため、`Admin`がプレフィックスを設定しない限り`Admin::User`は`admin_users`ではなく`users`と読まれます。唯一の呼び出し元である`rigor-activerecord`の`ModelIndex.inflected_table_name`は、`tableize`を呼び出す前に名前空間付きモデルのクラス名を脱モジュール化するので、実際には`tableize`に名前空間付きのものを渡しません。これは**近似を一切持ちません**（gemが到達不能なときは例外を投げるので、呼び出し側は黙らせるために辞退します）。`rigor-rails-routes` / `rigor-activerecord` / `rigor-actionpack` / `rigor-actionmailer` / `rigor-factorybot`がこれを使います。
- `Rigor::Plugin::Isolation` — 呼び出しのための**選択可能な分離戦略**で、`.rigor.yml`の`plugins_isolation:`キーまたは`RIGOR_PLUGIN_ISOLATION`によって選ばれます。**環境変数は設定に優先します**: これはオペレーターの1回の呼び出しごとのオーバーライドであり（1台のマシンでforkワーカーが異常動作する場合、forkできないCIイメージ）、コミットされたファイルが拒否権を行使できるオーバーライドはオーバーライドではありません;また、YAMLがパースされる前に、`exe/rigor`が環境変数単独に基づいて`RUBY_BOX=1`で再execするため、`ruby_box`が機能しうる唯一の順序付けでもあります（ADR-87 / ADR-104 boot-slim）。したがって設定ファイル内の`plugins_isolation: ruby_box`は環境変数を名指す`ConfigurationError`となり、別の戦略へ沈黙のうちにフォールバックすることはありません（[#911](https://github.com/rigortype/rigor/issues/911)）。3つのバックエンドにわたる1つの`call(feature:, receiver:, method:, args:)`インターフェースで、**`process`がデフォルト**:
...
| `rbs_complete_extends` | `Array<String>` | `rbs_complete_ancestors`の`extend`エッジのツイン：このプラグインの`signature_paths:`がそのRBS**インスタンス**サーフェスを**完全に**宣言しているモジュール名。リストされた`M`に対する`extend M`を本体に含むRubyソースクラスまたはモジュールは、`M`の宣言されたインスタンスメソッドを通じてシングルトン側の呼び出しを解決する ── `class F; extend T::Sig; sig { ... }; end`は`T::Sig#sig`に到達する（issue #1097）。この探索は検出された各スーパークラス上の`Environment#singleton_extended_modules`も参照するため、`class Doc < T::ImmutableStruct`は`T::ImmutableStruct`自身のRBS宣言された`extend T::Sig`を拾い上げる。スーパークラスブリッジと同じガードが適用される：RBS認識されたレシーバーは直接ルックアップを通じて回答し、より近いソース`def self.x`はブリッジをシャドウし、プロジェクトクラス（または許可リストにないRBSモジュール）へと解決されてメソッドを定義する候補はそのエッジを所有して探索全体を停止させる ── 後続の`extend`やRBSスーパークラスは検索されず、`MacroBlockSelfType`と一致する。宣言されたサーフェスが部分的であるモジュールはここに属さない ── ブリッジが実際の呼び出しを`call.undefined-method`として誤って報告してしまうからである。より近いソース`def self.x`は実行された後にのみシャドウする：`Scope#singleton_def_shadows_call?`は`discovered_deferred_ranges`テーブルを通じて呼び出しに対してそれを順序付ける ── def / block / lambda本体内の呼び出しは呼び出し時に実行され、同じクラスの同名defが既知である場合にのみシャドウされる一方で、先行評価されるクラス本体の呼び出しは、自身の前に開始する同じクラスの同名defによってのみシャドウされる。 |
