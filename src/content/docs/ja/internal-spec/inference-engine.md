...
sourceSha: "82ee8fe9e617077beb393baf65327f9fb0a7ab012e4b8348e69fa1df36fd4459"
sourceCommit: "0f252e3218936e8dc7004b574c709a434b996d2a"
sourceDate: "2026-09-20T20:26:39+09:00"
...
- `Bases::Self`はMUST`self_type:`で置換されます。シングルトンディスパッチでは`Singleton[C]`です。インスタンスディスパッチでは`Nominal[C]`ですが、射影が型引数を運ぶレシーバーは、`MethodDispatcher::SelfSubstitute`（[#1092](https://github.com/rigortype/rigor/issues/1092)）によって構築された**引数付き**の公称型を置換するため、`Array[Integer]#tap {}`は生の`Array`ではなく`Array[Integer]`となります。その置換の規則:
  - すべての型引数は深く拡大されなければなりません（MUST）: 値固定の型（`Constant`、リファインメント、境界付き整数または浮動小数点数）はその公称型の基底へ（`Type::Combinator.widen_value_pinned`）、ネストされた`Tuple`はその拡大された要素union `E`に対する`Array[E]`へ、ネストされた`HashShape`はその拡大されたキーと値のunionに対する`Hash[K, V]`へ（空のものは生の公称型へ）、union、適用された公称型、`Dynamic[T]`を通じて再帰します。ブロックはyieldされたエイリアスを通じて任意の深さでレシーバーをミューテートできますが、`MutationWidening`はそれを追跡しないため、引数内部のリテラルやシェイプのファクトは呼び出しの値を束縛しません: `Array[:a | :b]`と答える`[:a, :b].tap { |l| l << :c }`は`.last == :c`をfalseに畳み込んでしまい、`Array[[1, 2]]`と答える`[[1, 2]].tap { |a| a[0] << 3 }`は`.first.size == 3`を畳み込んでしまいます。
...
公称受理契約（`Rigor::Inference::Acceptance`、`Nominal` self / `Constant` other経路）は以下を満たさなければなりません（MUST）:
...
- `Prism::MultiWriteNode`（Slice 5 phase 2 sub-phase 2） — エントリースコープ下で右辺を一度評価し、それから分解木（`lefts` / `rest` / `rights`、`rest`は`Prism::SplatNode`で`expression`はMAY`Prism::LocalVariableTargetNode`）を歩き、`Rigor::Inference::MultiTargetBinder`を介してすべての名前付きローカルを束縛します。ペアの型はMUST右辺の型と等しくなります（Rubyの`(a, b = [1, 2]) #=> [1, 2]`セマンティクスと一致）。右辺が`Type::Tuple`のとき、スロットごとの束縛はMUST要素ごとです: 欠落した前方/後方スロットは`Constant[nil]`に縮退し、restスロットは中間要素の`Type::Tuple`（ソースに余剰がないとき`Tuple[]`）に束縛されます。`Array[T]`の右辺（その上の`Refined` / `Difference`を含む）は各固定スロットを`T`に、restを`Array[T]`に束縛し、それが束縛するすべての固定スロットは`Scope#with_optimistic_local`を通じても記録されなければなりません（MUST）（issue [#1093](https://github.com/rigortype/rigor/issues/1093);後述の §「Multi-Target Binder」）。unionの右辺はそのメンバー全体に分配され、暗黙の`to_ary`を持たないことが証明可能な値は単一要素のタプル`[rhs]`として束縛されます（issue [#1094](https://github.com/rigortype/rigor/issues/1094);同セクション）。その他の右辺はすべてのスロットを`Dynamic[top]`に束縛します。ネストされた`Prism::MultiTargetNode`ターゲットはMUST同じバインダーを通じて再帰します。インスタンス変数ターゲット（`@a, *@rest = xs`）は同じ規則で分解して`Scope#with_ivar`を通じて束縛されなければならず（MUST）、各楽観的スロットは`Scope#with_optimistic_ivar`を通じて記録されます ── これは単一の`@x = xs.first`書き込みがすでに記録しているマークです（issue [#1110](https://github.com/rigortype/rigor/issues/1110)）。その他の非ローカルターゲット（クラス／グローバル変数、定数、インデックス／呼び出しターゲット、無名splat）は静かにスキップされなければなりません（MUST）。
...
- その他の非ローカルターゲット（`ClassVariableTargetNode`・`GlobalVariableTargetNode`・`ConstantTargetNode`・`IndexTargetNode`・`CallTargetNode`・`ConstantPathTargetNode`・`ImplicitRestNode`・式のない無名splat...）は静かにスキップされなければなりません（MUST）: これらはStatementEvaluatorがスレッドするスコープに観測可能な寄与を持ちません。
...
- Issue #587（b）——要素ごとのTupleの畳み込み（`ExpressionTyper#try_per_element_block_fold`: `Tuple`レシーバーに対する`map` / `select` / `find`のファミリー）は、すべての位置を1つのエントリースコープから型付けするので、本体が再束縛する捕捉された外側のローカルについては初回反復のピン留めになります。したがってこの畳み込みは、上記の書き戻しと同じ「捕捉された再束縛の名前の集合」をMUST計算しなければならず（`Inference::CapturedLocals.writes`。`write_back_block_captures`とサブフェーズ3cのドロップと共有され、3者が食い違えないようにしてあります）、いずれの位置を型付けするより**前に**その上で同じ`BodyFixpoint`をMUST走らせなければならず（呼び出しサイトの束縛からシードし、ブロックパラメータをレシーバーの要素の合併に束縛し、上限3、最後に`Constant→Nominal`へ広げ、収束しない場合は`Dynamic[top]`の床）、そのような各ローカルを、すべての位置のエントリースコープでその収束した型にMUST束縛しなければなりません。パラメータの束縛の下に置くので、パラメータは依然としてシャドウします。本体が再束縛しない名前・ブロックパラメータ・ブロックローカルは、位置ごとの正確な束縛をMUST保たなければなりません——要素だけで判断する述語は依然として畳み込まれます。この畳み込みがブロック本体のスレッディングの抑制の下で走るとき（別のスレッディングされた本体の内側にネストしているとき）は、不動点を走らせる代わりにそれらの名前をMUST`Dynamic[top]`へ束縛しなければならず、不動点の失敗もMUST同じ床を取り、決してシードを取ってはなりません。要素ごとのスレッディングのアリティの上限は床にしません: 不動点のコストはアリティに依存しないからです。したがって`total = 0; [1, 2].map do total += 1; total end`は`[Integer, Integer]`と型付けされ、決して`[1, 1]`にはなりません。
