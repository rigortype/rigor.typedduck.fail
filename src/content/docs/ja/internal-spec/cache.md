...
- エントリーファイルが存在しない —— 存在確認とオープンの間に消えてしまう場合（`Errno::ENOENT`）も含む。これは並行プロセスがルートをクリアまたはコンパクトしているときに見られる挙動である。それ以外のすべての`SystemCallError`（パーミッション、I/O）は依然として送出される: 壊れたファイルシステムは、際限のない再計算へと劣化するのではなく、可視化されたままでなければならない。
...
- マーカーが存在するが**空** → 存在しない場合とまったく同様に扱われる: 現在の値を書き込み、続行し、エントリーを保持する。ディスク利用可能。空であることは何も情報を持たない —— これはインプレース書き込みが途切れたときの状態、作成と書き込みの間のクラッシュが残す状態、そして下記の「クリアしてから再書き込み」自体が通過する状態であり、曖昧なシグナルに対してキャッシュルートを破棄するのは誤った答えである。拒絶しても、エントリーに対して以前持っていなかった信頼を与えるわけではない。なぜなら既存のエントリー上にマーカーがない場合は上記のルールによって保持され提供されるからである。
...
`schema_version.txt`はステップ3〜5のみによって公開されます —— 宛先ロックのない、同じテンポラリファイルからのrenameです。これにはアトミック性が必要です: その読み取り側はロックを取得しない別のプロセスの`Store`コンストラクタであり、切り詰められたマーカーを見た読み取り側はマーカーが不一致であると判断し、読み取り途中の兄弟プロセスの下でルートをクリアしてしまうことになります。ロックは不要です: マーカーのすべての書き込み側は同一の値を書き込むため、最後の書き込み側が勝つという問題を解決する必要がないからです。
