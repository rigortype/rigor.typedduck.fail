...
Rigorのバージョンがそれ自身のコードを識別するのは、リリース済みのgemの場合だけです。gitのチェックアウトからRigorを動かしている場合 —— 自分でパッチを当てている作業コピーや、ブランチを追う`gem "rigor", github:`のように、2つのコミットが1つのバージョン番号を共有する場合 —— 解析結果を保持するエントリーはRigor自身のソースのコンテンツもキーにします。そのため、アナライザーを編集して再実行すると、前回の答えを再生する代わりに再計算されます。これは実行ごとにRigorのソースを1回走査するコスト（約20ミリ秒）を伴います。インストール済みのgemはそれを支払いませんし、必要ともしません。
...
**エフェクトシステムが生み出すものはすべてキャッシュヒットを生き延びます**。ウォームな`rigor check`はコールドなものとまったく同じ`effect.envelope-exceeded`警告を報告します;エフェクトチェックがその日の最初のビルドでは発火し、その後は沈黙する、という設定は存在しません。裁定そのものは決して保存されず、実行のたびにサマリーから再計算されるので、エンベロープ・`tolerated:`リスト・`%a{pure}`アノテーションを編集すれば、Rubyファイルが1つも変わっていなくても次の実行であなたのコードは裁定し直され、警告がそれを正当化したスタンザより長生きすることはあり得ません。

これが速度を犠牲にするのはちょうど1つの場合です。どこかにエンベロープを宣言すると、プロジェクトはキャッシュヒットの高速パスからオプトアウトします。Rigorが裁定し直すためにアナライザーをロードしなければならないからです: 中規模のRailsアプリケーションでは、ウォームな実行が約4分の1秒ではなく約1秒かかります。`effects:`がオンでもどこにも何も宣言していないプロジェクトは何も払いません。
...
スナップショットはキャッシュディレクトリ（`.rigor/cache`）の下に置かれ、設定内容、ロックされたgem、プロジェクト独自の`sig/` RBS、Rigorバージョン、そして —— チェックアウトからRigorを動かしている場合は —— Rigor自身のソースのコンテンツのフィンガープリントをキーとしています。これらのいずれかが変わるとスナップショットは破棄され次回の実行はフル実行になるため、インクリメンタル実行が古い結果を返すことはありません。（フィンガープリントは展開後のファイル一覧ではなく解析の*ルート*（たとえば`["lib"]`）をキーにするため、それらのルート配下のファイルを追加・削除してもスナップショットは破棄され**ません**。インクリメンタルセッションが追加されたファイルと削除されたファイルの依存先を再解析します。）キャッシュの他の部分と同様に、スナップショットが存在しないか破損していた場合は単純にフル実行になります。解析が詰まることはありません。
