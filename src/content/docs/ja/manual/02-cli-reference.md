...
sourceSha: "e71e65f359af60eba78ac8150c3a731b8cb1a874d757a1e7a806f6e5f09aeea2"
...
各メソッドが何を返すかではなく*何をするか* —— そのエフェクトラベル —— を報告し、ドリフトをゲートするコミット対象の**エフェクトスナップショット**を管理します。オプトインかつ観測的です: ここにあるものは何も診断を発しませんし`rigor check`の出力も変えません。非ゼロで終了するのは`rigor effects check`だけです。これらの動詞群を取り巻くワークフロー —— ラベルのボキャブラリー、規模が大きくなったときのレポートの読み方、レビューループとCIステップ —— は[エフェクトラベル](../19-effect-labels/)にあります。
...
パスを渡すことは出力だけでなく**解析**を狭めます。ラベルは解析されたものの上で推移的なので、呼び出し先が解析されなかったメソッドは、プロジェクト全体のランでの同じメソッドより少ないラベルと、より広い「おそらくそれ以上」を報告します —— 正直ではありますが、プロジェクト全体の答えを安価に得る方法ではありません。実際のフットプリントが欲しいときは出力のほうをフィルタしてください。
...
ラベルの後の` ≤ [...]`節は**宣言**レーンです: Rigorが信頼はするが検証はしていないソースが、そのメソッドがすると*主張する*ものです。これには2つのソースが流れ込みます —— あなたが有効化したプラグイン（フレームワークアプリケーションではその大半を占めます。`rails.session.write`をはじめとする`rails.*`はあなたのではなくAction Packの行です）と、何もモデル化していないgemメソッドについてあなたが書いた`effects.attribution:`テーブルです。証明されたラベルとは別に印字され、決してその中に畳み込まれません。2つは異なる問いに答えるからです。証明されたラベルとまったく同じに呼び出しエッジをたどるので、帰属付きgem呼び出しの2ホップ上のコントローラーは「おそらくそれ以上」だけでなく主張を運びます;証明済みリストが既にカバーする宣言ラベルは二重には印字されません。
...
省略されるメソッドは2種類あります: `mutate.local` —— 自身のフレームが確保して決して外に出さないオブジェクトの変更で、あらゆるエフェクトエンベロープが許容するもの —— 以外を何も証明せず、何も主張しないものと、どちらのレーンにもラベルがなく、その下の何かが未解決だったと記録するためだけに存在するものです。`--full`は両方を列挙します。`--pure`は最初のグループを名指しで要求します。これは`%a{pure}`をアノテーションする価値のある集合です。

`--label=LABEL`は、どちらかのレーンで`LABEL`またはその配下のラベルを運ぶメソッドだけを印字します。`--limit=N`は行数に上限を設けます。`--why`は、デフォルトでは件数に畳まれている、各行の未解決理由と各宣言ラベルの背後にあるプラグインの行を展開します。レポートは2つのレーンを別々に数えるフッターで締めくくられます。2つは異なる力を持つからです: 証明されたラベルはビルドを失敗させられますが、宣言されたラベルにはできません（[ADR-103](../../adr/103-effect-labels/) § WD17）。

`PATH`引数はどのメソッドが**印字される**かを選ぶのであって、どれが解析されるかを選ぶことは決してありません: Rigorはどちらにせよ設定済みの`paths:`を解析するので、選択された行はプロジェクト全体のランが与えるのとまったく同じラベルを運び、stderr上の注記が全体のうち何件を見ているかを伝えます。`paths:`の外のパスもそれらと同様に解析されるので、設定がカバーしていないツリーにこのコマンドを向けてもちゃんと動きます。
...
何が収集され、どう伝播するかは[エフェクトサマリー内部仕様](../../internal-spec/effect-summaries/)にあります。`--list-labels`は、このプロジェクトが名指せるボキャブラリー —— 出荷されるすべてのラベルとそのルートの意味、加えてあなたのプラグインと`effects.labels:`が開いたもの —— を印字し、何も解析せずに終了します。同じ表は[エフェクトラベル § ラベルのボキャブラリー](../19-effect-labels/)にあり、文法はgemには同梱されないエフェクトラベル仕様で規範的に定められています: <https://rigor.typedduck.fail/type-specification/effect-labels/>。
...
設定の誤りは、キーを名指す`rigor:`行を1行だけ印字し、それ以外は何も出しません —— バックトレースなしで、どのコマンドでも:
