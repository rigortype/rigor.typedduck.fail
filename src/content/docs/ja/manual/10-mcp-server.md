...
`rigortype`をプロジェクトの`Gemfile`に追加**しないでください**。Rigorはツールであり、ライブラリではありません。[Rigorのインストール §「推奨: ランタイムバージョンマネージャー」](../01-installation/#推奨-ランタイムバージョンマネージャー)を参照してください。
...
指定されたRubyソースファイルに各行の最後の式の型を`#=>`コメントとして付加して返します。`def`ヘッダー行はメソッドの推論戻り値型を表示します。
