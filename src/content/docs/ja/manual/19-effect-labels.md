...
- **`unresolved:`はリストではなくカウントです**。アナライザーが追えなかった呼び出しの件数を示すもので、それが`exhaustive:`がfalseである理由です。原因そのものは記録されません——それは推論品質のメタデータで、Rigorのアップグレードや無関係な編集で揺れるからです——なので、必要になったときに問い合わせてください: `rigor effects explain`がそれらを名指し、`exhaustive → not`の行についても名指します。レビュアーが読む行は`effects:`と`declared:`です。
