...
## Plugin / example reference links

Upstream prose links a plugin or example by its **source** path relative to the
repo root — `[…](../../plugins/rigor-sorbet/)`, `[…](../../examples/rigor-web/)`,
`[…](../../plugins/rigor-playground/wasm/smoke.mjs)`. Those paths escape the
upstream `docs/` tree and have **no on-site route** (the published plugin
reference pages live at `/manual/plugins/<slug>/`, autogenerated from
`src/content/docs/manual/plugins/`; there is no `/plugins/` or `/examples/`
route), so on this site every such link 404s. `sync-rigor-docs.mjs`'s
`rewriteMarkdownLinks` rewrites them:
...
- **Anything else under `plugins/` or `examples/`** (sources, dirs without a
  reference page, the bare `plugins/`/`examples/` listing) → the upstream GitHub
  repo (`tree` for directories, `blob` for files).
- Repo-root escapes **outside** those two trees (`lib/`, `sig/`, `spec/`,
  `references/`, …) keep the historical behavior — `.md` → GitHub, anything else
  passes through untouched — so this rewrite is scoped to the plugin-link
  breakage and does not perturb the body hash (translation-drift `sourceSha`) of
  unrelated pages.
...
`scripts/scan-plugin-links.mjs` is the guard:

- `node scripts/scan-plugin-links.mjs` scans the whole content tree and reports
  any relative link that resolves to a top-level `plugins/`/`examples/` route
  (locale-aware). Expected baseline is **zero**.
- `node scripts/scan-plugin-links.mjs --fix <glob> …` rewrites those in place
