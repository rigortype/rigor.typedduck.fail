...
| --- | --- | --- | --- |
| `TypeCombinator::union` | `Type::Combinator.union` | [`combinator.rb:363`](https://github.com/rigortype/rigor/blob/master/lib/rigor/type/combinator.rb) | ✅ Equivalent (deterministic normalization + lattice identities) |
| `TypeCombinator::intersect` | `Type::Combinator.intersection` | [`combinator.rb:325`](https://github.com/rigortype/rigor/blob/master/lib/rigor/type/combinator.rb) | ✅ Equivalent |
| `TypeCombinator::remove` | `Type::Combinator.difference` (the `T - U` operator) | [`combinator.rb:123`](https://github.com/rigortype/rigor/blob/master/lib/rigor/type/combinator.rb) | ✅ Rigor has an explicit difference operator, and the diagnostic display also has a `D - U` form ([type-operators.md](../../type-specification/type-operators/)). If anything, the expression is richer than PHPStan's |
...
| `Type::isSuperTypeOf` | `Type#accepts(other, mode:)` → `AcceptsResult` | each carrier's `accepts` (e.g. [`constant.rb:114`](https://github.com/rigortype/rigor/blob/master/lib/rigor/type/constant.rb)) | ✅ Implemented as gradual consistency. The strict subtype (`subtype_of`) returning `SubtypeResult` is planned for slice 5+ |
...
| The plugin type-construction facade | `services.type` (= `Type::Combinator`) | [`services.rb:43`](https://github.com/rigortype/rigor/blob/master/lib/rigor/plugin/services.rb) | ✅ Injects a facade that mandates normalization (consistent with PHPStan's "`TypeCombinator` over `new`" policy) |

The plugin extension points ([`plugin/base.rb`](https://github.com/rigortype/rigor/blob/master/lib/rigor/plugin/base.rb)) are `node_rule` (lines 86, 137) / `dynamic_return` (line 210) / `type_specifier` (line 239) / `producer` (line 86). PHPStan's `DynamicMethodReturnTypeExtension` ≈ Rigor's `dynamic_return`, and `TypeSpecifierExtension` ≈ `type_specifier`. **Only a hook corresponding to `OperatorTypeSpecifyingExtension` is missing** (`grep -i operator lib/rigor/plugin/` is empty).
...
We initially hypothesized "Rigor has no plugin binary-operation hook," but **a code spike plus an integration spec on 2026-06-03 refuted it and settled it** ([`spec/integration/plugin_operator_dynamic_return_spec.rb`](https://github.com/rigortype/rigor/blob/master/spec/integration/plugin_operator_dynamic_return_spec.rb), 4 examples green). In Ruby, `a + b` is a `Prism::CallNode` with `name: :+`, and like any ordinary call it flows through `call_type_for` → `MethodDispatcher.dispatch` with `call_node: node` / `method_name: :+` / `scope:` ([`expression_typer.rb:1233`](https://github.com/rigortype/rigor/blob/master/lib/rigor/inference/expression_typer.rb)). The dispatch priority is **`ConstantFolding` (precise tiers) → `try_plugin_contribution` (`dynamic_return`) → RBS** ([`method_dispatcher.rb:74-97`](https://github.com/rigortype/rigor/blob/master/lib/rigor/inference/method_dispatcher.rb)). A plugin-owned receiver is `Nominal[CustomType]`, not `Constant` / `IntegerRange`, so ConstantFolding returns `nil`, and **dispatch falls through to the plugin tier**. `dynamic_return_type` gates only on the receiver class and does not consider the method name at all ([`base.rb:382`](https://github.com/rigortype/rigor/blob/master/lib/rigor/plugin/base.rb)). The spec confirms that the block fires for all of `:+ :- :* :/` and that the contributed type is settled as the result type of `(a <op> b)`.
